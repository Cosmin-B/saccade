...
constexpr uint8_t full_target_fields = SACCADE_SCENE_DELTA_OWNER | SACCADE_SCENE_DELTA_GEOMETRY | SACCADE_SCENE_DELTA_CONFIDENCE |
                                       SACCADE_SCENE_DELTA_CLASSIFICATION | SACCADE_SCENE_DELTA_CAPABILITIES | SACCADE_SCENE_DELTA_FLAGS |
...
bool same_geometry(const SaccadeTargetRecord& left, const SaccadeTargetRecord& right) noexcept {
    return left.x_q8 == right.x_q8 && left.y_q8 == right.y_q8 && left.width_q8 == right.width_q8 && left.height_q8 == right.height_q8 &&
           left.safe_x_q8 == right.safe_x_q8 && left.safe_y_q8 == right.safe_y_q8;
}
...
SaccadeSpanU8 previous_text(const TemporalStorage& storage, const SaccadeTargetRecord& target) noexcept {
    return target.text.size == 0 ? SaccadeSpanU8{} : SaccadeSpanU8{storage.text.data() + target.text.offset, target.text.size};
}

bool same_text(SaccadeSpanU8 left, SaccadeSpanU8 right) noexcept {
    return left.size == right.size && (left.size == 0 || std::memcmp(left.data, right.data, static_cast<size_t>(left.size)) == 0);
}

uint8_t changed_fields(const SaccadeTargetRecord& previous, SaccadeSpanU8 previous_target_text, const SaccadeTargetRecord& current,
                       SaccadeSpanU8 current_target_text) noexcept {
    uint8_t changed = 0;
    if (!same_owner(previous, current))
        changed |= SACCADE_SCENE_DELTA_OWNER;
    if (!same_geometry(previous, current))
        changed |= SACCADE_SCENE_DELTA_GEOMETRY;
    if (previous.confidence_q16 != current.confidence_q16)
        changed |= SACCADE_SCENE_DELTA_CONFIDENCE;
    if (!same_classification(previous, current))
        changed |= SACCADE_SCENE_DELTA_CLASSIFICATION;
    if (previous.capability_bits != current.capability_bits)
        changed |= SACCADE_SCENE_DELTA_CAPABILITIES;
    if (previous.flags != current.flags)
        changed |= SACCADE_SCENE_DELTA_FLAGS;
    if (previous.order != current.order)
        changed |= SACCADE_SCENE_DELTA_ORDER;
    if (!same_text(previous_target_text, current_target_text))
        changed |= SACCADE_SCENE_DELTA_TEXT;
    return changed;
}
...
size_t payload_size(uint8_t fields, size_t text_size) noexcept {
    ...
    if ((fields & SACCADE_SCENE_DELTA_OWNER) != 0)
        size += sizeof(SaccadeSceneDeltaOwner);
    if ((fields & SACCADE_SCENE_DELTA_GEOMETRY) != 0)
        size += sizeof(SaccadeSceneDeltaGeometry);
    if ((fields & SACCADE_SCENE_DELTA_CONFIDENCE) != 0)
        size += sizeof(uint32_t);
    if ((fields & SACCADE_SCENE_DELTA_CLASSIFICATION) != 0)
        size += sizeof(SaccadeSceneDeltaClassification);
    if ((fields & SACCADE_SCENE_DELTA_CAPABILITIES) != 0)
        size += sizeof(uint32_t);
    if ((fields & SACCADE_SCENE_DELTA_FLAGS) != 0)
        size += sizeof(uint32_t);
    if ((fields & SACCADE_SCENE_DELTA_ORDER) != 0)
        size += sizeof(uint32_t);
    if ((fields & SACCADE_SCENE_DELTA_TEXT) != 0)
        size += text_size;
    return size;
}
...
uint32_t delta_flags(const SaccadeTargetPacketHeader& current, bool baseline, bool reset) noexcept {
    ...
    if (reset)
        flags |= SACCADE_SCENE_DELTA_RESET;
    if ((current.flags & SACCADE_TARGET_PACKET_INCOMPLETE) != 0)
        flags |= SACCADE_SCENE_DELTA_SOURCE_INCOMPLETE;
    if ((current.flags & SACCADE_TARGET_PACKET_TEXT_TRUNCATED) != 0)
        flags |= SACCADE_SCENE_DELTA_SOURCE_TEXT_TRUNCATED;
    return flags;
}

SaccadeSceneDeltaHeader make_header(const SaccadeTargetPacketHeader& previous, const SaccadeTargetPacketHeader& current, uint32_t flags,
                                    uint32_t operation_count, size_t payload_bytes) noexcept {
    ...
    static_assert(temporal_delta_max_bytes <= UINT32_MAX);
    header.payload_offset = static_cast<uint32_t>(header.operations_offset + operation_count * sizeof(SaccadeSceneDeltaRecord));
...
uint8_t* write_payload(uint8_t* output, const SaccadeTargetRecord& target, SaccadeSpanU8 text, uint8_t fields) noexcept {
    if ((fields & SACCADE_SCENE_DELTA_OWNER) != 0) {
    ...
    if ((fields & SACCADE_SCENE_DELTA_FLAGS) != 0)
        output = write_bytes(output, &target.flags, sizeof(target.flags));
    if ((fields & SACCADE_SCENE_DELTA_ORDER) != 0)
        output = write_bytes(output, &target.order, sizeof(target.order));
    if ((fields & SACCADE_SCENE_DELTA_TEXT) != 0 && text.size != 0)
        output = write_bytes(output, text.data, text.size);
    return output;
}

void write_record(SaccadeSceneDeltaRecord* record, uint8_t** payload, const SaccadeTargetRecord& target, SaccadeSpanU8 text, uint8_t fields,
                  SaccadeSceneDeltaOperation operation, const uint8_t* batch) noexcept {
...
void TemporalCompiler::build_window_transforms(const PacketView& current, TemporalStats* stats) noexcept {
    ...
    for (uint32_t index = 0; index < current.header->target_count; ++index) {
        ...
        transform.valid &= translation_x == transform.translation_x_q8 && translation_y == transform.translation_y_q8 &&
                           ...
                           target.safe_y_q8 - previous.safe_y_q8 == transform.translation_y_q8 && target.width_q8 == previous.width_q8 &&
                           target.height_q8 == previous.height_q8;
        transform.window_role |= target.role == SACCADE_TARGET_ROLE_WINDOW;
        transform.flags |= target.display_id != previous.display_id ? SACCADE_SCENE_WINDOW_TRANSFORM_DISPLAY_CHANGED : 0;
...
void TemporalCompiler::adopt(const PacketView& current) noexcept {
    ...
    std::memcpy(storage_->targets.data(), current.targets, static_cast<size_t>(previous_target_count_) * sizeof(SaccadeTargetRecord));
...
SaccadeResult TemporalCompiler::compile(const PacketView& current, SaccadeMutableSpanU8 output, size_t* required,
    ...
    if (!initialized_ || current.header == nullptr || current.targets == nullptr || required == nullptr || stats == nullptr ||
        current.header->coordinate_space != SACCADE_COORDINATE_SPACE_DESKTOP_Q8 ||
    ...
    *stats = {};
    ...
    stats->transform_changes = !baseline && current.header->transform_epoch != previous_header_.transform_epoch ? 1U : 0U;
    ...
    if (reset) {
        ...
        for (uint32_t index = 0; index < current.header->target_count; ++index) {
            ...
            counts.payload_bytes += payload_size(full_target_fields | (text.size != 0 ? SACCADE_SCENE_DELTA_TEXT : 0), text.size);
        }
    } else {
    ...
    const size_t total_size =
        sizeof(SaccadeSceneDeltaHeader) + static_cast<size_t>(counts.operations) * sizeof(SaccadeSceneDeltaRecord) + counts.payload_bytes;
    *required = total_size;
    ...
    if (counts.window_transforms != 0)
        flags |= SACCADE_SCENE_DELTA_WINDOW_TRANSFORMS;
    if (stats->transform_changes != 0)
        flags |= SACCADE_SCENE_DELTA_TRANSFORM_CHANGED;
    if (stats->topology_changes != 0)
        flags |= SACCADE_SCENE_DELTA_TOPOLOGY_CHANGED;
    SaccadeSceneDeltaHeader header = make_header(previous_header_, *current.header, flags, counts.operations, counts.payload_bytes);
    ...
    if (reset) {
        for (uint32_t index = 0; index < previous_target_count_; ++index) {
            write_record(&records[operation_index++], &payload, storage_->targets[index], {}, 0, SACCADE_SCENE_DELTA_REMOVE, output.data);
        }
        for (uint32_t index = 0; index < current.header->target_count; ++index) {
            const SaccadeSpanU8 text = current.target_text(index);
            const uint8_t fields = full_target_fields | (text.size != 0 ? SACCADE_SCENE_DELTA_TEXT : 0);
            write_record(&records[operation_index++], &payload, current.targets[index], text, fields, SACCADE_SCENE_DELTA_ADD, output.data);
        }
    } else {
        for (uint32_t index = 0; index < window_transform_count_; ++index) {
            ...
            const SaccadeSceneWindowTransform payload_value{transform.display_id, transform.translation_x_q8, transform.translation_y_q8,
                                                            transform.target_count, transform.flags};
        ...
        for (uint32_t index = 0; index < current.header->target_count; ++index) {
            ...
            if (previous_index == missing_target_) {
                const uint8_t fields = full_target_fields | (text.size != 0 ? SACCADE_SCENE_DELTA_TEXT : 0);
                write_record(&records[operation_index++], &payload, target, text, fields, SACCADE_SCENE_DELTA_ADD, output.data);
                continue;
            }
            const SaccadeTargetRecord& previous = storage_->targets[previous_index];
            uint8_t fields = changed_fields(previous, previous_text(*storage_, previous), target, text);
            ...
            if (fields != 0) {
                write_record(&records[operation_index++], &payload, target, text, fields, SACCADE_SCENE_DELTA_UPDATE, output.data);
            }
        }
        for (uint32_t index = 0; index < previous_target_count_; ++index) {
            if (storage_->seen[index] == 0) {
                write_record(&records[operation_index++], &payload, storage_->targets[index], {}, 0, SACCADE_SCENE_DELTA_REMOVE,
                             output.data);
            }
        }
    }
