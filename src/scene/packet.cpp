...
constexpr uint16_t source_mask =
...
constexpr uint32_t target_flag_mask = SACCADE_TARGET_ACTIONABLE | SACCADE_TARGET_DISABLED | SACCADE_TARGET_OCCLUDED |
                                      SACCADE_TARGET_SECURE | SACCADE_TARGET_APPROXIMATE | SACCADE_TARGET_TEXT_REDACTED |
...
constexpr uint32_t target_capability_mask =
    ...
    SACCADE_TARGET_CAPABILITY_INVOKE | SACCADE_TARGET_CAPABILITY_WINDOW_ACTIVATE | SACCADE_TARGET_CAPABILITY_TEXT_SELECT;
...
bool valid_target(const SaccadeTargetRecord& target) noexcept {
    if (target.width_q8 <= 0 || target.height_q8 <= 0 || target.confidence_q16 > UINT16_MAX || target.role > SACCADE_TARGET_ROLE_WINDOW ||
        (target.source_bits & static_cast<uint16_t>(~source_mask)) != 0 || (target.capability_bits & ~target_capability_mask) != 0 ||
        (target.flags & ~target_flag_mask) != 0 || target.target_id == 0 || target.parent_id == target.target_id ||
        ...
        ((target.flags & (SACCADE_TARGET_TEXT_REDACTED | SACCADE_TARGET_TEXT_TRUNCATED)) != 0 && target.text.size != 0) ||
    ...
    return target.safe_x_q8 >= target.x_q8 && target.safe_y_q8 >= target.y_q8 && static_cast<int64_t>(target.safe_x_q8) < right &&
           static_cast<int64_t>(target.safe_y_q8) < bottom;
}
...
SaccadeResult validate_packet(SaccadeSpanU8 bytes, PacketView* output) noexcept {
    ...
    *output = {};
    ...
    if (header->struct_size != sizeof(SaccadeTargetPacketHeader) || header->packet_version != SACCADE_TARGET_PACKET_VERSION ||
        header->target_count > SACCADE_TARGET_PACKET_MAX_TARGETS || header->target_stride != sizeof(SaccadeTargetRecord) ||
        (header->flags & ~packet_flag_mask) != 0 || !valid_coordinate_space(header->coordinate_space) || header->frame_id == 0 ||
        header->model_epoch == 0 || header->session_epoch == 0 || header->transform_epoch == 0 || header->topology_epoch == 0 ||
        header->source_id == 0 || header->targets_offset != sizeof(SaccadeTargetPacketHeader) || header->total_size > bytes.size) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    ...
    if (header->total_size != text_offset + expected_text_offset)
        return SACCADE_ERROR_INVALID_ARGUMENT;
