...
uint16_t rectangle_iou_q16(const SaccadeTargetRecord& left, const SaccadeTargetRecord& right) noexcept {
    ...
    const int64_t intersection_right =
        std::min<int64_t>(static_cast<int64_t>(left.x_q8) + left.width_q8, static_cast<int64_t>(right.x_q8) + right.width_q8);
    const int64_t intersection_bottom =
        std::min<int64_t>(static_cast<int64_t>(left.y_q8) + left.height_q8, static_cast<int64_t>(right.y_q8) + right.height_q8);
    if (intersection_left >= intersection_right || intersection_top >= intersection_bottom) {
        return 0;
    }

    const uint64_t intersection =
        static_cast<uint64_t>(intersection_right - intersection_left) * static_cast<uint64_t>(intersection_bottom - intersection_top);
...
uint32_t center_distance_squared_q16(const SaccadeTargetRecord& left, const SaccadeTargetRecord& right) noexcept {
    ...
    const uint64_t scale = static_cast<uint64_t>(std::max({left.width_q8, left.height_q8, right.width_q8, right.height_q8}));
...
SaccadeTargetRecord predicted_target(const VisualTargetTrackerTrack& track) noexcept {
    ...
    predicted.width_q8 = std::max(1, saturated_i32(static_cast<int64_t>(predicted.width_q8) + track.velocity_width_q8 * steps));
    predicted.height_q8 = std::max(1, saturated_i32(static_cast<int64_t>(predicted.height_q8) + track.velocity_height_q8 * steps));
...
bool same_window_geometry(const VisualTargetTrackerWindow& left, const VisualTargetTrackerWindow& right) noexcept {
    return left.window_id == right.window_id && left.display_id == right.display_id && left.x_q8 == right.x_q8 && left.y_q8 == right.y_q8 &&
           left.width_q8 == right.width_q8 && left.height_q8 == right.height_q8;
}
...
void relative_geometry(const SaccadeTargetRecord& target, const VisualTargetTrackerWindow& window, int32_t* center_x_q16,
                       int32_t* center_y_q16, int32_t* width_q16, int32_t* height_q16) noexcept {
...
uint32_t relative_distance_squared_q16(const VisualTargetTrackerTrack& track, const SaccadeTargetRecord& target,
                                       const VisualTargetTrackerWindow& window, uint32_t* size_difference_q16) noexcept {
    ...
    *size_difference_q16 =
        static_cast<uint32_t>(std::min<uint64_t>(absolute_u64(static_cast<int64_t>(width_q16) - track.relative_width_q16) +
...
bool config_valid(const VisualTargetTrackerConfig& config) noexcept {
    return config.maximum_tracks != 0 && config.maximum_tracks <= visual_target_tracker_maximum_tracks &&
           config.maximum_missed_frames != 0 && config.minimum_predicted_iou_q16 != 0 && config.maximum_center_distance_q8 != 0 &&
           config.maximum_window_center_distance_q16 != 0 && config.maximum_size_ratio_q8 >= 256 && config.reserved[0] == 0 &&
...
uint32_t collect_windows(VisualTargetTrackerStorage* storage, const SaccadeTargetPacketHeader& header,
    ...
    for (uint32_t index = 0; index < header.target_count && count < visual_target_tracker_maximum_windows; ++index) {
        ...
        storage->windows[count++] = {window_id, target.display_id, target.x_q8, target.y_q8, target.width_q8, target.height_q8};
    }
    return count;
}
...
bool association_edge(const VisualTargetTrackerTrack& track, const SaccadeTargetRecord& target, const VisualTargetTrackerWindow* window,
                      const VisualTargetTrackerConfig& config, uint16_t target_slot, uint16_t track_slot,
                      VisualTargetTrackerEdge* output) noexcept {
    if (!compatible_owner(track, target) || !compatible_role(track.target.role, target.role) || !compatible_source(track, target) ||
    ...
    const uint32_t center_limit_q16 = static_cast<uint32_t>(config.maximum_center_distance_q8) * config.maximum_center_distance_q8;
    ...
    const uint32_t relative_limit_q16 = static_cast<uint32_t>(
        static_cast<uint64_t>(config.maximum_window_center_distance_q16) * config.maximum_window_center_distance_q16 >> 16U);
    ...
    if (use_relative && relative_distance_q16 <= relative_limit_q16) {
        score = static_cast<uint64_t>(relative_distance_q16) * 32U + static_cast<uint64_t>(relative_size_difference_q16) * 4U;
    } else {
        score = static_cast<uint64_t>(UINT16_MAX - iou_q16) * 8U + std::min<uint32_t>(center_distance_q16, unit_q16 * 16U);
    }
...
void insert_best_edge(std::array<VisualTargetTrackerEdge, visual_target_tracker_edges_per_target>* edges, uint32_t* count,
                      const VisualTargetTrackerEdge& edge) noexcept {
...
void update_relative_geometry(VisualTargetTrackerTrack* track, const VisualTargetTrackerWindow* window) noexcept {
    ...
    relative_geometry(track->target, *window, &track->relative_center_x_q16, &track->relative_center_y_q16, &track->relative_width_q16,
                      &track->relative_height_q16);
}

void update_track(VisualTargetTrackerTrack* track, const SaccadeTargetRecord& target, const VisualTargetTrackerWindow* window,
    ...
    if (frame_id > track->last_frame_id) {
        ...
        track->velocity_width_q8 = saturated_i32((static_cast<int64_t>(target.width_q8) - track->target.width_q8) / steps);
        track->velocity_height_q8 = saturated_i32((static_cast<int64_t>(target.height_q8) - track->target.height_q8) / steps);
...
SaccadeResult VisualTargetTracker::initialize(VisualTargetTrackerStorage* storage, const VisualTargetTrackerConfig& config) noexcept {
    if (initialized_) {
...
SaccadeResult VisualTargetTracker::remap(SaccadeTargetPacketHeader* header, SaccadeTargetRecord* targets,
    ...
    if (!initialized_ || header == nullptr || targets == nullptr || stats == nullptr ||
        header->coordinate_space != SACCADE_COORDINATE_SPACE_DESKTOP_Q8 || header->target_count > SACCADE_TARGET_PACKET_MAX_TARGETS ||
        header->scene_epoch == 0 || header->frame_id == 0 || header->session_epoch == 0) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    if (session_epoch_ == header->session_epoch && model_epoch_ == header->model_epoch && topology_epoch_ == header->topology_epoch &&
        last_scene_epoch_ != 0 && (header->scene_epoch <= last_scene_epoch_ || header->frame_id < last_frame_id_)) {
        return SACCADE_ERROR_STALE_HANDLE;
    }
    if (session_epoch_ != 0 &&
        (session_epoch_ != header->session_epoch || model_epoch_ != header->model_epoch || topology_epoch_ != header->topology_epoch)) {
        reset();
    }
    ...
    *stats = {};
    ...
    for (uint32_t target_slot = 0; target_slot < target_count; ++target_slot) {
        ...
        for (uint32_t track_slot = 0; track_slot < config_.maximum_tracks; ++track_slot) {
            ...
            if (association_edge(track, target, window, config_, static_cast<uint16_t>(target_slot), static_cast<uint16_t>(track_slot),
                                 &edge)) {
