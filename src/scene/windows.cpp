...
SaccadeResult build_window_scene(const WindowSceneConfig& config, const SaccadeWindowInfo* windows, uint32_t count,
    ...
    if (byte_size == nullptr || windows == nullptr || count == 0 || count > SACCADE_TARGET_PACKET_MAX_TARGETS || config.scene_epoch == 0 ||
        config.frame_id == 0 || config.model_epoch == 0 || config.session_epoch == 0 || config.transform_epoch == 0 ||
        config.topology_epoch == 0 || config.source_id == 0)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    uint32_t text_size = 0;
    uint32_t packet_flags = 0;
    for (uint32_t index = 0; index < count; ++index) {
        const SaccadeSpanU8 title = windows[index].title;
        if (title.size > UINT16_MAX || !valid_utf8(title))
            return SACCADE_ERROR_INVALID_ARGUMENT;
    ...
    const size_t required = sizeof(SaccadeTargetPacketHeader) + static_cast<size_t>(count) * sizeof(SaccadeTargetRecord) + text_size;
    *byte_size = required;
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        if (window.stable_id == 0 || window.process_id == 0 || window.desktop_bounds.width <= 0 || window.desktop_bounds.height <= 0 ||
            !q8(window.desktop_bounds.x, &target.x_q8) || !q8(window.desktop_bounds.y, &target.y_q8) ||
            !q8(window.desktop_bounds.width, &target.width_q8) || !q8(window.desktop_bounds.height, &target.height_q8))
            return SACCADE_ERROR_INVALID_ARGUMENT;
