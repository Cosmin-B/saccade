...
constexpr size_t temporal_full_target_payload_bytes = sizeof(SaccadeSceneDeltaOwner) + sizeof(SaccadeSceneDeltaGeometry) +
                                                      sizeof(uint32_t) + sizeof(SaccadeSceneDeltaClassification) + 3U * sizeof(uint32_t);
constexpr size_t temporal_delta_max_bytes =
    sizeof(SaccadeSceneDeltaHeader) + static_cast<size_t>(SACCADE_SCENE_DELTA_MAX_OPERATIONS) * sizeof(SaccadeSceneDeltaRecord) +
    static_cast<size_t>(SACCADE_TARGET_PACKET_MAX_TARGETS) * temporal_full_target_payload_bytes +
    static_cast<size_t>(SACCADE_TARGET_PACKET_MAX_TARGETS) * sizeof(SaccadeSceneWindowTransform) + SACCADE_TARGET_PACKET_MAX_TEXT_BYTES;
