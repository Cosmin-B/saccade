...
constexpr std::array<BindingKey, binding_key_count> keys{
    {{0x04, "A"},     {0x05, "B"},    {0x06, "C"},    {0x07, "D"},  {0x08, "E"},     {0x09, "F"},      {0x0a, "G"},         {0x0b, "H"},
     ...
     {0x24, "7"},     {0x25, "8"},    {0x26, "9"},    {0x27, "0"},  {0x28, "Enter"}, {0x29, "Escape"}, {0x2a, "Backspace"}, {0x2b, "Tab"},
     {0x2c, "Space"}, {0x2d, "-"},    {0x2e, "="},    {0x2f, "["},  {0x30, "]"},     {0x31, "\\"},     {0x33, ";"},         {0x34, "'"},
     {0x35, "`"},     {0x36, ","},    {0x37, "."},    {0x38, "/"},  {0x3a, "F1"},    {0x3b, "F2"},     {0x3c, "F3"},        {0x3d, "F4"},
     {0x3e, "F5"},    {0x3f, "F6"},   {0x40, "F7"},   {0x41, "F8"}, {0x42, "F9"},    {0x43, "F10"},    {0x44, "F11"},       {0x45, "F12"},
...
constexpr std::array<KeyPlacement, binding_key_count> key_placements{{
    {0x29, 0, 0, 2},  {0x3a, 4, 0, 2},  {0x3b, 6, 0, 2},  {0x3c, 8, 0, 2},  {0x3d, 10, 0, 2}, {0x3e, 13, 0, 2}, {0x3f, 15, 0, 2},
    {0x40, 17, 0, 2}, {0x41, 19, 0, 2}, {0x42, 22, 0, 2}, {0x43, 24, 0, 2}, {0x44, 26, 0, 2}, {0x45, 28, 0, 2},

    {0x1e, 0, 1, 2},  {0x1f, 2, 1, 2},  {0x20, 4, 1, 2},  {0x21, 6, 1, 2},  {0x22, 8, 1, 2},  {0x23, 10, 1, 2}, {0x24, 12, 1, 2},
    {0x25, 14, 1, 2}, {0x26, 16, 1, 2}, {0x27, 18, 1, 2}, {0x2d, 20, 1, 2}, {0x2e, 22, 1, 2}, {0x2a, 24, 1, 4},

    {0x2b, 0, 2, 3},  {0x14, 3, 2, 2},  {0x1a, 5, 2, 2},  {0x08, 7, 2, 2},  {0x15, 9, 2, 2},  {0x17, 11, 2, 2}, {0x1c, 13, 2, 2},
    {0x18, 15, 2, 2}, {0x0c, 17, 2, 2}, {0x12, 19, 2, 2}, {0x13, 21, 2, 2}, {0x2f, 23, 2, 2}, {0x30, 25, 2, 2}, {0x31, 27, 2, 2},

    {0x04, 2, 3, 2},  {0x16, 4, 3, 2},  {0x07, 6, 3, 2},  {0x09, 8, 3, 2},  {0x0a, 10, 3, 2}, {0x0b, 12, 3, 2}, {0x0d, 14, 3, 2},
    {0x0e, 16, 3, 2}, {0x0f, 18, 3, 2}, {0x33, 20, 3, 2}, {0x34, 22, 3, 2}, {0x28, 24, 3, 4},

    {0x1d, 3, 4, 2},  {0x1b, 5, 4, 2},  {0x06, 7, 4, 2},  {0x19, 9, 4, 2},  {0x05, 11, 4, 2}, {0x11, 13, 4, 2}, {0x10, 15, 4, 2},
    {0x36, 17, 4, 2}, {0x37, 19, 4, 2}, {0x38, 21, 4, 2}, {0x52, 26, 4, 2},
...
SaccadeResult layout_binding_keyboard(int32_t width, int32_t height, int32_t gap, BindingKeyboardLayout* output) noexcept {
    if (output == nullptr || width < static_cast<int32_t>(keyboard_width_units) || height < static_cast<int32_t>(keyboard_height_units) ||
        gap < 0) {
    ...
    *output = {};
    for (const KeyPlacement& placement : key_placements) {
        ...
        const int32_t left = static_cast<int32_t>(placement.x_units * static_cast<uint32_t>(width) / keyboard_width_units);
        const int32_t right =
            static_cast<int32_t>((placement.x_units + placement.width_units) * static_cast<uint32_t>(width) / keyboard_width_units);
        const int32_t top = static_cast<int32_t>(placement.y_units * static_cast<uint32_t>(height) / keyboard_height_units);
        const int32_t bottom = static_cast<int32_t>((placement.y_units + 1U) * static_cast<uint32_t>(height) / keyboard_height_units);
        key = {placement.usage, left, top, std::max<int32_t>(1, right - left - gap), std::max<int32_t>(1, bottom - top - gap)};
    }
    return SACCADE_OK;
}

uint16_t default_logical_symbol(uint32_t physical_key) noexcept {
    if (physical_key >= 0x04 && physical_key <= 0x1d)
        return static_cast<uint16_t>('A' + physical_key - 0x04);
    if (physical_key >= 0x1e && physical_key <= 0x26)
        return static_cast<uint16_t>('1' + physical_key - 0x1e);
    if (physical_key == 0x27)
...
SaccadeResult find_binding(const SettingsDocument& settings, Command command, uint32_t* index) noexcept {
    if (!command_valid(command) || index == nullptr)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    *index = UINT32_MAX;
...
SaccadeResult remove_binding(SettingsDocument* settings, Command command) noexcept {
    if (settings == nullptr || !command_valid(command))
        return SACCADE_ERROR_INVALID_ARGUMENT;
