...
SaccadeResult DesktopHost::dispatch(const CommandEvent& event) noexcept {
    if (!initialized_ || event.timestamp_ns == 0)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    ++stats_.commands;
...
void DesktopHost::observe_physical_input(uint64_t timestamp_ns) noexcept {
    ...
    if (callbacks_.observe_input != nullptr)
        callbacks_.observe_input(callbacks_.context, timestamp_ns);
}
