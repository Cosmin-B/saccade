...
bool valid(const SaccadeWindowInfo& window) noexcept {
    return window.stable_id != 0 && window.process_id != 0 && window.desktop_bounds.width > 0 && window.desktop_bounds.height > 0 &&
           window.reserved0 == 0;
}
    ...
    *output = {};
    for (uint32_t index = 0; index < window_navigation_capacity; ++index) {
        ...
        if (result == SACCADE_ERROR_NOT_FOUND)
            return output->count == 0 ? SACCADE_ERROR_NOT_FOUND : SACCADE_OK;
        if (result != SACCADE_OK)
            return result;
        if (!valid(window) || window.process_id == excluded_process_id)
            continue;
    ...
    return provider.ops.enumerate_windows(provider.context, window_navigation_capacity, &extra) == SACCADE_ERROR_NOT_FOUND
               ? SACCADE_OK
...
SaccadeResult WindowNavigator::cycle(const WindowSnapshot& snapshot, uint64_t current_window_id, bool forward,
    ...
    for (uint32_t distance = 1; distance < snapshot.count; ++distance) {
        const uint32_t candidate = forward ? (current + distance) % snapshot.count : (current + snapshot.count - distance) % snapshot.count;
        if (!overlaps(snapshot.windows[current], snapshot.windows[candidate]))
            continue;
...
SaccadeResult WindowNavigator::behind(const WindowSnapshot& snapshot, uint64_t current_window_id, uint64_t* output) const noexcept {
    return cycle(snapshot, current_window_id, true, output);
}

SaccadeResult WindowNavigator::directional(const WindowSnapshot& snapshot, uint64_t current_window_id, WindowDirection direction,
                                           uint64_t* output) const noexcept {
    if (output == nullptr || snapshot.count == 0 || snapshot.count > snapshot.windows.size() || direction < WindowDirection::left ||
    ...
    for (uint32_t index = 0; index < snapshot.count; ++index) {
        ...
        if (candidate.stable_id == current_window_id)
            continue;
        ...
        const uint64_t primary =
            static_cast<uint64_t>(direction == WindowDirection::left || direction == WindowDirection::right ? dx < 0 ? -dx : dx
                                  : dy < 0                                                                  ? -dy
                                                                                                            : dy);
        const uint64_t secondary =
            static_cast<uint64_t>(direction == WindowDirection::left || direction == WindowDirection::right ? dy < 0 ? -dy : dy
                                  : dx < 0                                                                  ? -dx
                                                                                                            : dx);
