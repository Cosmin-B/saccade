...
bool action_compatible(interaction::SelectionMode mode, interaction::ActionKind action) noexcept {
    ...
    if (mode == interaction::SelectionMode::single)
        return true;
...
interaction::SelectionContext selection_context(const interaction::ActionContext& context) noexcept {
    return {context.scene_epoch, context.transform_epoch, context.topology_epoch, context.focus_id, context.deadline_ns};
}

constexpr std::array<geometry::PointQ8, 9> target_positions{{{1, 1}, {0, 0}, {1, 0}, {2, 0}, {2, 1}, {2, 2}, {1, 2}, {0, 2}, {0, 1}}};

uint32_t position_index(int32_t column, int32_t row) noexcept {
    for (uint32_t index = 0; index < target_positions.size(); ++index)
        if (target_positions[index].x == column && target_positions[index].y == row)
            return index;
    return 0;
}
...
SaccadeResult SessionEngine::initialize(scene::SceneStore* scenes, SessionStorage* storage, PlanExecutor executor) noexcept {
    if (initialized_)
        return SACCADE_ERROR_ALREADY_EXISTS;
    if (scenes == nullptr || storage == nullptr || executor.execute == nullptr)
...
SaccadeResult SessionEngine::begin_scene(const SessionConfig& config, const scene::PacketView& latest) noexcept {
    if (!action_compatible(config.mode, config.request.kind) || config.action.plan_id == 0 || config.action.scene_epoch == 0 ||
        config.action.transform_epoch == 0 || config.action.topology_epoch == 0 || config.action.permission_epoch == 0 ||
        config.action.deadline_ns == 0 || config.action.now_ns >= config.action.deadline_ns ||
        config.request.text.size > storage_->text.size() || (config.request.text.size != 0 && config.request.text.data == nullptr)) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }

    if (latest.header->scene_epoch != config.action.scene_epoch || latest.header->transform_epoch != config.action.transform_epoch ||
...
SaccadeResult SessionEngine::refresh_latest(const SessionEpochs& current) noexcept {
    ...
    if (result != SACCADE_OK || latest.header->scene_epoch != current.scene_epoch ||
        latest.header->transform_epoch != current.transform_epoch || latest.header->topology_epoch != current.topology_epoch) {
    ...
    if (result == SACCADE_OK)
        result = hints_.refresh_scene(owned_scene);
    if (result == SACCADE_OK)
        result = selection_.refresh_scene(owned_scene);
    if (result != SACCADE_OK) {
        ++stats_.refresh_failures;
        return SACCADE_ERROR_STALE_HANDLE;
    }
...
SaccadeResult SessionEngine::cycle_target_position() noexcept {
    if (!active_)
        return SACCADE_ERROR_STATE;
    target_position_ = target_position_ == UINT32_MAX ? 0U : (target_position_ + 1U) % static_cast<uint32_t>(target_positions.size());
    nudge_x_q8_ = 0;
    nudge_y_q8_ = 0;
    return SACCADE_OK;
}
...
SaccadeResult SessionEngine::set_target_position(uint32_t grid_index) noexcept {
    if (!active_ || grid_index >= target_positions.size())
        return SACCADE_ERROR_INVALID_ARGUMENT;
...
SaccadeResult SessionEngine::snap_target(TargetSnapDirection direction) noexcept {
    if (!active_ || direction < TargetSnapDirection::left || direction > TargetSnapDirection::down)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    geometry::PointQ8 position = target_position_ == UINT32_MAX ? geometry::PointQ8{1, 1} : target_positions[target_position_];
    switch (direction) {
...
SaccadeResult SessionEngine::nudge_target(int32_t delta_x_q8, int32_t delta_y_q8) noexcept {
    if (!active_ || (delta_x_q8 == 0 && delta_y_q8 == 0))
        return SACCADE_ERROR_INVALID_ARGUMENT;
...
geometry::PointQ8 SessionEngine::adjusted_point(const SaccadeTargetRecord& target) const noexcept {
    ...
    if (target_position_ != UINT32_MAX) {
        ...
        const std::array<int64_t, 3> xs{target.x_q8 + inset_x, target.x_q8 + maximum_x / 2, target.x_q8 + maximum_x - inset_x};
        const std::array<int64_t, 3> ys{target.y_q8 + inset_y, target.y_q8 + maximum_y / 2, target.y_q8 + maximum_y - inset_y};
...
SaccadeResult SessionEngine::enter_symbol(uint16_t symbol, uint64_t now_ns, SessionEvent* output) noexcept {
    if (!active_ || output == nullptr || symbol == 0)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    *output = {};
    ...
    if (prefix_count_ == storage_->prefix.size())
        return SACCADE_ERROR_CAPACITY;
...
SaccadeResult SessionEngine::backspace(SessionEvent* output) noexcept {
    ...
    *output = {};
    ...
    if (deferred_execution_ && selection_.view().state == interaction::SelectionState::complete) {
        ...
        if (result == SACCADE_OK)
            result = selection_.begin(scene_, interaction::SelectionMode::single, selection_context(action_context_), &storage_->selection);
...
SaccadeResult SessionEngine::execute_selection(uint64_t now_ns, SessionEvent* output) noexcept {
    ...
    const uint64_t pointer_steps = static_cast<uint64_t>(selected.target_count) + (request.move_to_final_pointer ? 1U : 0U);
    ...
    if (request.duration_ns > UINT64_MAX - pointer_duration)
        return SACCADE_ERROR_CAPACITY;
    const uint64_t execution_duration = request.duration_ns + pointer_duration;
    if (execution_duration > UINT64_MAX - execution_grace_ns - now_ns)
        return SACCADE_ERROR_CAPACITY;
...
SaccadeResult SessionEngine::tick(const SessionEpochs& current, uint64_t now_ns) noexcept {
    ...
    const interaction::SelectionContext context{current.scene_epoch, current.transform_epoch, current.topology_epoch, current.focus_id,
                                                action_context_.deadline_ns};
    SaccadeResult result = SACCADE_OK;
    if (deferred_execution_ && selection_.view().state == interaction::SelectionState::complete) {
        result = now_ns >= action_context_.deadline_ns || current.scene_epoch != action_context_.scene_epoch ||
                         current.transform_epoch != action_context_.transform_epoch ||
                         current.topology_epoch != action_context_.topology_epoch || current.focus_id != action_context_.focus_id
...
void SessionEngine::reset_session() noexcept {
    if (hints_.labels() != nullptr)
        (void)hints_.cancel();
    if (selection_.view().state != interaction::SelectionState::idle)
        (void)selection_.reset();
    scene_ = {};
    action_context_ = {};
    action_ = {};
