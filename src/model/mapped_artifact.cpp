...
SaccadeResult MappedArtifact::initialize(const char* path, ArtifactVerifier verifier) noexcept {
    ...
    if (path == nullptr || path[0] == '\0' || verifier.verify == nullptr)
        return SACCADE_ERROR_INVALID_ARGUMENT;
#if defined(_WIN32)
    std::array<wchar_t, 1024> wide_path{};
    const int converted =
        MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, path, -1, wide_path.data(), static_cast<int>(wide_path.size()));
    if (converted == 0)
        return GetLastError() == ERROR_INSUFFICIENT_BUFFER ? SACCADE_ERROR_CAPACITY : SACCADE_ERROR_INVALID_ARGUMENT;
    HANDLE file = CreateFileW(wide_path.data(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_DELETE, nullptr, OPEN_EXISTING,
                              FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file == INVALID_HANDLE_VALUE)
        return SACCADE_ERROR_NOT_FOUND;
    LARGE_INTEGER file_size{};
    if (GetFileSizeEx(file, &file_size) == 0 || file_size.QuadPart <= 0 || static_cast<uint64_t>(file_size.QuadPart) > UINT32_MAX) {
    ...
    if (fstat(file, &status) != 0 || !S_ISREG(status.st_mode) || status.st_size <= 0 ||
        static_cast<uint64_t>(status.st_size) > UINT32_MAX || static_cast<uint64_t>(status.st_size) > std::numeric_limits<size_t>::max()) {
        (void)close(file);
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    ...
    if (result == SACCADE_OK)
        result = verify_artifact(view_, verifier);
    if (result != SACCADE_OK) {
        unmap();
        return result;
    }
