...
bool packet_view_valid(const scene::PacketView& scene) noexcept {
    if (scene.header == nullptr || scene.byte_size < sizeof(SaccadeTargetPacketHeader) ||
        scene.header->struct_size != sizeof(SaccadeTargetPacketHeader) || scene.header->packet_version != SACCADE_TARGET_PACKET_VERSION ||
        scene.header->target_count > SACCADE_TARGET_PACKET_MAX_TARGETS || scene.header->target_stride != sizeof(SaccadeTargetRecord) ||
    ...
    for (uint32_t index = 0; index < scene.header->target_count; ++index) {
        ...
        if (ref.size != 0 && (ref.offset > scene.text_size || ref.size > static_cast<uint32_t>(scene.text_size - ref.offset) ||
...
SaccadeResult acquire_scoped_scene(const ServiceConfig& config, const SaccadeAgentScope& scope, const SaccadeAgentFreshness& freshness,
                                   scene::PacketView* scene, interaction::InteractionState* state) noexcept {
...
SaccadeAgentTarget target(const SaccadeTargetRecord& source, uint32_t source_index, const SaccadeAgentScope& scope,
    ...
    if (scope.kind == SACCADE_AGENT_SCOPE_WINDOW) {
        const bool actionable =
            (source.flags & (SACCADE_TARGET_ACTIONABLE | SACCADE_TARGET_DISABLED | SACCADE_TARGET_SECURE)) == SACCADE_TARGET_ACTIONABLE;
        ...
        const bool activatable =
            actionable && (source.capability_bits & (SACCADE_TARGET_CAPABILITY_BUTTON | SACCADE_TARGET_CAPABILITY_INVOKE)) != 0;
...
bool action_supported(const SaccadeAgentAction& action) noexcept {
    ...
    constexpr uint32_t click_flags =
    ...
    case SACCADE_AGENT_ACTION_INVOKE:
        return action.duration_ns == 0 && (action.flags & ~click_flags) == 0 && action.payload_size == 0 &&
               ((action.flags & SACCADE_AGENT_ACTION_ALLOW_ACTIVATION) == 0 || (action.flags & SACCADE_AGENT_ACTION_EXPLICIT_WINDOW) != 0);
    case SACCADE_AGENT_ACTION_KEY:
    case SACCADE_AGENT_ACTION_KEY_CHORD:
    case SACCADE_AGENT_ACTION_WINDOW_ACTIVATE:
    case SACCADE_AGENT_ACTION_RELEASE:
...
bool Service::pending_matches(SaccadeSpanU8 bytes, const SaccadeAgentActionBatch& batch) const noexcept {
    ...
    for (uint32_t index = 0; index < batch.action_count; ++index) {
        ...
        if (!load_record(bytes, offset, &action) || std::memcmp(&pending_verification_.actions[index], &action, sizeof(action)) != 0) {
            return false;
        }
    }
    return true;
...
SaccadeResult Service::resume_verification(const SaccadeAgentActionBatch& batch, uint64_t now_ns, SaccadeMutableSpanU8 output,
    ...
    if (pending_verification_.cancelled) {
        ...
        const SaccadeAgentFreshness freshness = scene_after(pending_verification_.after_generation, batch.deadline_ns - now_ns);
        const SaccadeResult result = acquire_scoped_scene(config_, pending_verification_.scope, freshness, &next_scene, &next_state);
...
SaccadeResult Service::observe(const SaccadeAgentObserveRequest& request, SaccadeAgentCapabilityBits client_capabilities, uint64_t now_ns,
                               SaccadeMutableSpanU8 output, size_t* output_size) noexcept {
...
SaccadeResult Service::act(SaccadeSpanU8 bytes, const SaccadeAgentActionBatch& batch, SaccadeAgentCapabilityBits client_capabilities,
    ...
    if (explicit_window && (batch.action_count != 1 || (batch.preconditions.flags & explicit_preconditions) != explicit_preconditions ||
            ...
            if (explicit_window) {
                ...
                if (source_target == nullptr) {
                    ...
                    policy.identity_current = (state.scene_flags & interaction::interaction_scene_explicit_window) != 0 &&
                                              state.process_id == preconditions.process_id && state.window_id == preconditions.window_id &&
                                              source_target->window_id == state.window_id;
                    ...
                    if (decision.backend == BackgroundActionBackend::background_ax) {
                        ...
                        if ((batch.header.flags & SACCADE_AGENT_BATCH_DRY_RUN) != 0) {
                            ...
                            const SaccadeResult execution_result =
                                config_.execute_background_press(config_.context, batch.request_id, generation(scene, state),
                                                                 scene.header->session_epoch, wire_target, false, &execution);
                        ...
                        if ((batch.header.flags & SACCADE_AGENT_BATCH_DRY_RUN) != 0) {
                            ...
                            BackgroundActionExecution execution{};
                            const SaccadeResult activation_result =
                                config_.prepare_window_activation(config_.context, batch.request_id, state.process_id, state.window_id,
                                                                  explicit_scene_epoch, batch.deadline_ns, false, &execution);
                            ...
                            if (activation_result != SACCADE_OK || execution.result != SACCADE_AGENT_OK) {
                                ...
                                result =
                                    acquire_scoped_scene(config_, active_window_scope(), scene_after(explicit_scene_epoch, remaining_ns),
                                                         &foreground_scene, &foreground_state);
                                ...
                                if (result != SACCADE_OK ||
                                    ...
                                    result = config_.read_physical_state(config_.context, &physical);
                                    if (result != SACCADE_OK ||
                                        (physical_precondition && (preconditions.expected_buttons != physical.buttons ||
                                                                   preconditions.expected_modifiers != physical.modifiers ||
                                                                   preconditions.physical_sequence != physical.physical_sequence))) {
                                        build_input_plan = false;
                                        result_is_explicit = true;
                                        action_result.result =
                                            result == SACCADE_OK ? SACCADE_AGENT_ERROR_STALE_GENERATION : agent_result(result);
                                        action_result.platform_error = result == SACCADE_OK ? SACCADE_ERROR_STALE_HANDLE : result;
