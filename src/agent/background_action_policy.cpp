...
BackgroundActionDecision choose_background_action(const BackgroundActionPolicyInput& input) noexcept {
    ...
    if (explicit_flag != input.explicit_window || (!input.explicit_window && (input.flags & SACCADE_AGENT_ACTION_ALLOW_ACTIVATION) != 0)) {
        return unsupported();
    }
    if ((input.target_flags & SACCADE_AGENT_TARGET_SECURE) != 0)
        return secure_surface();
    if ((input.target_flags & (SACCADE_AGENT_TARGET_ACTIONABLE | SACCADE_AGENT_TARGET_DISABLED)) != SACCADE_AGENT_TARGET_ACTIONABLE) {
        return inaccessible();
    }
    ...
    if (!input.explicit_window) {
        const uint32_t flags =
            (input.batch_flags & SACCADE_AGENT_BATCH_DRY_RUN) == 0 ? static_cast<uint32_t>(background_action_result_cg_event) : 0U;
