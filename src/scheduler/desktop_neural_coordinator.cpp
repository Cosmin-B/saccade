...
bool frame_valid(const DesktopNeuralFrame& frame) noexcept {
    ...
    return frame.frame != 0 && frame.source_id != 0 && frame.topology_epoch != 0 && frame.transform_epoch != 0 &&
           ...
           frame.source_count <= desktop_neural_source_capacity && frame.reserved == 0 && frame.width <= static_cast<uint32_t>(INT32_MAX) &&
           frame.height <= static_cast<uint32_t>(INT32_MAX) && frame.source_to_desktop.valid() &&
           transform.epoch == frame.transform_epoch && transform.destination_space == geometry::CoordinateSpace::desktop;
}
...
bool better(const SaccadeTargetRecord& left, const SaccadeTargetRecord& right) noexcept {
    ...
    if (left.y_q8 != right.y_q8)
        return left.y_q8 < right.y_q8;
    if (left.x_q8 != right.x_q8)
        return left.x_q8 < right.x_q8;
...
void sift_down(SaccadeTargetRecord* heap, uint32_t count, uint32_t index) noexcept {
    for (;;) {
        ...
        uint32_t worse = left;
        if (right < count && better(heap[left], heap[right]))
            worse = right;
...
SaccadeResult DesktopNeuralCoordinator::initialize(const DesktopNeuralCoordinatorConfig& config, DesktopNeuralCoordinatorStorage* storage,
    ...
    if (storage == nullptr || scenes == nullptr || config.runtime == 0 || config.session == 0 || config.model_epoch == 0 ||
        config.session_epoch == 0 || config.desktop_source_id == 0 || config.first_scene_epoch == 0 || config.maximum_output_bytes == 0 ||
...
void DesktopNeuralCoordinator::clear_frames(std::array<DesktopNeuralFrame, desktop_neural_source_capacity>* frames) noexcept {
    ...
    for (DesktopNeuralFrame& pending : pending_) {
        ...
        if (pending.frame != 0 && pending.source_id == frame.source_id) {
            if (frame.topology_epoch < pending.topology_epoch ||
                (frame.topology_epoch == pending.topology_epoch && frame.scene_transform_epoch < pending.scene_transform_epoch)) {
                release_frame(&frame);
                ++stats_.frames_stale;
        ...
        if (pending.frame == 0 && empty == nullptr)
            empty = &pending;
    }
    if (empty == nullptr)
...
SaccadeResult DesktopNeuralCoordinator::begin_batch(DesktopNeuralAdvance* advance) noexcept {
    ...
    for (const DesktopNeuralFrame& frame : pending_) {
        if (frame.frame == 0 || frame.topology_epoch != topology_epoch || frame.scene_transform_epoch != scene_transform_epoch) {
            continue;
        }
        if (expected_count == 0)
            expected_count = frame.source_count;
    ...
    std::sort(batch_.begin(), batch_.begin() + batch_count_,
              [](const DesktopNeuralFrame& left, const DesktopNeuralFrame& right) noexcept { return left.source_id < right.source_id; });
...
SaccadeResult DesktopNeuralCoordinator::append_output(size_t byte_size) noexcept {
    ...
    if (header.coordinate_space == SACCADE_COORDINATE_SPACE_DESKTOP_Q8 || header.scene_epoch != 0 || header.frame_id == 0 ||
        header.model_epoch != config_.model_epoch || header.session_epoch != config_.session_epoch ||
        header.transform_epoch != running_.transform_epoch || header.topology_epoch != running_.topology_epoch ||
        header.source_id != running_.source_id) {
        return SACCADE_ERROR_STALE_HANDLE;
    }
    ...
    for (uint32_t index = 0; index < header.target_count; ++index) {
        ...
        const SaccadeResult mapped_result =
            running_.source_to_desktop.map_rect_clipped({source.x_q8, source.y_q8, source.width_q8, source.height_q8}, &mapped);
        if (mapped_result == SACCADE_ERROR_NOT_FOUND)
            continue;
        if (mapped_result != SACCADE_OK)
            return mapped_result;
        geometry::PointQ8 safe{};
        if (running_.source_to_desktop.map_point({source.safe_x_q8, source.safe_y_q8}, &safe) != SACCADE_OK || safe.x < mapped.x ||
...
SaccadeResult DesktopNeuralCoordinator::finish_batch(DesktopNeuralAdvance* advance) noexcept {
    const bool scope_complete = batch_expected_count_ == batch_count_ && batch_completed_count_ == batch_count_ && batch_failed_count_ == 0;
    if (!scope_complete) {
    ...
    aggregate_header_.total_size = sizeof(aggregate_header_) + static_cast<uint64_t>(heap_size_) * sizeof(SaccadeTargetRecord);
    std::memcpy(aggregate_.data, &aggregate_header_, sizeof(aggregate_header_));
    const SaccadeResult committed = scenes_->commit_trusted(aggregate_, static_cast<size_t>(aggregate_header_.total_size));
    ...
    const uint64_t full_scope_latency_ns =
        batch_capture_time_ns_ != UINT64_MAX && current_time_ns_ >= batch_capture_time_ns_ ? current_time_ns_ - batch_capture_time_ns_ : 0;
    ...
    if (batch_latency_ns > config_.rates.scene_period_ns)
        ++stats_.batch_deadlines_missed;
    ...
    if (full_scope_latency_ns > config_.rates.scene_period_ns)
        ++stats_.full_scope_deadlines_missed;
...
SaccadeResult DesktopNeuralCoordinator::advance(uint64_t now_ns, DesktopNeuralAdvance* output) noexcept {
    ...
    *output = {};
    if (batch_active_)
        output->sources_expected = batch_expected_count_;
