...
constexpr uint32_t plan_flag_mask = SACCADE_INPUT_PLAN_DRY_RUN | SACCADE_INPUT_PLAN_STOP_ON_FAILURE | SACCADE_INPUT_PLAN_RESTORE_POINTER |
                                    SACCADE_INPUT_PLAN_FOLLOW_TARGETS;
...
bool payload_valid(const SaccadeInputCommand& command, const SaccadeInputPlanHeader& header, uint64_t command_bytes) noexcept {
...
bool command_valid(const SaccadeInputCommand& command, const SaccadeInputPlanHeader& header, uint64_t command_bytes,
    ...
    if (command.kind < SACCADE_INPUT_COMMAND_POINTER_MOVE || command.kind > SACCADE_INPUT_COMMAND_WAIT ||
        (command.flags & ~command_flag_mask) != 0 || command.reserved32 != 0 || !payload_valid(command, header, command_bytes)) {
        return false;
    }
    switch (command.kind) {
    case SACCADE_INPUT_COMMAND_POINTER_MOVE:
        *permissions |= SACCADE_INPUT_PERMISSION_POINTER;
        return (command.flags & SACCADE_INPUT_COMMAND_ABSOLUTE) != 0 && command.data0 == 0 && command.data1 == 0 && command.data2 == 0;
    ...
    case SACCADE_INPUT_COMMAND_CLICK:
        *permissions |= SACCADE_INPUT_PERMISSION_POINTER;
        return one_button(command.data0) && command.data1 >= 1 && command.data1 <= 16 && (command.data2 & ~modifier_mask) == 0;
    case SACCADE_INPUT_COMMAND_SCROLL:
        *permissions |= SACCADE_INPUT_PERMISSION_POINTER;
        return (command.delta_x_q8 != 0 || command.delta_y_q8 != 0) && command.data0 == 0 && command.data1 == 0 && command.data2 == 0;
    ...
    case SACCADE_INPUT_COMMAND_WINDOW_ACTIVATE:
        *permissions |= SACCADE_INPUT_PERMISSION_WINDOW;
        return command.target_id != 0 && command.data0 == 0 && command.data1 == 0 && command.data2 == 0 && command.duration_ns == 0;
...
SaccadeResult validate_plan(SaccadeSpanU8 bytes, PlanView* output) noexcept {
    ...
    *output = {};
    if (bytes.data == nullptr || bytes.size < sizeof(SaccadeInputPlanHeader) || !aligned(bytes.data, alignof(SaccadeInputPlanHeader))) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    const auto* header = reinterpret_cast<const SaccadeInputPlanHeader*>(bytes.data);
    if (header->struct_size != sizeof(*header) || header->plan_version != SACCADE_INPUT_PLAN_VERSION || header->command_count == 0 ||
        header->command_count > SACCADE_INPUT_PLAN_MAX_COMMANDS || header->command_stride != sizeof(SaccadeInputCommand) ||
        (header->flags & ~plan_flag_mask) != 0 || header->required_permissions == 0 ||
        (header->required_permissions & ~permission_mask) != 0 || (header->expected_buttons & ~button_mask) != 0 ||
        header->reserved32 != 0 || header->plan_id == 0 || header->scene_epoch == 0 || header->frame_id == 0 || header->model_epoch == 0 ||
        header->session_epoch == 0 || header->transform_epoch == 0 || header->topology_epoch == 0 || header->permission_epoch == 0 ||
        header->source_id == 0 || header->deadline_ns == 0 || header->commands_offset != sizeof(SaccadeInputPlanHeader) ||
        header->total_size > bytes.size || header->reserved != 0) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
