...
bool append_result(saccade::core::StackStringBuilder<2048>* output, const saccade::model::ArtifactView& artifact,
    ...
    return output->append("{\n  \"provider\": \"") && output->append(text(provider.info.name)) &&
           ...
           output->append(",\n  \"framework_opaque\": ") && output->append_unsigned(memory.framework_opaque) && output->append("\n}\n");
}
...
int main(int argc, char** argv) {
    ...
    if (!saccade::apps::model_trust::configured)
        return exit_code(ExitCode::trust_not_configured);
    ...
    if (result != SACCADE_OK)
        return fail(ExitCode::verifier_failed, "verifier initialization", result);

    static saccade::model::MappedArtifact artifact;
    result = artifact.initialize(argv[1], verifier.descriptor());
    if (result != SACCADE_OK)
        return fail(ExitCode::artifact_failed, "artifact admission", result);

#if defined(__APPLE__)
    static saccade::platform::macos::CoreMlInferenceProvider native_provider;
    result = native_provider.initialize(
        {argv[2], saccade::platform::macos::CoreMlComputePolicy::cpu_and_neural_engine, false, {}, verifier.descriptor()});
    if (result != SACCADE_OK)
        return fail(ExitCode::provider_failed, "Core ML provider initialization", result);
    ...
    if (result != SACCADE_OK)
        return fail(ExitCode::provider_failed, "DirectML provider initialization", result);
    ...
    config.preferred_capability_bits =
    ...
    if (result != SACCADE_OK)
        return fail(ExitCode::runtime_failed, "runtime session initialization", result);
    ...
    if (result != SACCADE_OK)
        return fail(ExitCode::memory_failed, "memory query", result);
    ...
    if (runtime.shutdown() != SACCADE_OK || native_provider.shutdown() != SACCADE_OK || artifact.shutdown() != SACCADE_OK ||
        verifier.shutdown() != SACCADE_OK) {
