...
uint64_t deadline_after(uint64_t timeout_ns) noexcept {
    ...
    return timeout_ns > std::numeric_limits<uint64_t>::max() - now_ns ? std::numeric_limits<uint64_t>::max() : now_ns + timeout_ns;
}
...
bool AgentClient::connect() noexcept {
    ...
    if (swprintf_s(endpoint.data(), endpoint.size(), L"\\\\.\\pipe\\Saccade.Agent.v1.%lu", static_cast<unsigned long>(session_id)) <= 0)
        return false;
    if (!WaitNamedPipeW(endpoint.data(), 2000))
        return false;
    pipe_ = CreateFileW(endpoint.data(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
    if (pipe_ == INVALID_HANDLE_VALUE)
        return false;
    DWORD mode = PIPE_READMODE_MESSAGE;
    if (SetNamedPipeHandleState(pipe_, &mode, nullptr, nullptr) != 0)
        return true;
    close();
    return false;
    ...
    if (root_size == 0 || root_size > root.size())
        return false;
    sockaddr_un address{};
    address.sun_family = AF_UNIX;
    const size_t prefix_size = std::strlen(root.data());
    if (prefix_size + sizeof(leaf) + 1U > sizeof(address.sun_path))
        return false;
    std::memcpy(address.sun_path, root.data(), prefix_size);
    size_t leaf_offset = prefix_size;
    if (leaf_offset != 0 && address.sun_path[leaf_offset - 1U] != '/')
        address.sun_path[leaf_offset++] = '/';
...
bool AgentClient::transact(const void* request, size_t request_size, AgentClientStorage* storage, size_t* response_size) noexcept {
    if (request == nullptr || storage == nullptr || response_size == nullptr || request_size == 0 || request_size > storage->request.size())
        return false;
    ...
    if (header.message_kind == SACCADE_AGENT_MESSAGE_OBSERVE_REQUEST && request_size >= sizeof(SaccadeAgentObserveRequest)) {
    ...
    if (header.message_kind == SACCADE_AGENT_MESSAGE_QUERY_REQUEST && request_size >= sizeof(SaccadeAgentQueryRequest)) {
    ...
    if (header.message_kind == SACCADE_AGENT_MESSAGE_ACTION_BATCH && request_size >= sizeof(SaccadeAgentActionBatch)) {
        ...
        const bool first_action_valid = batch.action_count != 0 && batch.action_stride >= sizeof(first_action) &&
                                        batch.actions_offset <= request_size && sizeof(first_action) <= request_size - batch.actions_offset;
        if (first_action_valid)
            std::memcpy(&first_action, static_cast<const uint8_t*>(request) + batch.actions_offset, sizeof(first_action));
        const bool explicit_window = first_action_valid && (first_action.flags & SACCADE_AGENT_ACTION_EXPLICIT_WINDOW) != 0;
        return transact_once(request, request_size, storage, response_size) &&
               ((batch.header.flags & SACCADE_AGENT_BATCH_VERIFY_NEXT_GENERATION) == 0 ||
                verify_action(batch, explicit_window, storage, response_size));
    }
    return transact_once(request, request_size, storage, response_size);
}

bool AgentClient::transact_once(const void* request, size_t request_size, AgentClientStorage* storage, size_t* response_size) noexcept {
#if defined(_WIN32)
    DWORD written = 0;
    if (!WriteFile(pipe_, request, static_cast<DWORD>(request_size), &written, nullptr) || written != static_cast<DWORD>(request_size))
        return false;
    DWORD read = 0;
...
bool AgentClient::transact_fresh(const void* request, size_t request_size, const SaccadeAgentFreshness& freshness,
    ...
    for (;;) {
        if (!transact_once(request, request_size, storage, response_size))
            return false;
        if (!waits)
            return true;
        ...
        if (header.message_kind == SACCADE_AGENT_MESSAGE_OBSERVE_COMPLETION && *response_size >= sizeof(SaccadeAgentObserveCompletion)) {
            SaccadeAgentObserveCompletion completion{};
            std::memcpy(&completion, storage->response.data(), sizeof(completion));
            result = completion.result;
        } else if (header.message_kind == SACCADE_AGENT_MESSAGE_QUERY_COMPLETION && *response_size >= sizeof(SaccadeAgentQueryCompletion)) {
            SaccadeAgentQueryCompletion completion{};
            std::memcpy(&completion, storage->response.data(), sizeof(completion));
            result = completion.result;
        }
        if (result != SACCADE_AGENT_ERROR_TIMEOUT || monotonic_time_ns() >= deadline_ns)
            return true;
    }
}

bool AgentClient::verify_action(const SaccadeAgentActionBatch& batch, bool explicit_window, AgentClientStorage* storage,
                                size_t* response_size) noexcept {
    if (*response_size < sizeof(SaccadeAgentActionCompletion))
        return true;
    ...
    if ((action_completion.header.flags & SACCADE_AGENT_MESSAGE_NEXT_GENERATION_AVAILABLE) != 0 ||
        ...
        action_completion.completed_action_count != batch.action_count || action_completion.failed_action_index != UINT32_MAX)
        return true;
    ...
    observe.scope.kind = explicit_window ? SACCADE_AGENT_SCOPE_WINDOW : SACCADE_AGENT_SCOPE_DESKTOP;
    observe.scope.source_mode = SACCADE_AGENT_SOURCE_FUSED;
    observe.scope.stable_id = explicit_window ? batch.preconditions.window_id : 0;
...
bool AgentClient::hello(SaccadeAgentCapabilityBits requested, AgentClientStorage* storage, SaccadeAgentCapabilityBits* granted) noexcept {
    ...
    if (!transact(&hello, sizeof(hello), storage, &response_size) || response_size != sizeof(SaccadeAgentHelloCompletion))
        return false;
