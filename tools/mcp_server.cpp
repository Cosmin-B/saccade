...
constexpr uint64_t default_action_timeout_ns = UINT64_C(2'000'000'000);
constexpr uint64_t nanoseconds_per_millisecond = UINT64_C(1'000'000);
constexpr uint64_t maximum_action_duration_ms = SACCADE_AGENT_MAX_ACTION_DURATION_NS / nanoseconds_per_millisecond;
...
int field(const JsonDocument& document, int object, std::string_view key) noexcept {
    if (object < 0 || document.tokens[object].type != JSMN_OBJECT)
        return -1;
    for (int index = object + 1; index + 1 < document.count; ++index) {
        if (document.tokens[index].parent == object && document.tokens[index].type == JSMN_STRING && token_equals(document, index, key))
            return index + 1;
    }
    return -1;
}
...
bool parse_i32(const JsonDocument& document, int token_index, int32_t* output) noexcept {
    if (token_index < 0 || document.tokens[token_index].type != JSMN_PRIMITIVE || output == nullptr)
        return false;
    const std::string_view text = token_text(document, token_index);
    if (text.empty())
        return false;
...
bool decode_string(const JsonDocument& document, int token_index, uint8_t* output, size_t capacity, size_t* output_size) noexcept {
    if (token_index < 0 || document.tokens[token_index].type != JSMN_STRING || output == nullptr || output_size == nullptr)
        return false;
    ...
    for (size_t index = 0; index < input.size(); ++index) {
        ...
        if (value != '\\') {
            if (written == capacity || value < 0x20U)
                return false;
        ...
        if (escaped == 'u') {
            ...
            if (codepoint >= 0xD800U && codepoint <= 0xDBFFU) {
                if (input.size() - index <= 6 || input[index + 1U] != '\\' || input[index + 2U] != 'u')
                    return false;
                index += 2U;
                uint32_t low = 0;
                for (uint32_t digit = 0; digit < 4; ++digit) {
            ...
            if (!append_utf8(codepoint, output, capacity, &written))
                return false;
            continue;
        }
        uint8_t decoded = 0;
...
bool write_tools(const JsonDocument& document, int id) noexcept {
    return response_prefix(document, id) &&
           write_text(",\"result\":{\"tools\":["
                      ...
                      "\"scope\":{\"enum\":[\"active-window\",\"window\",\"display\",\"desktop\",\"rect\"],"
                      "\"description\":\"Observation region: the active window, one explicit selected window, "
                      "one display, the full desktop, or an explicit rect.\"},\"scopeId\":{\"oneOf\":[{\"type\":\"string\",\"pattern\":"
                      ...
                      "\"scope\":{\"enum\":["
                      "\"active-window\",\"window\",\"display\",\"desktop\",\"rect\"]},\"scopeId\":{\"oneOf\":[{"
                      ...
                      "\"type\":\"integer\",\"description\":\"Required target-source bits.\"},"
                      ...
                      "as nextGeneration.\"},\"allowActivation\":{\"type\":\"boolean\","
                      "\"description\":\"Explicitly permit activating the exact requested window when a "
                      "background semantic action is unavailable.\"},\"explicitWindow\":{\"type\":\"boolean\","
                      "\"description\":\"Resolve the action only against the exact WINDOW scene named by "
                      "the generation, processId, and windowId preconditions.\"},\"button\":{\"type\":\"integer\"},"
                      "\"modifiers\":{\"type\":\"integer\"},\"keyUsage\":{\"type\":\"integer\","
                      "\"description\":\"HID key usage for key and key-chord.\"},"
                      "\"repeat\":{\"type\":\"integer\"},\"durationMs\":{\"type\":\"integer\",\"minimum\":0,"
                      "\"maximum\":60000,\"description\":\"Timed move, hold lease, drag, text selection, or "
                      "continuous scroll duration in milliseconds. A zero-duration hold remains pressed until "
                      "release or abort.\"},\"deltaXQ8\":{\"type\":\"integer\","
...
class Server final {
  public:
    ...
    bool process(const JsonDocument& document) noexcept {
        ...
        if (token_equals(document, method, "notifications/initialized") || token_equals(document, method, "notifications/cancelled"))
            return true;
        if (id < 0)
            return true;
        ...
        if (!initialized_)
            return write_error(document, id, -32002, "Server is not initialized");
        if (token_equals(document, method, "ping"))
            return response_prefix(document, id) && write_text(",\"result\":{}}\n");
        if (token_equals(document, method, "tools/list"))
            return write_tools(document, id);
        if (!token_equals(document, method, "tools/call"))
            return write_error(document, id, -32601, "Method not found");
        ...
        if (token_equals(document, name, "saccade_observe"))
            return observe(document, id, arguments);
        if (token_equals(document, name, "saccade_query"))
            return query(document, id, arguments);
        if (token_equals(document, name, "saccade_act"))
            return act(document, id, arguments);
  ...
  private:
    bool ensure_client() noexcept {
        ...
        constexpr SaccadeAgentCapabilityBits requested = SACCADE_AGENT_CAPABILITY_OBSERVE | SACCADE_AGENT_CAPABILITY_POINTER |
    ...
    bool transact(const void* request, size_t request_size, size_t* response_size, bool retry_safe) noexcept {
        if (client_.transact(request, request_size, &storage_->agent, response_size))
            return true;
        disconnect_client();
        if (!ensure_client() || !retry_safe)
            return false;
        if (client_.transact(request, request_size, &storage_->agent, response_size))
            return true;
        disconnect_client();
    ...
    bool write_tool_error(const JsonDocument& document, int id, std::string_view message) noexcept {
        return response_prefix(document, id) && write_text(",\"result\":{\"content\":[{\"type\":\"text\",\"text\":") &&
               write_json_string(reinterpret_cast<const uint8_t*>(message.data()), message.size()) && write_text("}],\"isError\":true}}\n");
    }

    bool write_agent_rejection(const JsonDocument& document, int id, const char* prefix, SaccadeAgentResult result) noexcept {
        saccade::core::StackStringBuilder<128> message;
        if (!message.append(prefix) || !message.append(saccade::tools::agent_result_text(result)))
            return false;
        return write_tool_error(document, id, message.view());
    }

    static bool append_action_results(saccade::core::StackStringBuilder<2048>& result, const SaccadeAgentActionCompletion& completion,
                                      const uint8_t* response, size_t response_size) noexcept {
        ...
        if (completion.action_result_count == 0 || completion.action_result_stride != sizeof(SaccadeAgentActionResult) ||
            completion.action_results_offset > response_size || bytes > response_size - completion.action_results_offset)
            return true;
        if (!result.append(",\"actions\":["))
            return false;
        for (uint32_t index = 0; index < completion.action_result_count; ++index) {
            SaccadeAgentActionResult entry{};
            std::memcpy(&entry, response + completion.action_results_offset + static_cast<size_t>(index) * completion.action_result_stride,
                        sizeof(entry));
            if (!result.append(index == 0 ? "{\"index\":" : ",{\"index\":") || !result.append_unsigned(entry.action_index) ||
                !result.append(",\"kind\":") || !result.append_unsigned(entry.kind) || !result.append(",\"result\":") ||
                !result.append_signed(entry.result) || !result.append(",\"resultText\":\"") ||
                !result.append(saccade::tools::agent_result_text(entry.result)) || !result.append("\",\"platformError\":") ||
                !result.append_signed(entry.platform_error) || !result.append(",\"targetId\":\"") ||
                !result.append_unsigned(entry.resolved_target_id) || !result.append("\",\"secondaryTargetId\":\"") ||
                !result.append_unsigned(entry.resolved_secondary_target_id) || !result.append("\",\"validatedGeneration\":\"") ||
    ...
    bool write_targets(const JsonDocument& document, int id, const uint8_t* response, size_t response_size, uint32_t targets_offset,
                       uint32_t target_stride, uint32_t target_count, const SaccadeAgentGeneration& generation,
                       const SaccadeAgentScope& scope, uint32_t message_flags) noexcept {
        ...
        if (!prefix.append_unsigned(generation.generation) || !prefix.append("\",\"epochs\":{\"scene\":\"") ||
            ...
            !prefix.append_unsigned(scope.stable_id) || !prefix.append("\",\"xQ8\":") || !prefix.append_signed(scope.rect.x_q8) ||
            !prefix.append(",\"yQ8\":") || !prefix.append_signed(scope.rect.y_q8) || !prefix.append(",\"widthQ8\":") ||
            !prefix.append_signed(scope.rect.width_q8) || !prefix.append(",\"heightQ8\":") || !prefix.append_signed(scope.rect.height_q8) ||
        ...
        for (uint32_t index = 0; index < target_count; ++index) {
            ...
            std::memcpy(&target, response + targets_offset + static_cast<size_t>(index) * target_stride, sizeof(target));
            saccade::core::StackStringBuilder<768> row;
            if (!row.append(index == 0 ? "{\"index\":" : ",{\"index\":") || !row.append_unsigned(index) || !row.append(",\"id\":\"") ||
                !row.append_unsigned(target.target_id) || !row.append("\",\"parentId\":\"") || !row.append_unsigned(target.parent_id) ||
                !row.append("\",\"windowId\":\"") || !row.append_unsigned(target.window_id) || !row.append("\",\"displayId\":\"") ||
                !row.append_unsigned(target.display_id) || !row.append("\",\"role\":\"") || !row.append(role_name(target.role)) ||
                !row.append("\",\"roleCode\":") || !row.append_unsigned(target.role) || !row.append(",\"capabilities\":") ||
                !row.append_unsigned(target.capability_bits) || !row.append(",\"flags\":") || !row.append_unsigned(target.flags) ||
                !row.append(",\"sourceBits\":") || !row.append_unsigned(target.source_bits) || !row.append(",\"confidenceQ16\":") ||
                !row.append_unsigned(target.confidence_q16) || !row.append(",\"order\":") || !row.append_unsigned(target.order) ||
                !row.append(",\"xQ8\":") || !row.append_signed(target.bounds.x_q8) || !row.append(",\"yQ8\":") ||
                !row.append_signed(target.bounds.y_q8) || !row.append(",\"widthQ8\":") || !row.append_signed(target.bounds.width_q8) ||
                !row.append(",\"heightQ8\":") || !row.append_signed(target.bounds.height_q8) || !row.append(",\"safeXQ8\":") ||
                !row.append_signed(target.safe_point.x_q8) || !row.append(",\"safeYQ8\":") || !row.append_signed(target.safe_point.y_q8))
                return false;
    ...
    bool observe(const JsonDocument& document, int id, int arguments) noexcept {
        ...
        if (!ensure_client())
            return write_tool_error(document, id, "Saccade service is unavailable");
        size_t response_size = 0;
        if (!transact(&request, sizeof(request), &response_size, true) || response_size < sizeof(SaccadeAgentObserveCompletion))
    ...
    bool query(const JsonDocument& document, int id, int arguments) noexcept {
        ...
        if (field(document, arguments, "targetId") >= 0)
        ...
        if (!query_u32(document, arguments, "role", SACCADE_AGENT_QUERY_ROLE, &filter->role, &filter->flags) ||
            !query_u32(document, arguments, "capability", SACCADE_AGENT_QUERY_CAPABILITY, &filter->required_capability_bits,
                       &filter->flags) ||
            !query_u32(document, arguments, "source", SACCADE_AGENT_QUERY_SOURCE, &filter->source_bits, &filter->flags) ||
            !query_u32(document, arguments, "minimumConfidenceQ16", SACCADE_AGENT_QUERY_CONFIDENCE, &filter->minimum_confidence_q16,
                       &filter->flags))
            return write_error(document, id, -32602, "Invalid query integer");
        ...
        if (maximum >= 0)
            request->maximum_results = static_cast<uint32_t>(value);
        size_t request_size = prefix_size;
        const int text = field(document, arguments, "text");
        if (text >= 0) {
            size_t text_size = 0;
            if (!decode_string(document, text, storage_->agent.request.data() + request_size, SACCADE_AGENT_MAX_TEXT_BYTES, &text_size) ||
                text_size == 0)
            ...
            if (match >= 0) {
                filter->text_match = token_equals(document, match, "exact")       ? SACCADE_AGENT_TEXT_EXACT
                                     ...
                                     : token_equals(document, match, "substring") ? SACCADE_AGENT_TEXT_SUBSTRING
                                                                                  : 0;
                if (filter->text_match == 0)
                    return write_error(document, id, -32602, "Invalid textMatch");
            }
        }
    ...
    bool act(const JsonDocument& document, int id, int arguments) noexcept {
        ...
        *action = {};
        ...
        const uint64_t action_begin_ns = saccade::tools::monotonic_time_ns();
        batch->deadline_ns = action_begin_ns + default_action_timeout_ns;
        ...
        if (!precondition_u64(document, arguments, "processId", SACCADE_AGENT_PRECONDITION_PROCESS, &batch->preconditions.process_id,
                              &batch->preconditions.flags) ||
            !precondition_u64(document, arguments, "windowId", SACCADE_AGENT_PRECONDITION_WINDOW, &batch->preconditions.window_id,
                              &batch->preconditions.flags) ||
            !precondition_u64(document, arguments, "displayId", SACCADE_AGENT_PRECONDITION_DISPLAY, &batch->preconditions.display_id,
                              &batch->preconditions.flags) ||
        ...
        if (!optional_u64(document, arguments, "durationMs", &duration_ms) || duration_ms > maximum_action_duration_ms)
            return write_error(document, id, -32602, "Invalid action duration");
        action->duration_ns = duration_ms * nanoseconds_per_millisecond;
        batch->deadline_ns = action_begin_ns + default_action_timeout_ns + action->duration_ns;
        ...
        const int allow_activation = field(document, arguments, "allowActivation");
        bool allow_activation_value = false;
        if (allow_activation >= 0 && !parse_bool(document, allow_activation, &allow_activation_value))
            return write_error(document, id, -32602, "Invalid allowActivation value");
        if (allow_activation_value)
            action->flags |= SACCADE_AGENT_ACTION_ALLOW_ACTIVATION;
        const int explicit_window = field(document, arguments, "explicitWindow");
        bool explicit_window_value = false;
        if (explicit_window >= 0 && !parse_bool(document, explicit_window, &explicit_window_value))
            return write_error(document, id, -32602, "Invalid explicitWindow value");
        if (explicit_window_value)
            action->flags |= SACCADE_AGENT_ACTION_EXPLICIT_WINDOW;
        ...
        if (!action_point(document, arguments, "xQ8", "yQ8", &action->point, &point_present) ||
            !action_point(document, arguments, "secondaryXQ8", "secondaryYQ8", &action->secondary_point, &secondary_point_present))
            return write_error(document, id, -32602, "Action points require coordinate pairs");
        const bool dual_target = action->kind == SACCADE_AGENT_ACTION_DRAG_DROP || action->kind == SACCADE_AGENT_ACTION_TEXT_SELECT;
        const bool point_action = action->kind == SACCADE_AGENT_ACTION_POINTER_MOVE || action->kind == SACCADE_AGENT_ACTION_POINTER_HOVER ||
        ...
        if (point_present)
            action->flags |= SACCADE_AGENT_ACTION_EXPLICIT_POINTS;
        ...
        if (backward_value)
            action->flags |= SACCADE_AGENT_ACTION_CYCLE_BACKWARD;
        const int text = field(document, arguments, "text");
        if (text >= 0) {
            size_t text_size = 0;
            if (!decode_string(document, text, storage_->agent.request.data() + batch->payload_offset, SACCADE_AGENT_MAX_TEXT_BYTES,
                               &text_size) ||
                text_size == 0)
        ...
        return result.append_signed(completion.result) && result.append(",\"resultText\":\"") &&
               result.append(saccade::tools::agent_result_text(completion.result)) && result.append("\",\"platformError\":") &&
               result.append_signed(completion.platform_error) && result.append(",\"completedActions\":") &&
               ...
               result.append_unsigned(completion.physical_state.active_lease_id) && result.append("\",\"permissionEpoch\":\"") &&
               result.append_unsigned(completion.physical_state.permission_epoch) && result.append("\",\"sequence\":\"") &&
               ...
               ((completion.header.flags & SACCADE_AGENT_MESSAGE_NEXT_GENERATION_AVAILABLE) == 0 ||
                (result.append(",\"nextGeneration\":{\"generation\":\"") && result.append_unsigned(completion.next_generation.generation) &&
                 result.append("\",\"sceneEpoch\":\"") && result.append_unsigned(completion.next_generation.scene_epoch) &&
                 result.append("\",\"frameId\":\"") && result.append_unsigned(completion.next_generation.frame_id) &&
                 result.append("\",\"captureTimeNs\":\"") && result.append_unsigned(completion.next_generation.capture_time_ns) &&
                 result.append("\",\"processId\":\"") && result.append_unsigned(completion.next_generation.process_id) &&
                 result.append("\",\"windowId\":\"") && result.append_unsigned(completion.next_generation.window_id) &&
                 result.append("\",\"displayId\":\"") && result.append_unsigned(completion.next_generation.display_id) &&
                 result.append("\"}"))) &&
               append_action_results(result, completion, storage_->agent.response.data(), response_size) &&
               result.append("},\"isError\":") && result.append(completion.result == SACCADE_AGENT_OK ? "false" : "true") &&
    ...
    bool parse_scope(const JsonDocument& document, int arguments, SaccadeAgentScope* scope) noexcept {
        ...
        if (kind >= 0) {
            scope->kind = token_equals(document, kind, "active-window") ? SACCADE_AGENT_SCOPE_ACTIVE_WINDOW
                          : token_equals(document, kind, "window")       ? SACCADE_AGENT_SCOPE_WINDOW
        ...
        if (stable_id >= 0 && !parse_u64(document, stable_id, &scope->stable_id))
            return false;
        ...
        if (any_rect_component && !complete_rect)
            return false;
        if (complete_rect && (!parse_i32(document, x, &scope->rect.x_q8) || !parse_i32(document, y, &scope->rect.y_q8) ||
                              !parse_i32(document, width, &scope->rect.width_q8) || !parse_i32(document, height, &scope->rect.height_q8)))
            return false;
        ...
        if (any_rect_component)
            return false;
        if (scope->kind == SACCADE_AGENT_SCOPE_DISPLAY)
            return scope->stable_id != 0;
        if (scope->kind == SACCADE_AGENT_SCOPE_WINDOW)
            return scope->stable_id != 0;
        return scope->kind == SACCADE_AGENT_SCOPE_ACTIVE_WINDOW || (scope->kind == SACCADE_AGENT_SCOPE_DESKTOP && scope->stable_id == 0);
    }
    ...
    bool precondition_u64(const JsonDocument& document, int arguments, std::string_view name, uint32_t flag, uint64_t* output,
                          uint32_t* flags) noexcept {
        const int token = field(document, arguments, name);
        if (token < 0)
            return true;
        if (!parse_u64(document, token, output))
            return false;
        *flags |= flag;
        return true;
    }
    ...
    bool query_u32(const JsonDocument& document, int arguments, std::string_view name, uint32_t flag, Value* output,
        ...
        if (token < 0)
            return true;
        uint64_t value = 0;
        if (!parse_u64(document, token, &value) || value > UINT32_MAX)
            return false;
    ...
    template <typename Value> bool action_u32(const JsonDocument& document, int arguments, std::string_view name, Value* output) noexcept {
        const int token = field(document, arguments, name);
        if (token < 0)
            return true;
        uint64_t value = 0;
    ...
    bool parse_action_kind(const JsonDocument& document, int kind, SaccadeAgentAction* action,
        ...
        if (kind < 0 || document.tokens[kind].type != JSMN_STRING)
            return false;
...
int main() {
    ...
    while (std::fgets(storage.input.data(), static_cast<int>(storage.input.size()), stdin) != nullptr) {
        ...
        if (storage.input[size - 1U] != '\n' && !std::feof(stdin))
            return static_cast<int>(ExitCode::input_failed);
        const size_t json_size = storage.input[size - 1U] == '\n' ? size - 1U : size;
        jsmn_parser parser{};
        jsmn_init(&parser);
        const int count =
            jsmn_parse(&parser, storage.input.data(), json_size, storage.tokens.data(), static_cast<unsigned int>(storage.tokens.size()));
        ...
        if (!written || std::fflush(stdout) != 0)
            return static_cast<int>(ExitCode::output_failed);
    }
