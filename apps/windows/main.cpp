...
bool settings_path(std::array<wchar_t, 32768>* output) noexcept {
    ...
    if (size + std::size(directory) + std::size(file) > output->size())
        return false;
...
bool load_settings(saccade::application::SettingsDocument* output) noexcept {
    ...
    if (!settings_path(&path))
        return false;
    HANDLE file = CreateFileW(path.data(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    ...
    return read != 0 && closed != 0 && size != 0 && saccade::application::decode_settings({bytes.data(), size}, output) == SACCADE_OK;
}
...
SaccadeResult save_settings(const saccade::application::SettingsDocument& settings) noexcept {
    ...
    if (path_size + std::size(suffix) > temporary.size())
        return SACCADE_ERROR_CAPACITY;
    std::memcpy(temporary.data() + path_size, suffix, sizeof(suffix));
    HANDLE file =
        CreateFileW(temporary.data(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, nullptr);
    if (file == INVALID_HANDLE_VALUE)
        return SACCADE_ERROR_BACKEND;
    DWORD written = 0;
    ...
    return MoveFileExW(temporary.data(), path.data(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH) != 0 ? SACCADE_OK
...
#if defined(SACCADE_HAS_WINDOWS_ML)
    static SaccadeResult process_agent(void*, SaccadeSpanU8, SaccadeAgentCapabilityBits, uint64_t, SaccadeMutableSpanU8, size_t*) noexcept;
#endif
...
SaccadeResult Application::dispatch(void* context, Command command, uint64_t now_ns) noexcept {
    ...
    if (command == Command::type_text) {
        if (OpenClipboard(app->window_) == 0)
        ...
        if (wide != nullptr) {
            ...
            const int required = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, wide,
                                                     wide_size <= INT_MAX ? static_cast<int>(wide_size) : 0, nullptr, 0, nullptr, nullptr);
            if (required > 0 && static_cast<size_t>(required) <= utf8.size()) {
                const int converted =
                    WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, wide, static_cast<int>(wide_size),
...
SaccadeResult Application::apply_settings(void* context, const saccade::application::SettingsDocument& settings) noexcept {
    auto* app = static_cast<Application*>(context);
#if defined(SACCADE_HAS_WINDOWS_ML)
    if (app->pipeline_initialized_) {
...
#endif
    if (replaced != SACCADE_OK || !app->settings_initialized_)
        return replaced;
...
#if defined(SACCADE_HAS_WINDOWS_ML)
    if (app->pipeline_initialized_)
        (void)app->pipeline_.observe_physical_input(now_ns);
#else
    (void)app;
    (void)now_ns;
#endif
...
#if defined(SACCADE_HAS_WINDOWS_ML)
    if (app->pipeline_initialized_)
        (void)app->pipeline_.neutralize_synthetic_input();
#else
    (void)app;
#endif
}
...
#if defined(SACCADE_HAS_WINDOWS_ML)
SaccadeResult Application::process_agent(void* context, SaccadeSpanU8 request, SaccadeAgentCapabilityBits client_capabilities,
                                         uint64_t now_ns, SaccadeMutableSpanU8 output, size_t* output_size) noexcept {
    auto* app = static_cast<Application*>(context);
    return app->pipeline_initialized_ ? app->pipeline_.process_agent(request, client_capabilities, now_ns, output, output_size)
                                      : SACCADE_ERROR_STATE;
}
#endif
...
SaccadeResult Application::show_settings() noexcept {
    ...
    settings_window_ =
        CreateWindowExW(WS_EX_TOOLWINDOW, settings_class_name, L"Saccade settings", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_VSCROLL, x,
                        y, settings_width, settings_height, window_, nullptr, instance_, this);
    if (settings_window_ == nullptr) {
        (void)settings_.cancel();
        return SACCADE_ERROR_BACKEND;
    }
...
void Application::create_settings_view() noexcept {
    ...
    auto control = [&](LPCWSTR klass, LPCWSTR text, DWORD style, int identifier, int x, int width,
        ...
        HWND item =
            CreateWindowExW(0, klass, text, WS_CHILD | WS_VISIBLE | style, x, 20 + row * settings_row_height - settings_scroll_y_, width,
                            height, settings_window_, reinterpret_cast<HMENU>(static_cast<INT_PTR>(identifier)), instance_, nullptr);
        if (item != nullptr)
            (void)SendMessageW(item, WM_SETFONT, reinterpret_cast<WPARAM>(font), TRUE);
        return item;
    };
    auto field = [&](LPCWSTR label, int identifier, LPCWSTR value) noexcept {
        ...
        (void)control(L"EDIT", value, WS_BORDER | ES_AUTOHSCROLL, identifier, settings_control_x, settings_control_width);
        ++row;
    };
    auto combo = [&](LPCWSTR label, int identifier, const wchar_t* const* items, uint32_t count, uint32_t selected) noexcept {
        (void)control(L"STATIC", label, SS_RIGHT, 0, settings_label_x, 175);
        HWND item = control(L"COMBOBOX", L"", CBS_DROPDOWNLIST | WS_VSCROLL, identifier, settings_control_x, settings_control_width, 220);
        ...
        if (item != nullptr)
            (void)SendMessageW(item, CB_SETCURSEL, selected, 0);
        ++row;
    };
    ...
    combo(L"Final pointer", settings_final_pointer, final_positions, 3, static_cast<uint32_t>(settings.pointer.final_position));
    ...
    const uint32_t modifier_bits[]{SACCADE_INPUT_MODIFIER_CONTROL, SACCADE_INPUT_MODIFIER_ALT, SACCADE_INPUT_MODIFIER_SHIFT,
    ...
    for (uint32_t index = 0; index < 4; ++index) {
        ...
        if (item != nullptr)
            (void)SendMessageW(item, BM_SETCHECK,
                               (settings.actions.click_modifiers & modifier_bits[index]) != 0 ? BST_CHECKED : BST_UNCHECKED, 0);
    }
    ++row;

    std::array<wchar_t, 128> font_family{};
    (void)MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, settings.appearance.font_family.data(), -1, font_family.data(),
                              static_cast<int>(font_family.size()));
    ...
    if (animate != nullptr)
        (void)SendMessageW(animate, BM_SETCHECK,
                           (settings.flags & saccade::application::settings_animate_overlay) != 0 ? BST_CHECKED : BST_UNCHECKED, 0);
    HWND reduced = control(L"BUTTON", L"Reduced motion", BS_AUTOCHECKBOX, settings_reduced_motion, settings_control_x + 165, 155);
    if (reduced != nullptr)
        (void)SendMessageW(reduced, BM_SETCHECK,
    ...
    constexpr const wchar_t* pages[]{L"Bindings", L"Hints", L"Detector", L"Scope", L"Pointer and actions", L"Appearance", L"Compute"};
    combo(L"Reset page", settings_reset_page, pages, 7, 1);
    (void)control(L"BUTTON", L"Reset selected page", BS_PUSHBUTTON, settings_reset_page_button, settings_control_x, 170);
    ++row;

#if defined(SACCADE_HAS_WINDOWS_ML)
    const auto stats = pipeline_.stats();
    wchar_t diagnostics[160]{};
    (void)swprintf_s(diagnostics, L"Frames %llu   Activations %llu   Overlay %llu   Failures %llu", stats.frames_offered, stats.activations,
                     stats.overlay_presents, stats.failures);
...
SaccadeResult Application::import_settings_document() noexcept {
    ...
    if (GetFileSizeEx(file, &file_size) == 0 || file_size.QuadPart <= 0 || static_cast<uint64_t>(file_size.QuadPart) > bytes.size()) {
...
SaccadeResult Application::export_settings_document() noexcept {
    ...
    if (GetSaveFileNameW(&dialog) == 0)
        return SACCADE_ERROR_CANCELLED;
    HANDLE file =
        CreateFileW(path.data(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, nullptr);
    if (file == INVALID_HANDLE_VALUE)
        return SACCADE_ERROR_BACKEND;
    DWORD written = 0;
    const BOOL saved = WriteFile(file, bytes.data(), static_cast<DWORD>(size), &written, nullptr);
...
SaccadeResult Application::show_binding_editor() noexcept {
    ...
    bindings_window_ =
        CreateWindowExW(WS_EX_TOOLWINDOW, bindings_class_name, L"Saccade keyboard bindings", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU,
                        CW_USEDEFAULT, CW_USEDEFAULT, 800, 760, settings_window_, nullptr, instance_, this);
    if (bindings_window_ == nullptr)
        return SACCADE_ERROR_BACKEND;
...
void Application::create_binding_view() noexcept {
    HFONT font = static_cast<HFONT>(GetStockObject(DEFAULT_GUI_FONT));
    auto item = [&](LPCWSTR klass, LPCWSTR text, DWORD style, int identifier, int x, int y, int width, int height) noexcept -> HWND {
        HWND control = CreateWindowExW(0, klass, text, WS_CHILD | WS_VISIBLE | style, x, y, width, height, bindings_window_,
    ...
    for (uint32_t index = 1; commands != nullptr && index <= saccade::application::binding_command_count; ++index) {
        ...
        if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, text.data(), static_cast<int>(text.size())) != 0)
            (void)SendMessageW(commands, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(text.data()));
    }
    if (commands != nullptr)
        (void)SendMessageW(commands, CB_SETCURSEL, 0, 0);
    ...
    for (const auto& key : saccade::application::binding_keys()) {
        if (keys != nullptr &&
            MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, key.name, -1, text.data(), static_cast<int>(text.size())) != 0)
            (void)SendMessageW(keys, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(text.data()));
    }
    if (keys != nullptr)
        (void)SendMessageW(keys, CB_SETCURSEL, 0, 0);
    ...
    if (saccade::application::layout_binding_keyboard(keyboard_width, keyboard_height, 4, &layout) == SACCADE_OK) {
        ...
        for (uint32_t layout_index = 0; layout_index < layout.key_count; ++layout_index) {
            ...
            if (key_index == binding_keys.size())
                continue;
            (void)MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, binding_keys[key_index].name, -1, text.data(),
                                      static_cast<int>(text.size()));
            (void)item(L"BUTTON", text.data(), BS_PUSHBUTTON, binding_visual_first + static_cast<int>(key_index), keyboard_x + rect.x,
                       keyboard_y + rect.y, rect.width, rect.height);
        }
    }
...
void Application::load_selected_binding() noexcept {
    ...
    if (binding != nullptr) {
        ...
        for (; key_index < keys.size(); ++key_index) {
            if (keys[key_index].usage == binding->physical_key)
                break;
        }
    ...
    if (session != nullptr) {
        (void)SendMessageW(session, BM_SETCHECK,
                           session_available && binding != nullptr && (binding->flags & saccade::application::hotkey_session_only) != 0
                               ? BST_CHECKED
                               : BST_UNCHECKED,
                           0);
...
void Application::refresh_binding_list() noexcept {
    ...
    for (size_t key_index = 0; key_index < keys.size(); ++key_index) {
        ...
        (void)MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, keys[key_index].name, -1, title.data(), static_cast<int>(title.size()));
        if (assigned && std::wcslen(title.data()) + 2U < title.size())
            (void)wcscat_s(title.data(), title.size(), L" *");
        (void)SetWindowTextW(GetDlgItem(bindings_window_, binding_visual_first + static_cast<int>(key_index)), title.data());
    }
    for (uint32_t index = 0; index < settings.binding_count; ++index) {
        ...
        (void)MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, saccade::application::command_name(binding.command), -1,
                                  command_text.data(), static_cast<int>(command_text.size()));
        (void)MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, key_name, -1, key_text.data(), static_cast<int>(key_text.size()));
...
SaccadeResult Application::modify_binding(bool remove) noexcept {
    ...
    if (remove) {
        ...
        if (logical_symbol == 0)
            logical_symbol = saccade::application::default_logical_symbol(physical_key);
        saccade::application::BindingConflict conflict{};
        result =
            saccade::application::set_binding(&staged, {command, physical_key, modifiers, static_cast<uint16_t>(flags), logical_symbol},
                                              &conflict);
        if (result == SACCADE_ERROR_ALREADY_EXISTS) {
            std::array<wchar_t, 128> owner{};
            (void)MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, saccade::application::command_name(conflict.command), -1, owner.data(),
...
SaccadeResult Application::stage_settings_view() noexcept {
    ...
    auto unsigned_number = [&](int identifier, uint64_t maximum, uint64_t* output) noexcept {
        ...
        if (errno == ERANGE || end == text.data() || *end != L'\0' || value > maximum)
            return false;
    ...
    auto utf8 = [&](int identifier, auto* output) noexcept {
        ...
        if (size < 0)
            return false;
        output->fill(0);
        return WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, text.data(), -1, output->data(), static_cast<int>(output->size()),
                                   nullptr, nullptr) != 0;
    };
    ...
    if (saccade::application::set_hint_alphabet(&staged.hints, symbols.data(), static_cast<uint32_t>(alphabet_count)) != SACCADE_OK) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    if (!utf8(settings_hint_language, &staged.hints.language) || !utf8(settings_font_family, &staged.appearance.font_family))
        return SACCADE_ERROR_INVALID_ARGUMENT;

    uint32_t selected = 0;
    uint64_t value = 0;
    if (!selection(settings_hint_priority, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.hints.priority = static_cast<saccade::interaction::HintPriority>(selected);
    if (!selection(settings_hint_placement, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.hints.placement = static_cast<saccade::application::HintPlacement>(selected);
    if (!selection(settings_hint_sorting, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.hints.sorting = static_cast<saccade::application::HintSorting>(selected);
    if (!selection(settings_source, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.source = static_cast<saccade::application::TargetSource>(selected);
    if (!unsigned_number(settings_confidence, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.detector.confidence_q16 = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_text_confidence, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.detector.text_sensitivity_q16 = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_duplicate_iou, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.detector.duplicate_iou_q16 = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_minimum_width, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.detector.minimum_width_q8 = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_minimum_height, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.detector.minimum_height_q8 = static_cast<uint16_t>(value);
    if (!selection(settings_merge_policy, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.detector.merge_policy = static_cast<saccade::application::MergePolicy>(selected);
    if (!unsigned_number(settings_grid_rows, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.grid.rows = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_grid_columns, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.grid.columns = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_grid_margin_x, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.grid.margin_x_q8 = static_cast<uint16_t>(value);
    if (!unsigned_number(settings_grid_margin_y, UINT16_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.grid.margin_y_q8 = static_cast<uint16_t>(value);

    if (!selection(settings_scope, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    ...
    if (!unsigned_number(settings_monitor, UINT64_MAX, &staged.monitor_stable_id))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    if (!selection(settings_final_pointer, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.pointer.final_position = static_cast<saccade::application::FinalPointerPosition>(selected);
    if (!unsigned_number(settings_pointer_movement, UINT32_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.pointer.movement_duration_ms = static_cast<uint32_t>(value);
    if (!signed_number(settings_anchor_x, &staged.pointer.anchor_x_q8) || !signed_number(settings_anchor_y, &staged.pointer.anchor_y_q8))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    if (!selection(settings_initial_mode, &selected))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.actions.initial_mode = static_cast<saccade::interaction::SelectionMode>(selected + 1U);
    if (!unsigned_number(settings_timeout, UINT32_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.actions.timeout_ms = static_cast<uint32_t>(value);
    if (!unsigned_number(settings_hold_duration, UINT32_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.actions.hold_duration_ms = static_cast<uint32_t>(value);
    if (!unsigned_number(settings_drag_duration, UINT32_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
    staged.actions.drag_duration_ms = static_cast<uint32_t>(value);
    if (!unsigned_number(settings_scroll_duration, UINT32_MAX, &value))
        return SACCADE_ERROR_INVALID_ARGUMENT;
...
uint64_t Application::timestamp_ns() const noexcept {
    ...
    if (counter_frequency_ == 0 || QueryPerformanceCounter(&counter) == 0 || counter.QuadPart <= 0)
        return 1;
    const uint64_t ticks = static_cast<uint64_t>(counter.QuadPart);
    constexpr uint64_t ns_per_second = UINT64_C(1'000'000'000);
    return (ticks / counter_frequency_) * ns_per_second + (ticks % counter_frequency_) * ns_per_second / counter_frequency_;
}

SaccadeResult Application::arm_runtime_timer(uint64_t now_ns) noexcept {
#if defined(SACCADE_HAS_WINDOWS_ML)
    if (runtime_timer_ == nullptr || now_ns == 0)
    ...
    return SetWaitableTimerEx(runtime_timer_, &due_time, 0, nullptr, nullptr, nullptr, 0) != 0 ? SACCADE_OK : SACCADE_ERROR_BACKEND;
#else
    (void)now_ns;
...
SaccadeResult Application::initialize(HINSTANCE instance, int show_command) noexcept {
    (void)show_command;
    if (initialized_ || owner_ != nullptr)
        return SACCADE_ERROR_ALREADY_EXISTS;
    if (!saccade::platform::windows::operating_system_supported())
        return SACCADE_ERROR_UNSUPPORTED;
    ...
    window_ = CreateWindowExW(WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE, window_class_name, application_name, WS_POPUP, 0, 0, 0, 0, nullptr,
    ...
    result = hotkeys_.initialize(
        {&host_, saccade::application::dispatch_desktop_command, saccade::application::observe_desktop_input, route_key, observe_command});
    if (result != SACCADE_OK)
        return result;
    ...
    if (result != SACCADE_OK) {
        ...
        if (result == SACCADE_ERROR_BACKEND)
            pipeline_recovery_.start(timestamp_ns());
    ...
    if (Shell_NotifyIconW(NIM_ADD, &tray_) == 0)
        return SACCADE_ERROR_BACKEND;
    tray_added_ = true;
    tray_.uVersion = NOTIFYICON_VERSION_4;
    (void)Shell_NotifyIconW(NIM_SETVERSION, &tray_);
#if defined(SACCADE_HAS_WINDOWS_ML)
    runtime_timer_ = CreateWaitableTimerExW(nullptr, nullptr, CREATE_WAITABLE_TIMER_HIGH_RESOLUTION, TIMER_MODIFY_STATE | SYNCHRONIZE);
    if (runtime_timer_ == nullptr)
        return SACCADE_ERROR_BACKEND;
    next_runtime_tick_ns_ = timestamp_ns();
    if (arm_runtime_timer(next_runtime_tick_ns_) != SACCADE_OK)
        return SACCADE_ERROR_BACKEND;
#endif
    initialized_ = true;
    return SACCADE_OK;
...
SaccadeResult Application::initialize_pipeline() noexcept {
    ...
    const int directory_size = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, executable.data(), -1, shader_directory_.data(),
    ...
    if (directory_size == 0 || separator + artifact_units > executable.size())
        return SACCADE_ERROR_CAPACITY;
    std::memcpy(executable.data() + separator - 1U, artifact_name, artifact_units * sizeof(wchar_t));
    const int artifact_size = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, executable.data(), -1, artifact_path_.data(),
    ...
    result = pipeline_.initialize(
        {artifact_path_.data(), shader_directory_.data(), &settings_.current(), verifier_.descriptor(), this, nullptr, timestamp_ns()});
    if (result == SACCADE_OK) {
        ...
        result = agent_pipe_.initialize({this, process_agent, neutralize, nullptr, agent_capabilities}, &agent_pipe_storage_);
        if (result == SACCADE_OK)
            agent_pipe_initialized_ = true;
    }
    if (result != SACCADE_OK) {
...
void Application::show_menu() noexcept {
    ...
    (void)TrackPopupMenu(menu, TPM_RIGHTBUTTON | TPM_BOTTOMALIGN | TPM_LEFTALIGN, point.x, point.y, 0, window_, nullptr);
...
void Application::show_diagnostics() noexcept {
    ...
    diagnostics_window_ = CreateWindowExW(WS_EX_TOOLWINDOW, diagnostics_class_name, L"Saccade Diagnostics",
                                          WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME, CW_USEDEFAULT, CW_USEDEFAULT, 900, 620,
...
void Application::create_diagnostics_view() noexcept {
    HFONT font = static_cast<HFONT>(GetStockObject(DEFAULT_GUI_FONT));
    HWND views = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, 16, 16, 852, 180, diagnostics_window_,
                                 reinterpret_cast<HMENU>(diagnostics_view), instance_, nullptr);
    constexpr const wchar_t*
        names[]{L"Overview", L"Displays", L"Runtime", L"Overlay / GPU", L"Memory", L"Trace", L"Frames / Transforms", L"Scene / Fusion"};
    ...
    HWND output =
        CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL,
                        16, 52, 852, 468, diagnostics_window_, reinterpret_cast<HMENU>(diagnostics_text), instance_, nullptr);
    ...
    for (uint32_t index = 0; index < 4; ++index) {
        HWND button = CreateWindowExW(0, L"BUTTON", button_names[index], WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    ...
    HWND faults = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, 520, 532, 160, 180, diagnostics_window_,
                                  reinterpret_cast<HMENU>(diagnostics_fault), instance_, nullptr);
    ...
    HWND arm = CreateWindowExW(0, L"BUTTON", L"Arm Fault", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 690, 532, 178, 28, diagnostics_window_,
                               reinterpret_cast<HMENU>(diagnostics_arm_fault), instance_, nullptr);
...
void Application::layout_diagnostics_view(int32_t width, int32_t height) noexcept {
    ...
    if (saccade::application::make_debugger_layout(width, height, &layout) != SACCADE_OK)
        return;

    const auto move = [this](int control, const saccade::application::DebuggerLayoutRect& rect, int32_t control_height) noexcept {
...
void Application::refresh_diagnostics() noexcept {
    ...
    if (result == SACCADE_OK) {
        ...
        const auto view =
            selected == CB_ERR ? saccade::application::DebuggerView::overview : static_cast<saccade::application::DebuggerView>(selected);
        size_t cursor = 0;
        ...
        if (debugger_operation_[0] != L'\0')
            append(L"%ls\r\n\r\n", debugger_operation_.data());
        const uint64_t inference_device = diagnostics.inference_memory.device_owned + diagnostics.inference_memory.device_imported;
        const saccade::application::DebugTraceSnapshot& trace = diagnostics.runtime.trace;
        const saccade::application::DebugTraceEvent* latest = trace.count == 0 ? nullptr : &trace.events[trace.count - 1U];
        if (view == saccade::application::DebuggerView::overview) {
            append(L"Permissions  capture %ls  accessibility %ls  input %ls\r\n"
                   ...
                   (diagnostics.permissions & saccade::platform::windows::diagnostic_capture_permission) != 0 ? L"yes" : L"no",
                   (diagnostics.permissions & saccade::platform::windows::diagnostic_accessibility_permission) != 0 ? L"yes" : L"no",
                   (diagnostics.permissions & saccade::platform::windows::diagnostic_input_permission) != 0 ? L"yes" : L"no",
                   ...
                   (diagnostics.runtime.scene_status.packet_flags & SACCADE_TARGET_PACKET_INCOMPLETE) != 0 ? L"yes" : L"no",
                   ...
                   static_cast<unsigned long long>(diagnostics.overlay.busy), static_cast<unsigned long long>(inference_device),
            ...
            for (uint32_t index = 0; index < diagnostics.display_count; ++index) {
                ...
                append(L"Display %u  id %016llx  %ux%u @ %u Hz  rotation %u  flags 0x%x\r\n"
                       ...
                       index, static_cast<unsigned long long>(display.display_id), display.backing_width, display.backing_height,
                       display.maximum_fps, static_cast<uint32_t>(display.rotation), display.flags, display.desktop_bounds.x,
                       display.desktop_bounds.y, display.desktop_bounds.width, display.desktop_bounds.height, display.work_bounds.x,
                       display.work_bounds.y, display.work_bounds.width, display.work_bounds.height);
            }
            ...
            append(L"Frames offered %llu  replaced %llu  stale %llu\r\n"
                   ...
                   static_cast<unsigned long long>(runtime.neural.sources_failed), static_cast<unsigned long long>(average_batch_ns),
                   ...
                   static_cast<unsigned long long>(runtime.scene.advances), static_cast<unsigned long long>(runtime.scene.neural_updates),
                   ...
                   static_cast<unsigned long long>(runtime.runtime.commands), static_cast<unsigned long long>(runtime.runtime.symbols),
            ...
            for (uint32_t index = 0; index < diagnostics.display_count; ++index) {
                ...
                append(L"Display %016llx  attempts %llu  rendered %llu  presented %llu  busy %llu\r\n"
                       ...
                       static_cast<unsigned long long>(display.overlay.busy_frames), display.gpu.slot_count, display.gpu.target_capacity,
                       display.gpu.instance_capacity, static_cast<unsigned long long>(display.gpu.submissions),
                       ...
                       static_cast<unsigned long long>(display.gpu.draw_calls), static_cast<unsigned long long>(display.gpu.failures));
            }
            ...
            append(L"Events %u  overwritten %llu  next %llu\r\n\r\n", trace.count, static_cast<unsigned long long>(trace.overwritten),
            ...
            for (uint32_t index = 0; index < trace.count; ++index) {
                ...
                append(L"#%llu  time %llu  code %u  result %d  flags 0x%x  argument %llu\r\n",
                       static_cast<unsigned long long>(event.sequence), static_cast<unsigned long long>(event.timestamp_ns),
                       static_cast<uint32_t>(event.code), event.result, event.flags, static_cast<unsigned long long>(event.argument));
            }
            ...
            append(L"Scene %llu  frame %llu  model %llu  session %llu\r\n"
                   ...
                   static_cast<unsigned long long>(frames.frame.byte_size), static_cast<unsigned long long>(frames.frame.timestamp_ns),
            ...
            for (uint32_t index = 0; index < frames.transform_count; ++index) {
                ...
                append(L"Transform %u  source %016llx  display %016llx  epoch %llu\r\n"
                       ...
                       index, static_cast<unsigned long long>(record.source_id), static_cast<unsigned long long>(record.display_id),
                       static_cast<unsigned long long>(transform.epoch), static_cast<uint32_t>(transform.source_space),
                       static_cast<uint32_t>(transform.destination_space), static_cast<uint32_t>(transform.rotation), transform.flags,
                       transform.source.x, transform.source.y, transform.source.width, transform.source.height, transform.destination.x,
                       transform.destination.y, transform.destination.width, transform.destination.height);
            }
        } else {
            ...
            append(L"Scene %llu  frame %llu  targets %u  samples %u  omitted %u\r\n"
                   ...
                   static_cast<unsigned long long>(scene.scene.scene_epoch), static_cast<unsigned long long>(scene.scene.frame_id),
                   targets.target_count, scene.sample_count, scene.samples_omitted, scene.fusion_input_count,
                   static_cast<unsigned long long>(fusion.candidates_read), fusion.targets_written,
                   static_cast<unsigned long long>(fusion.capacity_drops), static_cast<unsigned long long>(fusion.bucket_visits),
                   static_cast<unsigned long long>(fusion.overlap_tests), static_cast<unsigned long long>(fusion.duplicates_merged),
                   static_cast<unsigned long long>(fusion.safety_merges), targets.neural, targets.accessibility, targets.pixel,
                   targets.grid, targets.fused, targets.actionable, targets.disabled, targets.occluded, targets.secure, targets.approximate,
                   targets.text_bytes, targets.text_redacted, targets.text_truncated, targets.capability_bits);
            for (uint32_t index = 0; index < scene.sample_count; ++index) {
                ...
                append(L"#%u  id %016llx  role %u  source 0x%x  confidence %u  flags 0x%x\r\n"
                       ...
                       index, static_cast<unsigned long long>(target.target_id), target.role, target.source_bits, target.confidence_q16,
                       target.flags, target.bounds.x, target.bounds.y, target.bounds.width, target.bounds.height, target.safe_point.x,
                       target.safe_point.y, static_cast<unsigned long long>(target.window_id), target.order);
            }
        }
    } else {
        (void)swprintf_s(text.data(), text.size(),
                         L"Runtime unavailable\r\nResult %d\r\nStage %u\r\nRecovery %ls\r\nAttempt %u\r\nNext %llu ns", fault_,
                         static_cast<uint32_t>(pipeline_.last_stage()), pipeline_recovery_.pending() ? L"pending" : L"inactive",
                         pipeline_recovery_.attempt(), static_cast<unsigned long long>(pipeline_recovery_.next_attempt_ns()));
    }
#else
    (void)wcscpy_s(text.data(), text.size(), L"Runtime unavailable in this build.");
#endif
    if (diagnostics_window_ != nullptr)
        (void)SetDlgItemTextW(diagnostics_window_, diagnostics_text, text.data());
}
...
SaccadeResult Application::launch_replacement() noexcept {
    ...
    if (CreateProcessW(executable.data(), nullptr, nullptr, nullptr, FALSE, 0, nullptr, nullptr, &startup, &process) == 0) {
        return SACCADE_ERROR_BACKEND;
    }
...
void Application::shutdown() noexcept {
    ...
    if (runtime_scheduling_.initialized())
        (void)runtime_scheduling_.shutdown();
#endif
...
LRESULT CALLBACK Application::diagnostics_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) noexcept {
    ...
    if (message == WM_NCCREATE) {
        ...
        if (app != nullptr)
            app->diagnostics_window_ = window;
    }
    if (app == nullptr)
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wparam) == diagnostics_arm_fault) {
            ...
            if (selected != CB_ERR)
                result =
                    app->pipeline_.debug_arm_fault(static_cast<saccade::application::DebugFaultPoint>(selected), 1, SACCADE_ERROR_BACKEND);
#endif
            (void)swprintf_s(app->debugger_operation_.data(), app->debugger_operation_.size(), L"Arm fault %lld: result %d",
                             static_cast<long long>(selected), result);
            app->refresh_diagnostics();
            return 0;
        }
        return DefWindowProcW(window, message, wparam, lparam);
    case WM_TIMER:
        if (wparam == diagnostics_timer_identifier)
            app->refresh_diagnostics();
        return 0;
        ...
        if (LOWORD(wparam) == binding_done) {
            ...
            if (committed == SACCADE_OK) {
                ...
                if (app->settings_window_ != nullptr)
                    DestroyWindow(app->settings_window_);
            } else {
...
LRESULT CALLBACK Application::settings_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) noexcept {
    ...
    case WM_VSCROLL: {
        ...
        if (GetScrollInfo(window, SB_VERT, &scroll) == 0)
            return 0;
        ...
        if (position == app->settings_scroll_y_)
            return 0;
        const int delta = app->settings_scroll_y_ - position;
        app->settings_scroll_y_ = position;
        (void)ScrollWindowEx(window, 0, delta, nullptr, nullptr, nullptr, nullptr, SW_ERASE | SW_INVALIDATE | SW_SCROLLCHILDREN);
...
int Application::run(HINSTANCE instance, int show_command) noexcept {
    ...
    bool running = true;
    while (running) {
        const DWORD waited = MsgWaitForMultipleObjectsEx(1, &runtime_timer_, INFINITE, QS_ALLINPUT, MWMO_INPUTAVAILABLE);
    ...
    if (restart_requested_) {
        ...
        if (restarted != SACCADE_OK)
            return static_cast<int>(restarted);
    }
