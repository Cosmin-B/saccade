...
bool parse_args(int argc, char** argv, Limits* limits) noexcept {
    ...
    for (int index = 3; index < argc; ++index) {
        ...
        if (name == "--duration" && !parse_u32(value, 1, maximum_duration_seconds, &limits->duration_seconds_))
            return false;
        if (name == "--frames-per-second" && !parse_u32(value, 0, maximum_frames_per_second, &limits->frames_per_second_))
            return false;
        else if (name == "--minimum-measured-millihz" && !parse_u32(value, 0, maximum_measured_millihz, &limits->minimum_measured_millihz_))
            return false;
            ...
            return false;
        else if (name == "--source-width" && !parse_u32(value, minimum_source_extent, maximum_source_extent, &limits->source_width_))
            return false;
        else if (name == "--source-height" && !parse_u32(value, minimum_source_extent, maximum_source_extent, &limits->source_height_))
            return false;
                 ...
                 name != "--saccade-budget-mb" && name != "--monotonic-samples" && name != "--source-width" && name != "--source-height")
            return false;
    }
    return true;
}
...
bool execute_and_wait(ID3D12Device* device, ID3D12CommandQueue* queue, ID3D12GraphicsCommandList* commands) noexcept {
    ...
    if (event == nullptr)
        return false;
    const bool complete = SUCCEEDED(fence->SetEventOnCompletion(1, event)) && WaitForSingleObject(event, wait_timeout_ms) == WAIT_OBJECT_0;
    (void)CloseHandle(event);
    return complete;
}

ComPtr<ID3D12Resource> make_texture(ID3D12Device* device, ID3D12CommandQueue* queue, uint32_t width, uint32_t height) noexcept {
    ...
    if (FAILED(device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &upload_desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
    ...
    if (FAILED(device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(allocator.GetAddressOf()))) ||
        FAILED(
            device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, allocator.Get(), nullptr, IID_PPV_ARGS(commands.GetAddressOf()))))
        return {};
...
bool monotonic_growth(const SampleHistory& history, const Limits& limits) noexcept {
    if (history.count_ < limits.monotonic_samples_)
        return false;
    ...
    for (uint32_t index = first + 1U; index < history.count_; ++index) {
        ...
        video_growth += history.samples_[index].video_memory_valid_ && history.samples_[index - 1U].video_memory_valid_ &&
    ...
    const bool private_unbounded =
        private_growth == required && last_sample.private_bytes_ >= first_sample.private_bytes_ + minimum_private_monotonic_growth;
    const bool handles_unbounded = handle_growth == required && last_sample.handles_ > first_sample.handles_;
    const bool saccade_unbounded = saccade_growth == required && last_sample.saccade_bytes_ > first_sample.saccade_bytes_;
    const bool video_unbounded =
        video_growth == required && (last_sample.local_usage_ >= first_sample.local_usage_ + minimum_video_monotonic_growth ||
...
void emit_sample(uint32_t index, const MemorySample& sample) noexcept {
    char line[512]{};
    const auto append = [](char* begin, char* end, uint64_t value) noexcept { return std::to_chars(begin, end, value).ptr; };
    char* cursor = line;
    ...
    const char* labels[] = {" private=", " handles=", " local=", " local_budget=", " nonlocal=", " nonlocal_budget=", " saccade="};
    const uint64_t values[] = {sample.private_bytes_,  sample.handles_,         sample.local_usage_,  sample.local_budget_,
                               sample.nonlocal_usage_, sample.nonlocal_budget_, sample.saccade_bytes_};
...
void emit_result(uint64_t frames, uint64_t elapsed_ns, uint64_t measured_millihz, uint32_t minimum_measured_millihz,
                 uint32_t configured_frames_per_second, uint32_t source_width, uint32_t source_height, bool passed) noexcept {
    char line[320]{};
    char* cursor = line;
...
int main(int argc, char** argv) {
    ...
    if (artifact.initialize(argv[1], verifier) != SACCADE_OK)
        return fail(Result::artifact_failed, "artifact_map");
    ...
    if (graphics.initialize() != SACCADE_OK)
        return fail(Result::device_unavailable, "graphics_device");
    const ComPtr<ID3D12Resource> texture = make_texture(graphics.device(), graphics.queue(), limits.source_width_, limits.source_height_);
    const ComPtr<ID3D12Resource> candidates = make_candidate_buffer(graphics.device(), contract.candidate_capacity);
    if (texture == nullptr)
        return fail(Result::resource_failed, "synthetic_texture");
    if (candidates == nullptr)
        return fail(Result::resource_failed, "candidate_buffer");

    TensorSpec tensor_spec{};
    tensor_spec.width = artifact.view().input_width;
    tensor_spec.height = artifact.view().input_height;
    tensor_spec.format =
        contract.input_kind == saccade::model::directml::InputKind::planar_fp16 ? TensorFormat::planar_fp16 : TensorFormat::planar_int8;
    ...
    if (preprocessor.tensor_storage(&tensor) != SACCADE_OK)
        return fail(Result::pipeline_failed, "preprocessor_tensor");
    ...
    DirectMlBindingDesc input{input_name.data(),
                              ...
                              4,
                              tensor_spec.format == TensorFormat::planar_fp16 ? saccade::backend::d3d12::TensorElementType::fp16
                                                                              : saccade::backend::d3d12::TensorElementType::int8};
    DirectMlBindingDesc output{candidate_name.data(),
                               candidates.Get(),
    ...
    if (SUCCEEDED(CreateDXGIFactory1(IID_PPV_ARGS(factory.GetAddressOf())))) {
        ...
        for (UINT index = 0;; ++index) {
            ...
            if (SUCCEEDED(candidate->GetDesc1(&description)) && std::memcmp(&description.AdapterLuid, &adapter_luid, sizeof(LUID)) == 0) {
    ...
    const auto run_once = [&](uint64_t frame_id) noexcept -> const char* {
        ...
        const PostprocessEpochs epochs{frame_id,
                                       ...
                                       synthetic_source_id};
        if (preprocessor.submit(texture.Get(), limits.source_width_, limits.source_height_, {}, frame_id, synthetic_transform_epoch,
                                &preprocess_submission) != SACCADE_OK)
            return "preprocessor_submit";
        if (preprocessor.wait(&preprocess_submission, wait_timeout_ns) != SACCADE_OK)
            return "preprocessor_wait";
        if (inference.run() != SACCADE_OK)
            return "inference_run";
        if (inference.synchronize_outputs() != SACCADE_OK)
            return "inference_synchronize";
        if (postprocessor.submit(contract.candidate_capacity, limits.source_width_, limits.source_height_, config, epochs,
                                 &postprocess_submission) != SACCADE_OK)
            return "postprocessor_submit";
        if (postprocessor.wait(postprocess_submission, wait_timeout_ns) != SACCADE_OK)
            return "postprocessor_wait";
        return nullptr;
    };
    for (uint32_t index = 0; index < warmup_iterations; ++index) {
        if (const char* stage = run_once(index + 1U); stage != nullptr)
    ...
    const auto record = [&](MemorySample* sample) noexcept {
        ...
        const uint64_t owned = ImagePreprocessor::storage_size + DirectMlInference::storage_size + stats.input_bytes + stats.output_bytes +
                               stats.model_bytes + postprocessor.stats().workspace_bytes + postprocessor.stats().packet_readback_bytes;
        return process_metrics(adapter.Get(), sample, owned);
    };
    if (!record(&baseline))
        return code(Result::metric_failed);
    ...
    while (true) {
        ...
        if (now >= next_sample) {
            ...
            if (history.count_ >= maximum_samples)
                return code(Result::metric_failed);
            MemorySample sample{};
            if (!record(&sample))
                return code(Result::metric_failed);
    ...
    const uint64_t elapsed_ns = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - started).count());
    const long double measured_rate =
        elapsed_ns == 0 ? 0.0L : static_cast<long double>(frames) * 1'000'000'000'000.0L / static_cast<long double>(elapsed_ns);
    ...
    emit_result(frames, elapsed_ns, measured_millihz, limits.minimum_measured_millihz_, limits.frames_per_second_, limits.source_width_,
                limits.source_height_, cadence_passed);
    if (!cadence_passed)
        return fail(Result::qualification_failed, "measured_cadence");
