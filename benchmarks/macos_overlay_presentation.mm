...
SaccadeResult load_frame(void* context, uint64_t display_id, SaccadeOverlayFrameDesc* output) noexcept {
    if (context == nullptr || display_id == 0 || output == nullptr)
    ...
    if (storage->measuring) {
        ...
        if (storage->measured_load_count == 0)
            storage->first_load_ns = now_ns;
...
int main(int argc, char** argv) {
    ...
    @autoreleasepool {
        ...
        if (display == nullptr || display->maximum_fps < 120)
            return exit_code(ExitCode::setup);
        ...
        if (surface.initialize(*display, argv[1], saccade::backend::metal::PathPreference::automatic, callbacks) != SACCADE_OK ||
        ...
        if (surface.read_stats(&baseline_stats) != SACCADE_OK || surface.read_renderer_stats(&baseline_renderer) != SACCADE_OK ||
            baseline_stats.display_ticks == 0) {
            return exit_code(ExitCode::setup);
        }
        ...
        if (surface.stop() != SACCADE_OK || stopped_ns <= started_ns)
            return exit_code(ExitCode::setup);
        ...
        if (surface.read_info(&info) != SACCADE_OK || surface.read_stats(&stats) != SACCADE_OK ||
            ...
            stats.display_ticks <= baseline_stats.display_ticks || stats.rendered_frames < baseline_stats.rendered_frames ||
            renderer.presented_frames < baseline_renderer.presented_frames || stats.deadline_misses < baseline_stats.deadline_misses ||
            stats.busy_frames < baseline_stats.busy_frames || renderer.active_dispatches < baseline_renderer.active_dispatches ||
            packet.measured_load_count < 2 || packet.last_load_ns <= packet.first_load_ns) {
            return exit_code(ExitCode::setup);
        }
        ...
        const uint64_t refresh_millihz = (packet.measured_load_count - 1U) * UINT64_C(1000000000000) / cadence_duration_ns;
        ...
        const uint64_t minimum_refresh_millihz = static_cast<uint64_t>(info.preferred_fps) * minimum_refresh_ratio_per_thousand;
        const bool qualified = info.preferred_fps == 120 && refresh_millihz >= minimum_refresh_millihz &&
                               deadline_miss_ppm <= maximum_deadline_miss_ppm && busy_ppm <= maximum_busy_ppm && stats.failures == 0 &&
                               stats.maximum_callback_ns <= callback_budget_ns && rendered_frames == presented_frames &&
                               renderer.target_capacity == target_count && active_dispatches != 0 &&
                               packet.measured_load_count == display_ticks;

        StackStringBuilder<1024> output;
        const bool written =
            output.append("macos_overlay_presentation targets=") && output.append_unsigned(target_count) &&
            output.append(" transparent=1 path=") && output.append(path_name(renderer.path)) && output.append(" duration_ns=") &&
            output.append_unsigned(duration_ns) && output.append(" preferred_fps=") && output.append_unsigned(info.preferred_fps) &&
            output.append(" warmup_ticks=") && output.append_unsigned(baseline_stats.display_ticks) && output.append(" display_ticks=") &&
            output.append_unsigned(display_ticks) && output.append(" rendered_frames=") && output.append_unsigned(rendered_frames) &&
            output.append(" presented_frames=") && output.append_unsigned(presented_frames) && output.append(" wall_refresh_millihz=") &&
            ...
            output.append_unsigned(cadence_duration_ns) && output.append(" deadline_misses=") && output.append_unsigned(deadline_misses) &&
            output.append(" deadline_miss_ppm=") && output.append_unsigned(deadline_miss_ppm) && output.append(" busy_frames=") &&
            output.append_unsigned(busy_frames) && output.append(" busy_ppm=") && output.append_unsigned(busy_ppm) &&
            output.append(" failures=") && output.append_unsigned(stats.failures) && output.append(" maximum_callback_ns=") &&
            output.append_unsigned(stats.maximum_callback_ns) && output.append(" static_dispatches=") &&
            output.append_unsigned(renderer.static_dispatches) && output.append(" active_dispatches=") &&
            output.append_unsigned(active_dispatches) && output.append(" drawable_bytes_estimate=") &&
            output.append_unsigned(memory.drawable_bytes_estimate) && output.append(" total_known_and_estimated=") &&
            output.append_unsigned(memory.total_known_and_estimated) && output.append(" qualified=") &&
