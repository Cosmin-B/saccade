...
void emit_win32_failure(const char* operation, DWORD error) noexcept {
    ...
    if (!text.append("win32_failure operation=") || !text.append(operation) || !text.append(" error=") || !text.append_unsigned(error) ||
        !text.append('\n'))
        return;
    DWORD written = 0;
...
bool parse_session(const wchar_t* text, DWORD* output) noexcept {
    if (text == nullptr || output == nullptr || *text == L'\0')
        return false;
    DWORD value = 0;
    while (*text != L'\0') {
...
DWORD find_desktop_process(DWORD session_id) noexcept {
    ...
    if (Process32FirstW(snapshot, &entry) != FALSE) {
        do {
            ...
            if (_wcsicmp(entry.szExeFile, L"explorer.exe") == 0 && ProcessIdToSessionId(entry.th32ProcessID, &process_session) != FALSE &&
...
bool write_completion(const wchar_t* directory, DWORD exit_code) noexcept {
    ...
    if (!make_path(directory, L"complete.txt", &path))
        return false;
    const HANDLE file = CreateFileW(path.data(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file == INVALID_HANDLE_VALUE)
        return false;
    StackStringBuilder<32> text;
    DWORD written = 0;
    const bool ready = text.append_unsigned(exit_code) && text.append('\n');
    const bool saved = ready && WriteFile(file, text.view().data(), static_cast<DWORD>(text.size()), &written, nullptr) != FALSE &&
...
int wmain(int argc, wchar_t** argv) {
    ...
    DWORD session_id = 0;
    if (!parse_session(argv[1], &session_id))
        return result(ToolResult::usage);
    ...
    if (process == nullptr)
        return result(ToolResult::process_open_failed);
    ...
    if (DuplicateTokenEx(source_token, MAXIMUM_ALLOWED, nullptr, SecurityImpersonation, TokenPrimary, &primary_token) == FALSE) {
    ...
    bool command_ready = command_line.append_quoted(argv[2]) && command_line.append(L' ') && command_line.append_quoted(argv[3]);
    ...
    const HANDLE log = CreateFileW(log_path.data(), GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, &security, CREATE_ALWAYS,
    ...
    const BOOL launched = CreateProcessWithTokenW(primary_token, LOGON_WITH_PROFILE, argv[2], command_line.data(),
    ...
    CloseHandle(log);
    if (!exit_ready)
        return result(ToolResult::wait_failed);
