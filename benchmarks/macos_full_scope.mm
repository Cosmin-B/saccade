...
uint64_t next_capture_deadline(uint64_t now_ns, uint64_t start_time_ns) noexcept {
    ...
    return now_ns < phase
               ? phase
               : phase + ((now_ns - phase) / saccade::scheduler::scene_period_30hz_ns + 1U) * saccade::scheduler::scene_period_30hz_ns;
}
...
template <size_t Capacity> bool append_metric(StackStringBuilder<Capacity>* text, std::string_view name, uint64_t value) noexcept {
    return text->append(name) && text->append('=') && text->append_unsigned(value) && text->append(' ');
}

SaccadeResult begin_frames(const DisplaySnapshot& displays, SceneCaptureSet* captures, const saccade::geometry::RectQ8& scope,
                           BridgeSlot* bridge, Samples* samples, uint64_t* started, bool* retry) noexcept {
    *retry = false;
    ...
    if (oldest_capture_ns != UINT64_MAX && samples->capture_count < maximum_samples) {
        samples->capture_age[samples->capture_count] = started_ns >= oldest_capture_ns ? started_ns - oldest_capture_ns : 0;
    ...
    const SaccadeResult result =
        bridge->bridge.begin_scope(captures, capture_frames.data(), capture_displays.data(), acquired_count, scope, desktop_source_id);
    if (result != SACCADE_OK) {
        for (uint32_t index = 0; index < acquired_count; ++index) {
            (void)captures->release(capture_frames[index]);
        }
...
SaccadeResult poll_frames(BridgeSlot* bridge, DesktopNeuralCoordinator* coordinator, SaccadeRuntimeHandle runtime, Samples* samples,
                          uint64_t* offered) noexcept {
    if (!bridge->initialized || !bridge->bridge.preprocessing())
        return SACCADE_OK;
    saccade::scheduler::NeuralFrame neural{};
    bool ready = false;
    SaccadeResult result = bridge->bridge.poll(&neural, &ready);
    if (result != SACCADE_OK || !ready)
        return result;
...
bool desktop_scope(const DisplaySnapshot& displays, saccade::geometry::RectQ8* output) noexcept {
    if (displays.count == 0 || output == nullptr)
        return false;
    ...
    if (left < INT32_MIN || top < INT32_MIN || right > INT32_MAX || bottom > INT32_MAX || left >= right || top >= bottom) {
        return false;
    }
...
uint32_t captured_display_count(const DisplaySnapshot& displays) noexcept {
    ...
    for (uint32_t index = 0; index < displays.count; ++index) {
        ...
        if ((flags & saccade::geometry::display_surface_mirrored) == 0 || (flags & saccade::geometry::display_surface_main) != 0) {
            ++count;
        }
    }
    return count;
...
void record(const DesktopNeuralAdvance& advance, Samples* samples) noexcept {
    if (!advance.scene_published)
        return;
    if (samples->scenes_observed++ == 0)
        samples->cold_full_scope_ns = advance.full_scope_latency_ns;
    if (samples->scenes_observed <= warmup_scenes || samples->count >= maximum_samples)
        return;
...
uint64_t deadline_misses(const std::array<uint64_t, maximum_samples>& samples, uint32_t count) noexcept {
    ...
    for (uint32_t index = 0; index < count; ++index) {
        if (samples[index] > saccade::scheduler::scene_period_30hz_ns)
            ++misses;
    }
    return misses;
}
...
SaccadeResult sample_memory(InferenceRuntime* inference, SceneCaptureSet* captures, CoreMlImageBridge* bridge, uint64_t elapsed_ns,
                            uint64_t published_scenes, MemorySamples* samples) noexcept {
    if (samples->count == samples->values.size())
    ...
    SaccadeResult result = saccade_inference_memory_stats(inference->runtime(), inference->session(), &inference_memory);
    if (result == SACCADE_OK)
        result = captures->read_memory_stats(&capture_memory);
    if (result == SACCADE_OK)
        result = bridge->read_memory_stats(&preprocess_memory);
    if (result != SACCADE_OK)
        return result;
    ...
    sample.tracked_current = tracked_current(inference_memory) + tracked_current(capture_memory) + tracked_current(preprocess_memory);
    sample.tracked_high_water = inference_memory.high_water_bytes + capture_memory.high_water_bytes + preprocess_memory.high_water_bytes;
...
MemoryTrend analyze_memory(const MemorySamples& samples, uint64_t maximum_output_bytes) noexcept {
    ...
    if (baseline_index == samples.count || baseline_index + 1U >= samples.count)
        return trend;
    ...
    for (uint32_t index = baseline_index; index < samples.count; ++index) {
        trend.tracked_high_water_growth =
            std::max(trend.tracked_high_water_growth, growth(samples.values[index].tracked_high_water, trend.baseline.tracked_high_water));
        ...
        trend.resident_peak_growth = std::max(trend.resident_peak_growth, growth(samples.values[index].resident, trend.baseline.resident));
    }
    ...
    trend.qualified = trend.tracked_high_water_growth <= maximum_tracked_high_water_growth &&
                      trend.copied_byte_growth <= trend.copied_byte_limit && trend.resident_final_growth <= maximum_resident_final_growth &&
...
int main(int argc, char** argv) {
    @autoreleasepool {
        ...
        if (argc == 6 && !parse_mode(argv[5], &mode))
            return exit_code(ExitCode::usage);
        if (mode != RunMode::live && duration_seconds < minimum_soak_duration_seconds) {
            return exit_code(ExitCode::usage);
        }
        if (!CGPreflightScreenCaptureAccess())
            return exit_code(ExitCode::permission);

        id<MTLDevice> device = MTLCreateSystemDefaultDevice();
        if (device == nil)
            return exit_code(ExitCode::device);
        ...
        if (result != SACCADE_OK)
            return fail(ExitCode::capture, "capture", result);

        saccade::geometry::RectQ8 scope{};
        if (!desktop_scope(displays.snapshot(), &scope))
            return exit_code(ExitCode::topology);
        ...
        while (run_result == SACCADE_OK) {
            ...
            if (run_result == SACCADE_OK && now_ns >= next_capture_ns) {
                ...
                if (mode == RunMode::stable_cached && bridge.bridge.atlas_matches(scope, displays.snapshot().epoch)) {
                    ...
                    run_result = begin_frames(displays.snapshot(), &captures, scope, &bridge, &samples, &frames_started, &retry);
                }
                ++capture_cycles;
                next_capture_ns = retry ? now_ns + stale_capture_retry_ns : next_capture_deadline(now_ns, coordinator_config.start_time_ns);
            }
        ...
        if (sample_memory(&inference, &captures, &bridge.bridge, run_ended - run_started, coordinator.stats().batches_published,
                          &memory_samples) != SACCADE_OK) {
        ...
        if (bridge.bridge.read_memory_stats(&memory) != SACCADE_OK)
            return exit_code(ExitCode::memory);
        ...
        const uint64_t refresh_millihz = coordinator_stats.batches_published * UINT64_C(1'000'000'000'000) / measured_ns;
        const uint64_t measured_deadline_misses = std::max(batch_misses, full_scope_misses);
        const uint64_t deadline_miss_ppm = samples.count == 0 ? UINT64_MAX : measured_deadline_misses * UINT64_C(1'000'000) / samples.count;
        ...
        const bool stable_admitted = mode != RunMode::stable_cached || (frames_started != 0 && frames_started == expected_stable_frames);
        const bool continuous_admitted = mode != RunMode::continuous_change || frames_started >= coordinator_stats.batches_published;
        ...
        const bool written =
            ...
            append_metric(&output, "capture_cycles", capture_cycles) && append_metric(&output, "frames_started", frames_started) &&
            append_metric(&output, "frames_offered", frames_offered) &&
            append_metric(&output, "scenes", coordinator_stats.batches_published) &&
            append_metric(&output, "warmup_scenes", warmup_scenes) && append_metric(&output, "measured_scenes", samples.count) &&
            append_metric(&output, "refresh_millihz", refresh_millihz) && append_metric(&output, "batch_p50_ns", batch_p50) &&
            append_metric(&output, "batch_p95_ns", batch_p95) && append_metric(&output, "batch_p99_ns", batch_p99) &&
            append_metric(&output, "full_scope_p50_ns", full_p50) && append_metric(&output, "full_scope_p95_ns", full_p95) &&
            append_metric(&output, "full_scope_p99_ns", full_p99) && append_metric(&output, "capture_age_p50_ns", capture_age_p50) &&
            ...
            append_metric(&output, "preprocess_p50_ns", preprocess_p50) && append_metric(&output, "preprocess_p95_ns", preprocess_p95) &&
            append_metric(&output, "preprocess_p99_ns", preprocess_p99) &&
            append_metric(&output, "cold_full_scope_ns", samples.cold_full_scope_ns) &&
            append_metric(&output, "batch_misses", batch_misses) && append_metric(&output, "full_scope_misses", full_scope_misses) &&
            ...
            append_metric(&output, "resident_bytes", resident_bytes()) && append_metric(&output, "memory_samples", memory_samples.count) &&
        ...
        if (bridge_stopped != SACCADE_OK && cleanup == SACCADE_OK)
            cleanup = bridge_stopped;
