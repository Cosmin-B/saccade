...
bool parse_runs(const char* text, uint32_t* output) noexcept {
    ...
    if (parsed.ec != std::errc{} || parsed.ptr != end || value == 0 || value > maximum_runs)
        return false;
    *output = value;
    return true;
}
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && !parse_runs(argv[1], &runs))
        return exit_code(ExitCode::usage);
    ...
    for (uint32_t index = 0; index < runs; ++index) {
        ...
        if (completed_ns < started_ns || snapshot.epoch != static_cast<uint64_t>(index) + 1U ||
            snapshot.disposition == SurfaceDisposition::unknown || (qualified != (snapshot.disposition == SurfaceDisposition::qualified))) {
    ...
    const bool written = output.append("macos_surface_qualifier runs=") && output.append_unsigned(runs) && output.append(" qualified=") &&
                         output.append_unsigned(qualified_count) && output.append(" blocked=") && output.append_unsigned(blocked_count) &&
                         output.append(" reason_bits=") && output.append_unsigned(combined_reasons) && output.append(" p50_ns=") &&
                         output.append_unsigned(p50_ns) && output.append(" p95_ns=") && output.append_unsigned(p95_ns) &&
                         output.append(" p99_ns=") && output.append_unsigned(p99_ns) && output.append(" accepted=") &&
