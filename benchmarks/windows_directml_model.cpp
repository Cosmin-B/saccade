...
enum class ExitCode : int { success, usage, file_open, file_size, file_mapping, device, resource, upload, session, warmup, run, timing };
...
bool parse_count(const wchar_t* text, uint32_t* output) noexcept {
    ...
    if (end == text || *end != L'\0' || value == 0 || value > maximum_runs)
        return false;
...
ComPtr<ID3D12Resource> create_buffer(ID3D12Device* device, size_t bytes, D3D12_HEAP_TYPE heap, D3D12_RESOURCE_STATES state,
...
bool execute(ID3D12Device* device, ID3D12CommandQueue* queue, ID3D12GraphicsCommandList* commands) noexcept {
    ...
    const bool completed = SUCCEEDED(fence->SetEventOnCompletion(1, event)) && WaitForSingleObject(event, 10'000) == WAIT_OBJECT_0;
...
bool clear_input(ID3D12Device* device, ID3D12CommandQueue* queue, ID3D12Resource* input) noexcept {
    const ComPtr<ID3D12Resource> upload =
        create_buffer(device, input_bytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE);
    if (upload == nullptr)
        return false;
...
int wmain(int argc, wchar_t** argv) {
    ...
    if (opened != ExitCode::success)
        return exit_code(opened);

    GraphicsDevice graphics;
    if (graphics.initialize(device_preference) != SACCADE_OK)
        return exit_code(ExitCode::device);

    const ComPtr<ID3D12Resource> input = create_buffer(graphics.device(), input_bytes, D3D12_HEAP_TYPE_DEFAULT,
                                                       D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    const ComPtr<ID3D12Resource> output = create_buffer(graphics.device(), output_bytes, D3D12_HEAP_TYPE_DEFAULT,
                                                        D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    if (input == nullptr || output == nullptr)
        return exit_code(ExitCode::resource);
    if (!clear_input(graphics.device(), graphics.queue(), input.Get()))
        return exit_code(ExitCode::upload);
    ...
    for (uint32_t index = 0; index < measured_runs; ++index) {
        LARGE_INTEGER start{};
        LARGE_INTEGER end{};
        if (QueryPerformanceCounter(&start) == 0 || inference.run() != SACCADE_OK || inference.synchronize_outputs() != SACCADE_OK ||
    ...
    std::printf("{\n"
                ...
                milliseconds(session_start, session_end, frequency), warmup_runs, measured_runs, samples[0], median, p95, p99,
                samples[measured_runs - 1U], static_cast<unsigned long long>(memory_before), static_cast<unsigned long long>(memory_after),
                static_cast<unsigned long long>(stats.runs), static_cast<unsigned long long>(stats.failures));
