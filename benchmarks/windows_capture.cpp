...
bool write_all(HANDLE file, const void* bytes, size_t size) noexcept {
    ...
    while (size != 0) {
        ...
        DWORD written = 0;
        if (WriteFile(file, source, chunk, &written, nullptr) == FALSE || written == 0)
            return false;
        source += written;
        size -= written;
    }
...
bool make_output_path(const char* directory, uint32_t capture_index, uint32_t width, uint32_t height,
    ...
    return output->append(root) && (!separator || output->append('\\')) && output->append("capture-") &&
           output->append_unsigned(capture_index) && output->append('-') && output->append_unsigned(width) && output->append('x') &&
           output->append_unsigned(height) && output->append(".pam");
}

bool write_pam(const char* path, const uint8_t* base, size_t row_pitch, uint32_t width, uint32_t height, bool force_opaque) noexcept {
    ...
    const bool header_ready = header.append("P7\nWIDTH ") && header.append_unsigned(width) && header.append("\nHEIGHT ") &&
                              header.append_unsigned(height) && header.append("\nDEPTH 4\nMAXVAL 255\nTUPLTYPE RGB_ALPHA\nENDHDR\n");
    ...
    for (uint32_t y = 0; written && y < height; ++y) {
        ...
        while (written && x < width) {
            const uint32_t count = (width - x) < conversion_pixels ? width - x : static_cast<uint32_t>(conversion_pixels);
...
bool staging_texture(ID3D11Texture2D* source, ComPtr<ID3D11Texture2D>* staging, ComPtr<ID3D11DeviceContext>* context) noexcept {
    if (source == nullptr || staging == nullptr || context == nullptr)
        return false;
...
void emit_capture(const char* path, const SaccadeCapturedFrame& frame) noexcept {
    ...
    if (!text.append("captured path=") || !text.append(path) || !text.append(" width=") || !text.append_unsigned(frame.width) ||
        !text.append(" height=") || !text.append_unsigned(frame.height) || !text.append(" frame_id=") ||
        !text.append_unsigned(frame.frame_id) || !text.append(" timestamp_ns=") || !text.append_unsigned(frame.timestamp_ns) ||
        !text.append('\n'))
        return;
    DWORD written = 0;
    (void)WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), text.view().data(), static_cast<DWORD>(text.view().size()), &written, nullptr);
}
...
void emit_environment() noexcept {
    ...
    if (text.append("process_id=") && text.append_unsigned(GetCurrentProcessId()) && text.append(" session_id=") &&
        text.append_unsigned(session) && text.append(" active_console_session=") && text.append_unsigned(WTSGetActiveConsoleSessionId()) &&
        text.append('\n')) {
...
void emit_provider_failure(std::string_view operation, SaccadeResult operation_result,
    ...
    const auto converted =
        std::to_chars(hexadecimal.data(), hexadecimal.data() + hexadecimal.size(), static_cast<uint32_t>(native_error), 16);
    StackStringBuilder<512> text;
    if (converted.ec == std::errc{} && text.append("failure operation=") && text.append(operation) && text.append(" result=") &&
        text.append_signed(operation_result) && text.append(" hresult=0x") &&
        text.append(std::string_view(hexadecimal.data(), static_cast<size_t>(converted.ptr - hexadecimal.data()))) && text.append('\n')) {
...
ToolResult capture_gdi(const char* directory, uint32_t capture_count, uint32_t interval_ms, const SaccadeRectI32& bounds) noexcept {
    if (bounds.width <= 0 || bounds.height <= 0)
        return ToolResult::source_failed;
    ...
    void* pixels = nullptr;
    HBITMAP bitmap = memory != nullptr ? CreateDIBSection(desktop, &info, DIB_RGB_COLORS, &pixels, nullptr, 0) : nullptr;
    ...
    for (uint32_t capture_index = 1; capture_index <= capture_count; ++capture_index) {
        if (BitBlt(memory, 0, 0, bounds.width, bounds.height, desktop, bounds.x, bounds.y, SRCCOPY | CAPTUREBLT) == FALSE) {
            emit_win32_failure("bitblt");
            capture_result = ToolResult::output_failed;
            break;
        }
        if (!make_output_path(directory, capture_index, static_cast<uint32_t>(bounds.width), static_cast<uint32_t>(bounds.height), &path)) {
            emit_win32_failure("output_path");
            capture_result = ToolResult::output_failed;
            break;
        }
        if (!write_pam(path.c_str(), static_cast<const uint8_t*>(pixels), static_cast<size_t>(bounds.width) * pixel_bytes,
                       static_cast<uint32_t>(bounds.width), static_cast<uint32_t>(bounds.height), true)) {
        ...
        if (capture_index != capture_count && interval_ms != 0)
            Sleep(interval_ms);
    }
...
int main(int argc, char** argv) {
    ...
    if ((argc >= 3 && !parse_u32(argv[2], 1, maximum_capture_count, &capture_count)) ||
        (argc >= 4 && !parse_u32(argv[3], 0, 63, &source_index)) || (argc >= 5 && !parse_u32(argv[4], 0, 60'000, &interval_ms))) {
        return result(ToolResult::usage);
    }
    ...
    if (previous == nullptr)
        return result(ToolResult::dpi_awareness_failed);
    emit_environment();
    ...
    for (uint32_t capture_index = 1; capture_index <= capture_count; ++capture_index) {
        ...
        if (acquired != SACCADE_OK)
            return result(ToolResult::acquire_failed);
