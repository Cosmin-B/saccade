...
enum class ExitCode : int { success, usage, runtime, environment, options, session, memory, tensor, warmup, run, timing };
...
HMODULE load_adjacent_runtime() noexcept {
    ...
    if (separator == 0 || separator + std::size(name) > path.size())
        return nullptr;
...
bool parse_count(const wchar_t* text, uint32_t maximum, uint32_t* output) noexcept {
    ...
    if (end == text || *end != L'\0' || value == 0 || value > maximum)
        return false;
...
uint64_t private_bytes() noexcept {
    ...
    return GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(&counters), sizeof(counters)) != 0
...
struct OrtState {
    ...
    ~OrtState() {
        if (api_ != nullptr) {
            if (output_ != nullptr)
                api_->ReleaseValue(output_);
            if (input_ != nullptr)
                api_->ReleaseValue(input_);
            if (memory_ != nullptr)
...
int wmain(int argc, wchar_t** argv) {
    ...
    if ((argc >= 3 && !parse_count(argv[2], maximum_runs, &measured_runs)) ||
        ...
        (argc >= 5 && !parse_count(argv[4], maximum_threads, &thread_count)) || (argc == 6 && !parse_provider(argv[5], &provider))) {
        return exit_code(ExitCode::usage);
    }
    ...
    if (ort.runtime_ == nullptr)
        return exit_code(ExitCode::runtime);
    ...
    if (!ort.succeeded(ort.api_->CreateSessionOptions(&ort.options_)) ||
        ...
        !ort.succeeded(ort.api_->SetIntraOpNumThreads(ort.options_, provider == Provider::cpu ? static_cast<int>(thread_count) : 1)) ||
    ...
    const auto converted = std::to_chars(thread_count_text.data(), thread_count_text.data() + thread_count_text.size(), thread_count);
    ...
    *converted.ptr = '\0';
    if (provider != Provider::cpu) {
        ...
        if (!ort.succeeded(
                ort.api_->SessionOptionsAppendExecutionProvider(ort.options_, "XNNPACK", keys.data(), values.data(), keys.size()))) {
            return exit_code(ExitCode::options);
        }
        if (provider == Provider::xnnpack_strict &&
            !ort.succeeded(ort.api_->AddSessionConfigEntry(ort.options_, kOrtSessionOptionsDisableCPUEPFallback, "1"))) {
            return exit_code(ExitCode::options);
        }
    }
    ...
    if (!ort.succeeded(ort.api_->CreateTensorWithDataAsOrtValue(ort.memory_, tensors.input_.data(), sizeof(tensors.input_),
                                                                input_shape.data(), input_shape.size(), ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT,
                                                                &ort.input_)) ||
        !ort.succeeded(ort.api_->CreateTensorWithDataAsOrtValue(ort.memory_, tensors.output_.data(), sizeof(tensors.output_),
                                                                output_shape.data(), output_shape.size(),
                                                                ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &ort.output_))) {
        return exit_code(ExitCode::tensor);
    }
    if (QueryPerformanceCounter(&session_end) == 0)
        return exit_code(ExitCode::timing);
    ...
    const auto run = [&]() noexcept {
        return ort.succeeded(ort.api_->Run(ort.session_, nullptr, input_names.data(), inputs.data(), inputs.size(), output_names.data(),
    ...
    std::printf("{\n"
                ...
                "}\n",
                provider == Provider::cpu ? "ort-cpu" : (provider == Provider::xnnpack ? "ort-xnnpack-hybrid" : "ort-xnnpack-strict"),
                ort.api_base_->GetVersionString(), thread_count, milliseconds(session_start, session_end, frequency), warmup_runs,
                measured_runs, samples[0], percentile(samples, measured_runs, 50, 100), percentile(samples, measured_runs, 95, 100),
                percentile(samples, measured_runs, 99, 100), samples[measured_runs - 1U], static_cast<unsigned long long>(memory_before),
