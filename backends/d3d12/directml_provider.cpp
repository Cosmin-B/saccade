...
constexpr uint32_t base_capability_bits =
...
bool copy_name(SaccadeSpanU8 source, std::array<char, maximum_name_bytes + 1U>* output) noexcept {
    if (source.data == nullptr || source.size == 0 || source.size > maximum_name_bytes)
        return false;
...
bool frame_valid(const SaccadeInferenceDispatchDesc& desc) noexcept {
    return desc.frame.storage == SACCADE_FRAME_STORAGE_WIN32_CAPTURE && desc.frame.native_id != 0 &&
           ...
           (desc.frame.ready_fence == 0) == (desc.frame.ready_value == 0) && desc.frame.frame_id != 0 && desc.scope.x == 0 &&
           desc.scope.y == 0 && desc.scope.width == static_cast<int32_t>(desc.frame.width) &&
           ...
           desc.priority_regions == nullptr && desc.priority_region_count == 0;
}
...
struct DirectMlInferenceProvider::Impl {
    ...
    void process_ticket() noexcept {
        ...
        if (result == SACCADE_OK && ticket_.dispatch_.frame.ready_fence != 0) {
            result = SUCCEEDED(graphics_.queue()->Wait(reinterpret_cast<ID3D12Fence*>(ticket_.dispatch_.frame.ready_fence),
        ...
        if (result == SACCADE_OK) {
            result = preprocessor_.submit(input_texture, ticket_.dispatch_.frame.width, ticket_.dispatch_.frame.height,
                                          ...
                                          ticket_.dispatch_.frame.frame_id, ticket_.dispatch_.transform_epoch, &preprocess_submission);
        }
        ...
        if (result == SACCADE_OK) {
            ...
            if (result == SACCADE_OK)
                result = inference_.synchronize_outputs();
        }
        if (profiling) {
        ...
        if (result == SACCADE_OK) {
            ...
            const kernels::targets::PostprocessEpochs epochs{ticket_.dispatch_.frame.frame_id, ticket_.dispatch_.model_epoch,
            ...
            result = postprocessor_.submit(candidate_capacity_, ticket_.dispatch_.frame.width, ticket_.dispatch_.frame.height, config,
                                           epochs, &postprocess_submission);
        }
        if (result == SACCADE_OK) {
    ...
    void worker_loop() noexcept {
        ...
        const SaccadeResult cleanup =
            scheduling_task == nullptr || AvRevertMmThreadCharacteristics(scheduling_task) != 0 ? SACCADE_OK : SACCADE_ERROR_BACKEND;
...
DirectMlInferenceProvider::~DirectMlInferenceProvider() {
    ...
    if (state.command_event_ != nullptr)
        (void)CloseHandle(state.command_event_);
    if (state.completion_event_ != nullptr)
        (void)CloseHandle(state.completion_event_);
    if (state.worker_ready_event_ != nullptr)
        (void)CloseHandle(state.worker_ready_event_);
    state.~Impl();
}
...
SaccadeResult DirectMlInferenceProvider::initialize(DirectMlProviderConfig config) noexcept {
    Impl& state = impl();
    if (state.initialized_ || config.verifier.verify == nullptr || config.execution_policy < DirectMlExecutionPolicy::hardware_only ||
        config.execution_policy > DirectMlExecutionPolicy::hardware_then_software || config.reserved[0] != 0 || config.reserved[1] != 0 ||
        config.reserved[2] != 0 || !copy_text(config.shader_directory, &state.shader_directory_)) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    const DevicePreference preference = config.execution_policy == DirectMlExecutionPolicy::software_only ? DevicePreference::software_only
    ...
    state.capability_bits_ = base_capability_bits | (software ? SACCADE_PROVIDER_CAPABILITY_CPU : SACCADE_PROVIDER_CAPABILITY_GPU);
...
SaccadeResult DirectMlInferenceProvider::shutdown() noexcept {
    ...
    if (graphics_shutdown != SACCADE_OK)
        return graphics_shutdown;
    state.verifier_ = {};
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::enumerate_devices(void* context, uint32_t index, SaccadeDeviceInfo* output) {
    ...
    info.name = state->graphics_.software_device() ? literal_span("DirectML WARP CPU device") : literal_span("DirectML D3D12 GPU device");
    return write_structure(output, info);
}

SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::query_model(void* context, SaccadeSpanU8 bytes, SaccadeModelInfo* output) {
    Impl* state = from(context);
    ...
    info.required_device_bytes = static_cast<uint64_t>(artifact.input_width) * artifact.input_height * artifact.input_channels *
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::create_model(void* context, const SaccadeModelDesc* description,
    ...
    *output = 0;
    ...
    candidate_desc.Width = static_cast<uint64_t>(contract.candidate_capacity) * model::directml::normalized_target_row_bytes;
    ...
    if (FAILED(state->graphics_.device()->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &candidate_desc, D3D12_RESOURCE_STATE_COMMON,
                                                                  nullptr, IID_PPV_ARGS(state->candidate_buffer_.GetAddressOf())))) {
        return SACCADE_ERROR_BACKEND;
    }
    image::TensorSpec tensor_spec{};
    tensor_spec.width = artifact.input_width;
    tensor_spec.height = artifact.input_height;
    tensor_spec.format = contract.input_kind == model::directml::InputKind::planar_fp16 ? image::TensorFormat::planar_fp16
    ...
    result =
        state->preprocessor_.initialize(state->graphics_.device(), state->graphics_.queue(), state->shader_directory_.data(), tensor_spec);
    ...
    input_binding.element_type = tensor_spec.format == image::TensorFormat::planar_fp16 ? TensorElementType::fp16 : TensorElementType::int8;
    ...
    if (result == SACCADE_OK) {
        ...
        const TargetPostprocessorSpec postprocess_spec{contract.candidate_capacity,     artifact.max_targets,
                                                       state->candidate_buffer_.Get(),  0,
                                                       CandidateInput::normalized_fp16, artifact.input_width,
                                                       artifact.input_height,           0};
        result = state->postprocessor_.initialize(state->graphics_.device(), state->graphics_.queue(), state->shader_directory_.data(),
                                                  postprocess_spec);
    }
    if (result != SACCADE_OK) {
        state->reset_model_objects();
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::create_context(void* context, const SaccadeExecutionContextDesc* description,
    ...
    *output = 0;
    ...
    if (state->context_live_ || value.device_id != state->device_id_ || value.queue_capacity != 1 || value.max_in_flight != 1 ||
        value.flags != 0) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    ...
    if (WaitForSingleObject(state->worker_ready_event_, 5'000) != WAIT_OBJECT_0 || state->worker_start_result_ != SACCADE_OK) {
        (void)state->stop_worker();
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::destroy_context(void* context,
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::submit(void* context, SaccadeExecutionContextHandle execution_context,
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::poll(void* context, SaccadeExecutionContextHandle execution_context,
    ...
    return write_structure(output, status(handle, state->ticket_, state->control_state_.load(std::memory_order_acquire)));
}

SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::wait(void* context, SaccadeExecutionContextHandle execution_context,
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::collect(void* context, SaccadeExecutionContextHandle execution_context,
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::cancel(void* context, SaccadeExecutionContextHandle execution_context,
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::reset(void* context, SaccadeExecutionContextHandle execution_context) {
    Impl* state = from(context);
    if (state == nullptr)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    if (!state->context_live_ || !decode_handle(execution_context, state->context_generation_)) {
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::synchronize(void* context, SaccadeExecutionContextHandle execution_context,
                                                                        uint64_t timeout_ns) {
    Impl* state = from(context);
    if (state == nullptr)
        return SACCADE_ERROR_INVALID_ARGUMENT;
...
SaccadeResult SACCADE_CALL DirectMlInferenceProvider::Impl::memory_stats(void* context, SaccadeExecutionContextHandle execution_context,
    ...
    stats.device_imported = state->ticket_.active_
                                ...
                                : 0;
    stats.device_owned =
        state->postprocessor_.stats().workspace_bytes + state->inference_.stats().input_bytes + state->inference_.stats().output_bytes;
