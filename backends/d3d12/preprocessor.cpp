...
bool tensor_spec_valid(const TensorSpec& spec) noexcept {
    return spec.width != 0 && spec.height != 0 && (spec.format == TensorFormat::planar_fp16 || spec.format == TensorFormat::planar_int8) &&
           spec.reserved == 0;
}
...
struct ImagePreprocessor::Impl {
    ...
    SaccadeResult completion_status() noexcept {
        ...
        if (sequence_ == completed_sequence_)
            return SACCADE_OK;
...
SaccadeResult ImagePreprocessor::initialize(ID3D12Device* device, ID3D12CommandQueue* queue, const char* shader_directory,
    ...
    const char* shader_name = spec.format == TensorFormat::planar_fp16 ? "preprocess_fp16.dxil" : "preprocess_int8.dxil";
    ...
    if (FAILED(device->CreateCommittedResource(&heap_properties, D3D12_HEAP_FLAG_NONE, &output_desc, D3D12_RESOURCE_STATE_COMMON, nullptr,
    ...
    if (state.completion_event_ == nullptr ||
        FAILED(device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(state.allocator_.GetAddressOf()))) ||
        FAILED(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, state.allocator_.Get(), state.pipeline_.Get(),
...
SaccadeResult ImagePreprocessor::submit(ID3D12Resource* source, uint32_t source_width, uint32_t source_height, SourceRegion region,
    ...
    if (static_cast<uint64_t>(region.x) + region.width > source_width || static_cast<uint64_t>(region.y) + region.height > source_height) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    ...
    state.device_->CreateShaderResourceView(source, &source_view, state.descriptors_->GetCPUDescriptorHandleForHeapStart());
    if (FAILED(state.allocator_->Reset()) || FAILED(state.commands_->Reset(state.allocator_.Get(), state.pipeline_.Get()))) {
...
SaccadeResult ImagePreprocessor::tensor(const PreprocessSubmission* submission, TensorView* output) noexcept {
    ...
    *output =
        {state.output_.Get(), state.output_bytes_, state.plane_stride_bytes_, state.spec_.width, state.spec_.height, state.spec_.format, 3};
    return SACCADE_OK;
}
...
SaccadeResult ImagePreprocessor::completion_dependency(const PreprocessSubmission* submission, ID3D12Fence** fence,
    ...
    if (!initialized_ || !impl().owns_thread() || !impl().submission_matches(submission) || fence == nullptr || value == nullptr) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
