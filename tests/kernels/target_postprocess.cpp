...
int main() {
    ...
    const std::array<DenseCandidate, 7> candidates{
        {{0, 0, 80, 80, 65000, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0},
         ...
         {240, 0, 32, 32, 61000, SACCADE_TARGET_ROLE_TEXT_FIELD, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0},
         {320, 0, 32, 32, 1000, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0},
         {400, 0, 32, 32, 65535, SACCADE_TARGET_ROLE_UNKNOWN, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0}}};
    ...
    const SaccadeResult result = postprocess(candidates.data(), static_cast<uint32_t>(candidates.size()), config, epochs, &workspace,
    ...
    if (result != SACCADE_OK || allocations != 0 || saccade::scene::validate_packet({output.data(), required}, &packet) != SACCADE_OK ||
        ...
        packet.targets[2].capability_bits !=
            (SACCADE_TARGET_CAPABILITY_POINTER_MOVE | SACCADE_TARGET_CAPABILITY_BUTTON | SACCADE_TARGET_CAPABILITY_INVOKE) ||
        stats.candidates_read != candidates.size() || stats.candidates_above_threshold != 6 || stats.heap_replacements != 1 ||
        stats.containment_suppressed != 1 || stats.targets_written != 3) {
        return 1;
    }

    size_t small_required = 0;
    if (postprocess(candidates.data(), static_cast<uint32_t>(candidates.size()), config, epochs, &workspace, {output.data(), 8},
                    &small_required, &stats) != SACCADE_ERROR_CAPACITY ||
    ...
    const std::array<DenseCandidate, 4> band_candidates{{
        ...
        {160, 160, 128, 160, 1500, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0},
        {400, 160, 200, 240, 1500, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0},
        {720, 160, 80, 80, 2500, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE, 0},
    }};
    ...
    if (postprocess(band_candidates.data(), static_cast<uint32_t>(band_candidates.size()), config, epochs, &workspace,
        ...
        saccade::scene::validate_packet({output.data(), required}, &packet) != SACCADE_OK || packet.header->target_count != 2 ||
        packet.targets[0].confidence_q16 != 2500 || packet.targets[1].confidence_q16 != 1500 || stats.candidates_above_threshold != 2) {
        return 3;
    }
    ...
    const DenseCandidate tiny{880, 160, 16, 16, 65535, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_NEURAL, SACCADE_TARGET_ACTIONABLE,
                              0};
    config.band_minimum_confidence_q16 = 0;
    config.band_min_short_side_q3 = 0;
    config.maximum_targets = 1;
    if (postprocess(&tiny, 1, config, epochs, &workspace, {output.data(), output.size()}, &required, &stats) != SACCADE_OK ||
        saccade::scene::validate_packet({output.data(), required}, &packet) != SACCADE_OK || packet.header->target_count != 1 ||
        packet.targets[0].safe_x_q8 != (888 << 5) || packet.targets[0].safe_y_q8 != (168 << 5) || packet.targets[0].capability_bits != 0 ||
