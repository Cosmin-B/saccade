...
int main() {
    ...
    if (previous_priority_class == 0)
        return exit_code(ExitCode::initial_state);

    saccade::platform::windows::RuntimeScheduling scheduling;
    if (scheduling.initialized())
        return exit_code(ExitCode::initial_state);
    if (scheduling.initialize() != SACCADE_OK)
        return exit_code(ExitCode::initialization);
    if (!scheduling.initialized())
        return exit_code(ExitCode::active_state);
    ...
    if (scheduling.initialize() != SACCADE_ERROR_ALREADY_EXISTS)
        return exit_code(ExitCode::duplicate_initialization);
    if (scheduling.shutdown() != SACCADE_OK)
        return exit_code(ExitCode::shutdown);
    if (scheduling.initialized())
        return exit_code(ExitCode::final_state);
    if (GetPriorityClass(GetCurrentProcess()) != previous_priority_class)
        return exit_code(ExitCode::process_priority_restore);
    if (scheduling.shutdown() != SACCADE_ERROR_STATE)
        return exit_code(ExitCode::duplicate_shutdown);
