...
int main() {
    ...
    if (saccade::platform::windows::resolve_hint_language(settings.hints, &resolved, &layout_token) != SACCADE_OK || layout_token == 0 ||
    ...
    const std::array<saccade::application::HotkeyBinding, 3> bindings{
        {{saccade::application::Command::pointer_move, hid_keyboard_f11, binding_modifiers, 0},
         {saccade::application::Command::suspend_toggle, hid_keyboard_f12, binding_modifiers, saccade::application::hotkey_always_active},
         {saccade::application::Command::scope_toggle, hid_keyboard_tab, 0, saccade::application::hotkey_session_only}}};
    ...
    if (hotkeys.dispatch_physical(hid_keyboard_f11, binding_modifiers, first_timestamp_ns) != SACCADE_OK || capture.count != 1 ||
    ...
    if (hotkeys.set_suspended(true) != SACCADE_OK ||
        hotkeys.dispatch_physical(hid_keyboard_f11, binding_modifiers, second_timestamp_ns) != SACCADE_OK || capture.count != 1 ||
        capture.command_inputs != 1 || hotkeys.dispatch_physical(hid_keyboard_f12, binding_modifiers, second_timestamp_ns) != SACCADE_OK ||
        capture.count != 2 || capture.command_inputs != 2 || capture.events[1].command != saccade::application::Command::suspend_toggle ||
    ...
    if (hotkeys.dispatch_physical(hid_keyboard_tab, 0, second_timestamp_ns) != SACCADE_ERROR_NOT_FOUND || capture.count != 2 ||
        capture.command_inputs != 2) {
    ...
    const saccade::application::HotkeyBinding latest{saccade::application::Command::target_position_9, hid_keyboard_f11, binding_modifiers,
                                                     0};
