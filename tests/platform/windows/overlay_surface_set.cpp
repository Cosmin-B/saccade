...
int main(int argc, char** argv) {
    ...
    if (surfaces.read_surface_info(1, &first) != SACCADE_OK || surfaces.read_surface_info(2, &second) != SACCADE_OK ||
        ...
        (first.flags & saccade::platform::windows::overlay_surface_visible) == 0 || surfaces.set_click_through(false) != SACCADE_OK) {
        return 5;
    }
    snapshot.epoch = 2;
    snapshot.count = 1;
    ...
    if (surfaces.synchronize(snapshot) != SACCADE_OK || surfaces.synchronize(snapshot) != SACCADE_OK ||
        surfaces.read_surface_info(2, &second) != SACCADE_ERROR_NOT_FOUND || surfaces.read_surface_info(1, &first) != SACCADE_OK ||
        first.drawable_width != 320) {
        return 6;
    }
    ...
    if (surfaces.read_stats(&stats) != SACCADE_OK || stats.synchronize_attempts != 3 || stats.topology_changes != 2 ||
        stats.surfaces_added != 2 || stats.surfaces_removed != 1 || stats.surfaces_updated != 1 || stats.active_surfaces != 1 ||
        stats.running != 1 || surfaces.stop() != SACCADE_OK || surfaces.shutdown() != SACCADE_OK ||
        surfaces.initialize(graphics.device(), graphics.queue(), argv[1], {nullptr, no_frame, nullptr}) != SACCADE_OK ||
        surfaces.synchronize(snapshot) != SACCADE_OK || surfaces.start() != SACCADE_OK || surfaces.stop() != SACCADE_OK ||
