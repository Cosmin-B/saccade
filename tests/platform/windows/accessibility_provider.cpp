...
int main() {
    ...
    HWND window = CreateWindowExW(0, type.lpszClassName, L"provider test", WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, nullptr, nullptr,
                                  instance, nullptr);
    if (window == nullptr)
        return 2;
    HWND button =
        CreateWindowExW(0, L"BUTTON", L"Run", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 30, 30, 120, 40, window, nullptr, instance, nullptr);
    HWND edit = CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 30, 90, 240, 32, window, nullptr,
                                instance, nullptr);
    if (button == nullptr || edit == nullptr)
        return 3;
    ...
    if (polled != SACCADE_OK || status.state != SACCADE_TICKET_COMPLETE || status.snapshot == 0 || status.target_count < 2)
        return 6;

    size_t required = 0;
    if (desc.ops.collect(desc.context, status.snapshot, {}, &required) != SACCADE_ERROR_CAPACITY || required != status.required_bytes)
        return 7;
    ...
    if (saccade::scene::validate_packet({packet.data(), packet.size()}, &validated) != SACCADE_OK)
        return 9;
    if (header->packet_version != SACCADE_TARGET_PACKET_VERSION || header->coordinate_space != SACCADE_COORDINATE_SPACE_DESKTOP_Q8 ||
        header->session_epoch != 7 || header->transform_epoch != 9 || header->topology_epoch != 11 || header->frame_id != 13 ||
    ...
    for (uint32_t index = 0; index < header->target_count; ++index) {
        found_button |=
            targets[index].role == SACCADE_TARGET_ROLE_BUTTON && (targets[index].capability_bits & SACCADE_TARGET_CAPABILITY_BUTTON) != 0;
    ...
    if (!found_button || !found_text || desc.ops.release(desc.context, status.snapshot) != SACCADE_OK)
        return 10;
