...
bool exact_selection_and_background_validation() noexcept {
    ...
    if (session.publish(background, generation(100, 200, 300), &token) != SACCADE_OK || token.session_epoch != 10 ||
        token.scene_epoch != 100 || token.process_id != 410 || token.window_id != 91 || token.transform_epoch != 200 ||
        token.permission_epoch != 300 || token.bounds.x != 100 || token.bounds.y != 200 || token.bounds.width != 800 ||
        token.bounds.height != 600) {
        return false;
    }
...
bool cancelled_activation_is_not_replayed() noexcept {
    ...
    if (state.admit(first) != ExplicitWindowActivationAdmission::start || !state.waiting() ||
        ...
        state.admit(first) != ExplicitWindowActivationAdmission::resume || state.admit(second) != ExplicitWindowActivationAdmission::busy) {
        return false;
    }
    ...
    state.cancel();
    if (state.admit(second) != ExplicitWindowActivationAdmission::start || state.set_previous_scene_epoch(701) != SACCADE_OK) {
        return false;
    }
    state.complete();
    const ExplicitWindowActivationKey invalid{};
    return !state.waiting() && state.previous_scene_epoch() == 0 && state.admit(invalid) == ExplicitWindowActivationAdmission::invalid;
}
