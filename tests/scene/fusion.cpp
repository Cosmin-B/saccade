...
constexpr size_t maximum_packet_size = sizeof(SaccadeTargetPacketHeader) +
...
PacketView packet(PacketStorage* storage, uint32_t count, uint64_t model_epoch = 70) {
    ...
    storage->header.total_size = sizeof(SaccadeTargetPacketHeader) + static_cast<uint64_t>(count) * sizeof(SaccadeTargetRecord);
...
int main() {
    ...
    if (saccade::scene::validate_packet({output.data(), required}, &fused) != SACCADE_OK || fused.header->scene_epoch != 100 ||
        fused.header->capture_time_ns != 2 || fused.header->target_count != 2 || fused.header->flags != SACCADE_TARGET_PACKET_INCOMPLETE ||
        fused.targets[0].target_id != 10 ||
        fused.targets[0].source_bits != (SACCADE_TARGET_SOURCE_ACCESSIBILITY | SACCADE_TARGET_SOURCE_NEURAL) ||
        fused.targets[0].flags != SACCADE_TARGET_DISABLED || fused.targets[0].capability_bits != 0 || fused.targets[1].target_id != 30) {
        return 2;
    }
    ...
    if (saccade::scene::fuse(inputs.data(), static_cast<uint32_t>(inputs.size()), config, epochs(), &workspace,
        ...
        saccade::scene::validate_packet({output.data(), required}, &fused) != SACCADE_OK || fused.header->target_count != 3 ||
        fused.targets[0].source_bits != SACCADE_TARGET_SOURCE_ACCESSIBILITY ||
    ...
    SaccadeTargetRecord semantic_label = target(40, 20, 20, SACCADE_TARGET_ROLE_BUTTON, SACCADE_TARGET_SOURCE_ACCESSIBILITY);
    ...
    if (saccade::scene::fuse(label_inputs, 2, config, epochs(), &workspace, {output.data(), output.size()}, &required, &stats) !=
            SACCADE_OK ||
        saccade::scene::validate_packet({output.data(), required}, &fused) != SACCADE_OK || fused.header->target_count != 1 ||
        fused.target_text(0).size != 6 ||
        fused.targets[0].capability_bits != (SACCADE_TARGET_CAPABILITY_POINTER_MOVE | SACCADE_TARGET_CAPABILITY_BUTTON) ||
    ...
    if (saccade::scene::fuse(inputs.data(), 3, config, stale, &workspace, {output.data(), output.size()}, &required, &stats) !=
    ...
    if (saccade::scene::fuse(inputs.data(), 3, config, epochs(), &workspace, {output.data(), 64}, &required, &stats) !=
        ...
        required != sizeof(SaccadeTargetPacketHeader) + 8 * sizeof(SaccadeTargetRecord) + SACCADE_TARGET_PACKET_MAX_TEXT_BYTES) {
        return 5;
    }
    ...
    if (saccade::scene::fuse(&full, 1, config, epochs(), &workspace, {output.data(), output.size()}, &required, &stats) != SACCADE_OK ||
        stats.targets_written != SACCADE_TARGET_PACKET_MAX_TARGETS || stats.duplicates_merged != 0 || stats.capacity_drops != 0 ||
