...
struct Fixture {
    ...
    bool remap(Packet* packet) noexcept { return tracker.remap(&packet->header, packet->targets.data(), &stats) == SACCADE_OK; }
};
...
SaccadeTargetRecord visual_target(uint64_t incoming_id, int32_t x, int32_t y, int32_t width = 48, int32_t height = 32) noexcept {
    SaccadeTargetRecord target{};
...
Packet packet(uint64_t frame_id, int32_t window_x, int32_t window_y, int32_t window_width, int32_t window_height) noexcept {
    Packet value{};
...
void append(Packet* packet, const SaccadeTargetRecord& target) noexcept {
    ...
    packet->header.total_size =
        sizeof(SaccadeTargetPacketHeader) + static_cast<uint64_t>(packet->header.target_count) * sizeof(SaccadeTargetRecord);
}
...
bool crossing_test() noexcept {
    ...
    return fixture.remap(&fourth) && fourth.targets[1].target_id == leftward_id && fourth.targets[2].target_id == rightward_id;
}
...
bool semantic_identity_test() noexcept {
    ...
    return fixture.remap(&value) && value.targets[0].target_id == test_window_id && value.targets[1].target_id == accessibility_id &&
           value.targets[2].target_id == fused_id && value.targets[3].target_id != 303 && fixture.stats.passthrough_targets == 3;
}
