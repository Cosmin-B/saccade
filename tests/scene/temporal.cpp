...
void initialize(PacketStorage* packet, uint64_t scene_epoch, uint64_t frame_id, uint64_t session_epoch, uint64_t transform_epoch,
                uint32_t count, const char* text) noexcept {
    packet->header = {};
    ...
    packet->header.total_size = sizeof(SaccadeTargetPacketHeader) + static_cast<size_t>(count) * sizeof(SaccadeTargetRecord) + text_size;
}

saccade::scene::PacketView view(PacketStorage* packet, size_t text_size) noexcept {
    return {&packet->header, packet->targets.data(), static_cast<size_t>(packet->header.total_size), packet->text.data(),
...
int main() {
    ...
    if (compiler.compile(view(&first, 6), {output.data(), output.size()}, &required, &stats) != SACCADE_OK ||
        header(output)->operation_count != 3 || (header(output)->flags & SACCADE_SCENE_DELTA_BASELINE) == 0 || stats.additions != 3 ||
        stats.updates != 0 || stats.removals != 0 || records(output)[0].operation != SACCADE_SCENE_DELTA_ADD ||
        records(output)[0].window_id != 11 || (records(output)[0].changed_fields & SACCADE_SCENE_DELTA_TEXT) == 0) {
    ...
    if (compiler.compile(view(&second, 10), {output.data(), output.size()}, &required, &stats) != SACCADE_OK || stats.additions != 1 ||
        stats.updates != 1 || stats.removals != 1 || header(output)->operation_count != 3 || records(output)[0].target_id != 2 ||
        records(output)[0].operation != SACCADE_SCENE_DELTA_UPDATE ||
        records(output)[0].changed_fields != (SACCADE_SCENE_DELTA_GEOMETRY | SACCADE_SCENE_DELTA_CONFIDENCE | SACCADE_SCENE_DELTA_TEXT) ||
        records(output)[1].target_id != 4 || records(output)[1].operation != SACCADE_SCENE_DELTA_ADD || records(output)[2].target_id != 3 ||
        records(output)[2].operation != SACCADE_SCENE_DELTA_REMOVE) {
    ...
    if (compiler.compile(view(&capacity, 10), {output.data(), sizeof(SaccadeSceneDeltaHeader) - 1U}, &exact_required, &stats) !=
    ...
    if (overlapping_compiler.initialize(&overlapping_storage) != SACCADE_OK ||
        overlapping_compiler.compile(view(&overlapping_first, 0), {output.data(), output.size()}, &required, &stats) != SACCADE_OK ||
        overlapping_compiler.compile(view(&overlapping_second, 0), {output.data(), output.size()}, &required, &stats) != SACCADE_OK ||
    ...
    if (compiler.compile(view(&reset, 3), {output.data(), output.size()}, &required, &stats) != SACCADE_OK ||
        ...
        records(output)[0].operation != SACCADE_SCENE_DELTA_REMOVE || records(output)[3].operation != SACCADE_SCENE_DELTA_ADD) {
    ...
    const SaccadeResult measured_result = compiler.compile(view(&measured, 3), {output.data(), output.size()}, &required, &stats);
