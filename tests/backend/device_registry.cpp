...
SaccadeInferenceProviderDesc provider_desc(uint64_t stable_id) {
    ...
    desc.info.capability_bits = SACCADE_PROVIDER_CAPABILITY_CPU |
                                ...
                                SACCADE_PROVIDER_CAPABILITY_NATIVE_IMPORT |
...
int main() {
    ...
    SaccadeDeviceInfo cpu = device_info(
        100,
        SACCADE_PROVIDER_CAPABILITY_CPU,
        SACCADE_FORMAT_BGRA8 | SACCADE_FORMAT_RGBA8,
        SACCADE_PRECISION_FP32,
        SACCADE_IMPORT_HOST);
    cpu.name = {
    ...
    if (registry.register_device(provider_handle, &cpu, &cpu_handle) != SACCADE_OK ||
        cpu_handle == 0) {
        return 2;
    }
    mutable_name[0] = 'x';
    const auto* stored_cpu = registry.device(cpu_handle);
    if (stored_cpu == nullptr || stored_cpu->provider != provider_handle ||
        stored_cpu->info.name.size != 7 || stored_cpu->info.name.data[0] != 'm') {
        return 3;
    }
    if (registry.register_device(provider_handle, &cpu, nullptr) !=
        SACCADE_ERROR_ALREADY_EXISTS) {
        return 4;
    }

    SaccadeDeviceInfo gpu = device_info(
        101,
        SACCADE_PROVIDER_CAPABILITY_GPU | SACCADE_PROVIDER_CAPABILITY_ASYNC,
        SACCADE_FORMAT_BGRA8,
        SACCADE_PRECISION_FP16 | SACCADE_PRECISION_INT8,
        SACCADE_IMPORT_HOST | SACCADE_IMPORT_IOSURFACE,
        "gpu");
    ...
    if (registry.select_device(requirements, &selection) != SACCADE_OK ||
        selection.handle != gpu_handle || selection.provider != provider_handle ||
        selection.stable_id != 101 ||
    ...
    requirements.required_import_bits = SACCADE_IMPORT_D3D11;
    ...
    SaccadeDeviceInfo malformed = device_info(
        200,
        SACCADE_PROVIDER_CAPABILITY_CPU,
        SACCADE_FORMAT_BGRA8,
        SACCADE_PRECISION_FP32,
    ...
    if (malformed_registry.register_device(provider_handle, &malformed, nullptr) !=
        SACCADE_ERROR_INVALID_ARGUMENT) {
        return 10;
    }
    malformed = device_info(
        201,
        SACCADE_PROVIDER_CAPABILITY_CPU,
        SACCADE_FORMAT_BGRA8,
        SACCADE_PRECISION_FP32,
    ...
    if (malformed_registry.register_device(UINT64_C(0xBAD), &malformed, nullptr) !=
    ...
    SaccadeDeviceInfo misaligned = device_info(
        203,
        SACCADE_PROVIDER_CAPABILITY_CPU,
        SACCADE_FORMAT_BGRA8,
        SACCADE_PRECISION_FP32,
        SACCADE_IMPORT_HOST);
    alignas(SaccadeDeviceInfo)
        std::array<std::byte, sizeof(SaccadeDeviceInfo) + 1> device_storage{};
    std::memcpy(device_storage.data() + 1, &misaligned, sizeof(misaligned));
    const auto* misaligned_info = reinterpret_cast<const SaccadeDeviceInfo*>(
        device_storage.data() + 1);
    if (malformed_registry.register_device(provider_handle, misaligned_info, nullptr) !=
        SACCADE_OK) {
        return 23;
    }
    ...
    for (size_t index = 0; index < ProviderRegistry::device_capacity; ++index) {
        SaccadeDeviceInfo item = device_info(
            300 + static_cast<uint64_t>(index),
            SACCADE_PROVIDER_CAPABILITY_CPU,
            SACCADE_FORMAT_BGRA8,
            SACCADE_PRECISION_FP32,
            SACCADE_IMPORT_HOST);
    ...
    SaccadeDeviceInfo overflow = device_info(
        999,
        SACCADE_PROVIDER_CAPABILITY_CPU,
        SACCADE_FORMAT_BGRA8,
        SACCADE_PRECISION_FP32,
        SACCADE_IMPORT_HOST);
    if (capacity_registry.register_device(provider_handle, &overflow, nullptr) !=
    ...
    SaccadeDeviceInfo foreign = device_info(
        400,
        SACCADE_PROVIDER_CAPABILITY_CPU,
        SACCADE_FORMAT_BGRA8,
        SACCADE_PRECISION_FP32,
    ...
    if (foreign_registry.register_device(foreign_provider, &foreign, &foreign_handle) !=
            SACCADE_OK ||
