...
SaccadeDeviceInfo device_info(uint64_t stable_id, uint32_t capabilities, uint32_t formats, uint32_t precisions, uint32_t imports,
...
int main() {
    ...
    SaccadeDeviceInfo cpu = device_info(100, SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8 | SACCADE_FORMAT_RGBA8,
    ...
    SaccadeDeviceInfo gpu =
        device_info(101, SACCADE_PROVIDER_CAPABILITY_GPU | SACCADE_PROVIDER_CAPABILITY_ASYNC, SACCADE_FORMAT_BGRA8,
                    SACCADE_PRECISION_FP16 | SACCADE_PRECISION_INT8, SACCADE_IMPORT_HOST | SACCADE_IMPORT_IOSURFACE, "gpu");
    ...
    if (registry.select_device(requirements, &selection) != SACCADE_OK || selection.handle != gpu_handle ||
        selection.provider != provider_handle || selection.stable_id != 101 || selection.reason != SelectionReason::preferred_capability) {
        return 6;
    }
    if (registry.select_device_by_id(provider_handle, 100, &selection) != SACCADE_OK || selection.handle != cpu_handle ||
    ...
    SaccadeDeviceInfo malformed =
        device_info(200, SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8, SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
    ...
    malformed = device_info(201, SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8, SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
    ...
    malformed =
        device_info(202, SACCADE_PROVIDER_CAPABILITY_ACCELERATOR, SACCADE_FORMAT_BGRA8, SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
    if (malformed_registry.register_device(provider_handle, &malformed, nullptr) != SACCADE_ERROR_INVALID_ARGUMENT) {
        return 22;
    }

    SaccadeDeviceInfo misaligned =
        device_info(203, SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8, SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
    ...
    for (size_t index = 0; index < ProviderRegistry::device_capacity; ++index) {
        SaccadeDeviceInfo item = device_info(300 + static_cast<uint64_t>(index), SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8,
                                             SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
        if (capacity_registry.register_device(provider_handle, &item, nullptr) != SACCADE_OK) {
            return 15;
        }
    }
    SaccadeDeviceInfo overflow =
        device_info(999, SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8, SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
    ...
    for (int index = 0; index < 1000; ++index) {
        if (registry.select_device(requirements, &selection) != SACCADE_OK || registry.device(selection.handle) == nullptr) {
            return 18;
        }
    }
    ...
    SaccadeDeviceInfo foreign =
        device_info(400, SACCADE_PROVIDER_CAPABILITY_CPU, SACCADE_FORMAT_BGRA8, SACCADE_PRECISION_FP32, SACCADE_IMPORT_HOST);
    SaccadeDeviceHandle foreign_handle = 0;
    if (foreign_registry.register_device(foreign_provider, &foreign, &foreign_handle) != SACCADE_OK || foreign_handle == cpu_handle ||
        foreign_registry.device(cpu_handle) != nullptr) {
        return 21;
    }
