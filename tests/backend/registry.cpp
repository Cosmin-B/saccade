...
SaccadeProviderInfo provider_info(
    uint32_t family,
    uint64_t stable_id,
...
SaccadeInferenceOps inference_ops() {
    ...
    ops.destroy_model = +[](void*, SaccadeModelHandle) -> SaccadeResult {
        return SACCADE_OK;
    };
    ...
    ops.poll = +[](void*, SaccadeExecutionContextHandle, SaccadeTicketHandle,
                   SaccadeInferenceStatus*) -> SaccadeResult {
        return SACCADE_OK;
    };
    ...
    ops.collect = +[](void*, SaccadeExecutionContextHandle, SaccadeTicketHandle,
                      SaccadeMutableSpanU8, size_t*) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.cancel = +[](void*, SaccadeExecutionContextHandle,
                     SaccadeTicketHandle) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.reset = +[](void*, SaccadeExecutionContextHandle) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.synchronize = +[](void*, SaccadeExecutionContextHandle, uint64_t) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.memory_stats = +[](void*, SaccadeExecutionContextHandle,
                           SaccadeMemoryStats*) -> SaccadeResult {
        return SACCADE_OK;
    };
    return ops;
}
...
SaccadeCaptureOps capture_ops() {
    ...
    ops.struct_size = static_cast<uint32_t>(sizeof(ops));
    ops.api_version = SACCADE_API_VERSION;
    ops.enumerate_sources = +[](void*, uint32_t, SaccadeCaptureSourceInfo*) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.create = +[](void*, const SaccadeCaptureStreamDesc*,
                     SaccadeCaptureStreamHandle*) -> SaccadeResult { return SACCADE_OK; };
    ...
    ops.acquire = +[](void*, SaccadeCaptureStreamHandle, uint64_t,
                      SaccadeCapturedFrame*) -> SaccadeResult { return SACCADE_OK; };
    ops.copy_damage = +[](void*, SaccadeCaptureStreamHandle, SaccadeFrameHandle,
                          SaccadeRectI32*, uint32_t, uint32_t*) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.release = +[](void*, SaccadeCaptureStreamHandle,
                      SaccadeFrameHandle) -> SaccadeResult { return SACCADE_OK; };
    ops.synchronize = +[](void*, SaccadeCaptureStreamHandle,
                          uint64_t) -> SaccadeResult { return SACCADE_OK; };
    ops.memory_stats = +[](void*, SaccadeCaptureStreamHandle,
...
SaccadeOverlayOps overlay_ops() {
    ...
    ops.struct_size = static_cast<uint32_t>(sizeof(ops));
    ops.api_version = SACCADE_API_VERSION;
    ops.create = +[](void*, const SaccadeOverlayDesc*,
                     SaccadeOverlayHandle*) -> SaccadeResult { return SACCADE_OK; };
    ops.destroy = +[](void*, SaccadeOverlayHandle) -> SaccadeResult { return SACCADE_OK; };
    ops.submit = +[](void*, SaccadeOverlayHandle,
                     const SaccadeOverlayFrameDesc*) -> SaccadeResult { return SACCADE_OK; };
    ops.set_visible = +[](void*, SaccadeOverlayHandle, uint32_t) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.synchronize = +[](void*, SaccadeOverlayHandle, uint64_t) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.memory_stats = +[](void*, SaccadeOverlayHandle,
                           SaccadeMemoryStats*) -> SaccadeResult { return SACCADE_OK; };
...
SaccadeAccessibilityOps accessibility_ops() {
    ...
    ops.struct_size = static_cast<uint32_t>(sizeof(ops));
    ops.api_version = SACCADE_API_VERSION;
    ops.enumerate_windows = +[](void*, uint32_t, SaccadeWindowInfo*) -> SaccadeResult {
        return SACCADE_OK;
    };
    ops.request = +[](void*, const SaccadeAccessibilityQueryDesc*,
                      SaccadeTicketHandle*) -> SaccadeResult { return SACCADE_OK; };
    ops.poll = +[](void*, SaccadeTicketHandle,
                   SaccadeAccessibilityStatus*) -> SaccadeResult { return SACCADE_OK; };
    ops.wait = +[](void*, SaccadeTicketHandle, uint64_t,
                   SaccadeAccessibilityStatus*) -> SaccadeResult { return SACCADE_OK; };
    ops.collect = +[](void*, SaccadeSnapshotHandle, SaccadeMutableSpanU8,
...
SaccadeInputOps input_ops() {
    ...
    ops.struct_size = static_cast<uint32_t>(sizeof(ops));
    ops.api_version = SACCADE_API_VERSION;
    ops.execute = +[](void*, const SaccadeInputPlanDesc*,
                      SaccadeTicketHandle*) -> SaccadeResult { return SACCADE_OK; };
    ops.poll = +[](void*, SaccadeTicketHandle,
                   SaccadeInputStatus*) -> SaccadeResult { return SACCADE_OK; };
    ops.wait = +[](void*, SaccadeTicketHandle, uint64_t,
                   SaccadeInputStatus*) -> SaccadeResult { return SACCADE_OK; };
    ops.cancel = +[](void*, SaccadeTicketHandle) -> SaccadeResult { return SACCADE_OK; };
...
SaccadeInferenceProviderDesc inference_provider(
    uint64_t id,
    uint32_t capabilities,
    const char* name = "inference") {
...
int main() {
    ...
    SaccadeInferenceProviderDesc cpu = inference_provider(
        10, SACCADE_PROVIDER_CAPABILITY_CPU, "mutable");
    char mutable_name[] = "mutable";
    cpu.info.name = {
        reinterpret_cast<const uint8_t*>(mutable_name),
        sizeof(mutable_name) - 1
    };
    ...
    if (stored_cpu == nullptr || stored_cpu->info.name.size != 7 ||
    ...
    SaccadeInferenceProviderDesc gpu = inference_provider(
        11,
        SACCADE_PROVIDER_CAPABILITY_CPU | SACCADE_PROVIDER_CAPABILITY_GPU,
        "gpu");
    ...
    if (registry.select_inference(
            SACCADE_PROVIDER_CAPABILITY_CPU,
            SACCADE_PROVIDER_CAPABILITY_GPU,
            &selection) != SACCADE_OK ||
        selection.handle != gpu_handle || selection.stable_id != 11 ||
        selection.reason != SelectionReason::preferred_capability) {
        return 5;
    }
    if (registry.select_inference_by_id(10, &selection) != SACCADE_OK ||
        selection.handle != cpu_handle ||
        selection.reason != SelectionReason::explicit_id) {
        return 6;
    }
    if (registry.select_inference(
            SACCADE_PROVIDER_CAPABILITY_ACCELERATOR, 0, &selection) !=
    ...
    SaccadeInferenceProviderDesc malformed = inference_provider(
        100, SACCADE_PROVIDER_CAPABILITY_CPU);
    malformed.struct_size = 0;
    if (malformed_registry.register_inference(&malformed, nullptr) !=
    ...
    malformed.struct_size = static_cast<uint32_t>(
        offsetof(SaccadeInferenceProviderDesc, reserved) - 1);
    if (malformed_registry.register_inference(&malformed, nullptr) !=
        SACCADE_ERROR_INVALID_ARGUMENT) {
        return 11;
    }
    ...
    SaccadeInferenceProviderDesc smaller = inference_provider(
        200, SACCADE_PROVIDER_CAPABILITY_CPU);
    smaller.struct_size = static_cast<uint32_t>(
        offsetof(SaccadeInferenceProviderDesc, reserved));
    ...
    if (stored_smaller == nullptr || stored_smaller->info.reserved[0] != 0 ||
        stored_smaller->ops.reserved[0] != 0) {
        return 22;
    }
    ...
    for (size_t index = 0; index < ProviderRegistry::capacity_per_family; ++index) {
        SaccadeInferenceProviderDesc item = inference_provider(
            300 + static_cast<uint64_t>(index), SACCADE_PROVIDER_CAPABILITY_CPU);
        if (capacity_registry.register_inference(&item, nullptr) != SACCADE_OK) {
            return 24;
        }
    }
    SaccadeInferenceProviderDesc overflow = inference_provider(
        400, SACCADE_PROVIDER_CAPABILITY_CPU);
    if (capacity_registry.register_inference(&overflow, nullptr) != SACCADE_ERROR_CAPACITY) {
        return 25;
    }

    registry.freeze();
    SaccadeInferenceProviderDesc late = inference_provider(
        500, SACCADE_PROVIDER_CAPABILITY_CPU);
    if (!registry.frozen() ||
        registry.register_inference(&late, nullptr) != SACCADE_ERROR_STATE) {
        return 26;
    }
    ...
    SaccadeInferenceProviderDesc foreign_provider = inference_provider(
        600, SACCADE_PROVIDER_CAPABILITY_CPU);
    SaccadeProviderHandle foreign_handle = 0;
    if (foreign_registry.register_inference(&foreign_provider, &foreign_handle) !=
            SACCADE_OK ||
