...
bool max_in_flight_is_enforced() {
    ...
    submit.output_capacity = 32;
    submit.model_epoch = 1;
    submit.session_epoch = 1;
    submit.transform_epoch = 1;
    ...
    size_t required = 0;
    if (provider.ops.collect(provider.context, execution, first, {output.data(), output.size()},
                             &required) != SACCADE_OK ||
        provider.ops.collect(provider.context, execution, second, {output.data(), output.size()},
    ...
    if (provider.ops.submit(provider.context, execution, &submit, &reset_ticket) != SACCADE_OK ||
        ...
        first_status.state != SACCADE_TICKET_RUNNING ||
        provider.ops.reset(provider.context, execution) != SACCADE_OK ||
        ...
        provider.ops.collect(provider.context, execution, reset_ticket,
    ...
    if (provider.ops.submit(provider.context, execution, &submit, &queued) != SACCADE_OK ||
        ...
        provider.ops.collect(provider.context, execution, queued, {output.data(), output.size()},
                             &required) != SACCADE_OK ||
        provider.ops.collect(provider.context, execution, running, {output.data(), output.size()},
                             &required) != SACCADE_OK ||
...
bool input_tickets_are_reclaimed() {
    ...
    const uint8_t actions[] = {1};
    SaccadeInputPlanDesc plan{};
    plan.struct_size = static_cast<uint32_t>(sizeof(plan));
    plan.api_version = SACCADE_API_VERSION;
    plan.session_epoch = 1;
    plan.transform_epoch = 1;
    plan.actions = {actions, sizeof(actions)};
...
int main() {
    ...
    if (saccade::backend::detail::guarded_callback<&successful_callback>(nullptr) != SACCADE_OK ||
        saccade::backend::detail::guarded_callback<&throwing_callback>(nullptr) !=
    ...
    if (inference.ops.enumerate_devices(inference.context, 0, &enumerated_device) != SACCADE_OK ||
        ...
        inference.ops.enumerate_devices(inference.context, 1, &enumerated_device) !=
    ...
    if (inference.ops.query_model(inference.context, {model_bytes, sizeof(model_bytes)},
                                  &model_info) != SACCADE_OK ||
    ...
    model_desc.device_id = device.stable_id;
    SaccadeModelHandle model = 0;
    if (inference.ops.create_model(inference.context, &model_desc, &model) != SACCADE_OK ||
        model == 0) {
        return 5;
    }
    ...
    if (inference.ops.create_context(inference.context, &context_desc, &context) != SACCADE_OK ||
        context == 0) {
        return 6;
    }
    ...
    if (inference.ops.submit(inference.context, context, &submit, &inference_ticket) !=
            SACCADE_OK ||
        inference.ops.submit(inference.context, context, &submit, &extra_ticket) !=
            SACCADE_ERROR_BUSY) {
        return 7;
    }

    SaccadeInferenceStatus inference_status = output_structure<SaccadeInferenceStatus>();
    if (inference.ops.poll(inference.context, context, inference_ticket, &inference_status) !=
            SACCADE_OK ||
        inference_status.state != SACCADE_TICKET_RUNNING ||
        inference.ops.poll(inference.context, context, inference_ticket, &inference_status) !=
            SACCADE_OK ||
    ...
    size_t required = 0;
    if (inference.ops.collect(inference.context, context, inference_ticket,
                              {inference_output.data(), 8}, &required) != SACCADE_ERROR_CAPACITY ||
        ...
        inference.ops.collect(inference.context, context, inference_ticket,
                              ...
                              &required) != SACCADE_OK ||
        read_u64_le(inference_output.data()) != 42 ||
        read_u64_le(inference_output.data() + 8) != 7 ||
        read_u64_le(inference_output.data() + 16) != 8 ||
        read_u64_le(inference_output.data() + 24) != 9) {
        return 9;
    }
    ...
    if (capture.ops.acquire(capture.context, stream, 0, &captured) != SACCADE_OK ||
        captured.frame == 0 ||
        capture.ops.acquire(capture.context, stream, 0, &captured) != SACCADE_ERROR_BUSY) {
        return 15;
    }
    uint32_t damage_count = 0;
    if (capture.ops.copy_damage(capture.context, stream, captured.frame, nullptr, 0,
                                &damage_count) != SACCADE_ERROR_CAPACITY ||
        damage_count != 1) {
        return 16;
    }
    SaccadeRectI32 damage{};
    if (capture.ops.copy_damage(capture.context, stream, captured.frame, &damage, 1,
                                &damage_count) != SACCADE_OK ||
    ...
    if (capture.ops.memory_stats(capture.context, stream, &memory) != SACCADE_OK ||
        memory.host_committed != 1234 || memory.device_imported != 5678) {
        return 31;
    }
    ...
    const uint8_t commands[] = {4, 3, 2, 1};
    ...
    overlay_frame.commands = {commands, sizeof(commands)};
    ...
    const auto overlay_observations = backend.observations();
    if (overlay_observations.overlay_submissions != 1 ||
        overlay_observations.last_scene_epoch != 55 ||
        overlay_observations.last_transform_epoch != 66 ||
        overlay_observations.last_command_hash == 0 || overlay_observations.overlay_visible != 1) {
        return 20;
    }
    memory = output_structure<SaccadeMemoryStats>();
    ...
    if (overlay.ops.submit(overlay.context, overlay_handle, &overlay_frame) !=
            SACCADE_ERROR_BACKEND ||
    ...
    if (accessibility.ops.request(accessibility.context, &query, &accessibility_ticket) !=
            SACCADE_OK ||
        accessibility.ops.request(accessibility.context, &query, &extra_ticket) !=
            SACCADE_ERROR_BUSY) {
        return 23;
    }
    SaccadeAccessibilityStatus accessibility_status =
        output_structure<SaccadeAccessibilityStatus>();
    if (accessibility.ops.poll(accessibility.context, accessibility_ticket,
                               &accessibility_status) != SACCADE_OK ||
        accessibility_status.state != SACCADE_TICKET_RUNNING ||
        accessibility.ops.wait(accessibility.context, accessibility_ticket, 1,
                               &accessibility_status) != SACCADE_OK ||
        accessibility_status.state != SACCADE_TICKET_COMPLETE ||
        accessibility_status.snapshot == 0) {
        return 24;
    }
    std::array<uint8_t, 24> accessibility_output{};
    if (accessibility.ops.collect(accessibility.context, accessibility_status.snapshot,
                                  {accessibility_output.data(), 8},
                                  &required) != SACCADE_ERROR_CAPACITY ||
        ...
        accessibility.ops.collect(accessibility.context, accessibility_status.snapshot,
                                  ...
                                  &required) != SACCADE_OK ||
        read_u64_le(accessibility_output.data()) != window.stable_id ||
        read_u64_le(accessibility_output.data() + 8) != 88 ||
        read_u64_le(accessibility_output.data() + 16) != 99 ||
        accessibility.ops.release(accessibility.context, accessibility_status.snapshot) !=
            SACCADE_OK) {
        return 25;
    }
    memory = output_structure<SaccadeMemoryStats>();
    if (accessibility.ops.memory_stats(accessibility.context, &memory) != SACCADE_OK ||
        memory.host_committed != 1234 || memory.device_imported != 5678) {
        return 35;
    }
    ...
    if (accessibility.ops.synchronize(accessibility.context, 1) != SACCADE_ERROR_BACKEND ||
        ...
        accessibility.ops.request(accessibility.context, &query, &accessibility_ticket) !=
            SACCADE_OK ||
        accessibility.ops.cancel(accessibility.context, accessibility_ticket) != SACCADE_OK ||
        accessibility.ops.poll(accessibility.context, accessibility_ticket,
                               &accessibility_status) != SACCADE_OK ||
        accessibility_status.state != SACCADE_TICKET_CANCELLED ||
        accessibility.ops.poll(accessibility.context, accessibility_ticket,
                               &accessibility_status) != SACCADE_ERROR_STALE_HANDLE) {
        return 36;
    }
    for (int index = 0; index < 9; ++index) {
        if (accessibility.ops.request(accessibility.context, &query, &accessibility_ticket) !=
                SACCADE_OK ||
            accessibility.ops.cancel(accessibility.context, accessibility_ticket) != SACCADE_OK ||
            accessibility.ops.wait(accessibility.context, accessibility_ticket, 0,
                                   &accessibility_status) != SACCADE_OK ||
            accessibility_status.state != SACCADE_TICKET_CANCELLED) {
            return 38;
        }
    ...
    const uint8_t actions[] = {1, 2, 3};
    SaccadeInputPlanDesc plan{};
    plan.struct_size = static_cast<uint32_t>(sizeof(plan));
    plan.api_version = SACCADE_API_VERSION;
    plan.session_epoch = 111;
    plan.transform_epoch = 222;
    plan.actions = {actions, sizeof(actions)};
    ...
    if (input.ops.poll(input.context, input_ticket, &input_status) != SACCADE_OK ||
        ...
        input_status.state != SACCADE_TICKET_COMPLETE ||
        input_status.completed_actions != sizeof(actions) ||
    ...
    if (input.ops.execute(input.context, &plan, &input_ticket) != SACCADE_ERROR_BACKEND ||
        ...
        input.ops.memory_stats(input.context, &memory) != SACCADE_OK ||
        memory.host_committed != 1234 || memory.device_imported != 5678) {
        return 28;
    }
    ...
    if (final_observations.inference_submissions != 2 || final_observations.captured_frames != 1 ||
        final_observations.accessibility_requests != 11 ||
        final_observations.input_executions != 2 || final_observations.release_all_calls != 1 ||
        allocations != 0) {
        return 29;
    }
