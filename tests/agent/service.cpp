...
SaccadeResult acquire_scoped_scene(void* context, const SaccadeAgentScope& scope, const SaccadeAgentFreshness& freshness,
    ...
    if (freshness.policy == SACCADE_AGENT_FRESHNESS_AFTER_GENERATION && fixture->scene.header->scene_epoch <= freshness.after_generation) {
...
SaccadeResult execute_background_press(void* context, uint64_t, const SaccadeAgentGeneration&, uint64_t, const SaccadeAgentTarget&,
...
SaccadeResult prepare_window_activation(void* context, uint64_t, uint64_t process_id, uint64_t window_id, uint64_t, uint64_t, bool dry_run,
...
int main() {
    ...
    saccade::agent::ServiceConfig service_config{&fixture,    acquire_scoped_scene, execute_plan, read_physical_state,
                                                 abort_input, cycle_window,         capabilities};
    ...
    if (service.process({reinterpret_cast<const uint8_t*>(&observe), sizeof(observe)}, capabilities, 2, {output.data(), output.size()},
        ...
        return result(TestResult::freshness_failed);
    observe.request_id = 1;
    if (service.process({reinterpret_cast<const uint8_t*>(&observe), sizeof(observe)}, capabilities, 1001, {output.data(), output.size()},
                        &output_size) != SACCADE_OK ||
        reinterpret_cast<const SaccadeAgentObserveCompletion*>(output.data())->result != SACCADE_AGENT_ERROR_TIMEOUT)
        return result(TestResult::freshness_failed);
    if (service.process({reinterpret_cast<const uint8_t*>(&observe), sizeof(observe)}, capabilities, 1002, {output.data(), output.size()},
                        &output_size) != SACCADE_ERROR_BUSY)
        return result(TestResult::freshness_failed);
    service.cancel_pending_requests();
    if (service.process({reinterpret_cast<const uint8_t*>(&observe), sizeof(observe)}, capabilities, 1003, {output.data(), output.size()},
                        &output_size) != SACCADE_OK ||
    ...
    if (service.process({action_bytes.data(), action_bytes.size()}, capabilities, 1, {output.data(), output.size()}, &output_size) !=
            SACCADE_OK ||
        reinterpret_cast<const SaccadeAgentActionCompletion*>(output.data())->result != SACCADE_AGENT_OK || fixture.activation_calls != 2 ||
        fixture.plans != plans_before_activation + 1 ||
        (reinterpret_cast<const SaccadeAgentActionResult*>(output.data() + sizeof(SaccadeAgentActionCompletion))->flags &
