...
constexpr std::array<uint8_t, 162> model{};
...
void report_runtime_module() noexcept {
    ...
    const DWORD wide_size = module == nullptr ? 0 : GetModuleFileNameW(module, wide.data(), static_cast<DWORD>(wide.size()));
    if (wide_size == 0 || wide_size >= wide.size())
        return;
    std::array<char, 2048> utf8{};
    const int utf8_size = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, wide.data(), static_cast<int>(wide_size), utf8.data(),
                                              static_cast<int>(utf8.size() - 2U), nullptr, nullptr);
    if (utf8_size <= 0)
        return;
...
int main() {
    ...
    const ComPtr<ID3D12Resource> input = create_buffer(graphics.device(), D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_COPY_DEST,
                                                       D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    const ComPtr<ID3D12Resource> output =
        create_buffer(graphics.device(), D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    ...
    if (stats.runs != 2 || stats.failures != 0 || stats.model_bytes != model.size() || stats.input_bytes != sizeof(float) * 4U ||
        stats.output_bytes != sizeof(float) * 4U || stats.input_count != 1 || stats.output_count != 1) {
