...
int main() {
    ...
    if (!text.empty() || text.size() != 0 || text.capacity() != 64 ||
        text.truncated() || text.c_str()[0] != '\0') {
        return 2;
    }

    if (!text.append("frame ") || !text.append_unsigned(42) ||
        !text.append(" at ") || !text.append_signed(-7) ||
        !equals(text.view(), "frame 42 at -7")) {
        return 3;
    }

    text.reset();
    if (!text.append_unsigned(std::numeric_limits<uint64_t>::max()) ||
        !text.append(' ') ||
    ...
    if (bounded.append("abcdef") || !bounded.truncated() ||
        bounded.size() != bounded.capacity() ||
    ...
    bounded.reset();
    if (bounded.truncated() || !bounded.append("abcde") ||
        !equals(bounded.view(), "abcde")) {
        return 7;
    }

    StackStringBuilder<3> number;
    if (number.append_unsigned(1234) || !number.truncated() ||
        !equals(number.view(), "123")) {
        return 8;
    }

    StackStringBuilder<0> empty;
    if (empty.append('x') || !empty.truncated() || !empty.view().empty() ||
        empty.c_str()[0] != '\0') {
        return 9;
    }
    ...
    saccade::test::begin_allocation_tracking();
    {
        ...
        measured_complete = measured.append("frame ") &&
                            measured.append_unsigned(120) &&
