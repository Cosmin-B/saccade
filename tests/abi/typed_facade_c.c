...
_Static_assert(offsetof(SaccadeIOSurfaceFrameDesc, transform_epoch) == 40, "IOSurface descriptor transform epoch offset");
_Static_assert(offsetof(SaccadeWin32CaptureFrameDesc, transform_epoch) == 40, "Win32 capture descriptor transform epoch offset");
_Static_assert(offsetof(SaccadeWin32CaptureFrameDesc, ready_fence) == 48, "Win32 capture descriptor ready fence offset");
_Static_assert(offsetof(SaccadeWin32CaptureFrameDesc, ready_value) == 56, "Win32 capture descriptor ready value offset");
...
int main(void) {
    const SaccadeIOSurfaceFrameDesc iosurface_desc =
        {(uint32_t)sizeof(SaccadeIOSurfaceFrameDesc), SACCADE_API_VERSION, UINT64_C(7), 0, 0, 1920, 1080, UINT64_C(1), UINT64_C(2), {0}};
    ...
    if ((result != IOSURFACE_RESULT) || (frame != UINT64_C(202)) || (iosurface_calls != 1) || (win32_capture_calls != 0)) {
        return 2;
    }
