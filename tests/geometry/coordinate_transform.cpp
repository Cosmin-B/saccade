...
bool maps_corner(saccade::geometry::CoordinateTransform* transform, saccade::geometry::PointQ8 source, int32_t x, int32_t y) noexcept {
    saccade::geometry::PointQ8 mapped{};
    return transform->map_point(source, &mapped) == SACCADE_OK && point_is(mapped, x, y);
}

void rotate_edges(saccade::geometry::QuarterTurn rotation, int32_t source_width, int32_t source_height, int32_t left, int32_t top,
                  int32_t right, int32_t bottom, int32_t* rotated_left, int32_t* rotated_top, int32_t* rotated_right,
                  int32_t* rotated_bottom) noexcept {
    switch (rotation) {
...
int main() {
    ...
    if (transform.initialize(rotated) != SACCADE_OK || !maps_corner(&transform, {0, 0}, q8(350), q8(25)) ||
        !maps_corner(&transform, {q8(100), 0}, q8(350), q8(125)) || !maps_corner(&transform, {0, q8(200)}, q8(-50), q8(25)) ||
    ...
    if (transform.initialize(fractional) != SACCADE_OK || transform.map_rect_clipped({q8(1), q8(2), q8(1), q8(3)}, &mapped) != SACCADE_OK) {
        return 9;
    }
    ...
    if (transform.inverse(&inverse) != SACCADE_OK || inverse.descriptor().epoch != fractional.epoch ||
        inverse.descriptor().rotation != QuarterTurn::clockwise_0 || inverse.descriptor().source_space != fractional.destination_space ||
    ...
    for (int32_t y = 0; y <= q8(7); y += 19) {
        for (int32_t x = 0; x <= q8(3); x += 13) {
            PointQ8 forward{};
            PointQ8 back{};
            if (transform.map_point({x, y}, &forward) != SACCADE_OK || inverse.map_point(forward, &back) != SACCADE_OK ||
                std::abs(back.x - x) > 2 || std::abs(back.y - y) > 3) {
                return 12;
            }
        }
    }

    constexpr std::array<QuarterTurn, 4> rotations{QuarterTurn::clockwise_0, QuarterTurn::clockwise_90, QuarterTurn::clockwise_180,
                                                   QuarterTurn::clockwise_270};
        ...
        for (int32_t source_width : source_extents) {
            for (int32_t source_height : source_extents) {
                for (int32_t destination_width : destination_extents) {
                    const int32_t destination_height = destination_extents[static_cast<size_t>(source_width) % destination_extents.size()];
                    TransformDesc desc{};
                    ...
                    for (int32_t top = 0; top < source_height; top += y_step) {
                        for (int32_t left = 0; left < source_width; left += x_step) {
                            ...
                            RectQ8 output{};
                            if (checked.map_rect_clipped({desc.source.x + left, desc.source.y + top, right - left, bottom - top},
                                                         &output) != SACCADE_OK ||
                            ...
                            rotate_edges(rotation, source_width, source_height, left, top, right, bottom, &rotated_left, &rotated_top,
                                         &rotated_right, &rotated_bottom);
                            const bool swaps = rotation == QuarterTurn::clockwise_90 || rotation == QuarterTurn::clockwise_270;
    ...
    for (uint32_t index = 0; index < 10000; ++index) {
        const RectQ8 input{q8(-2000) + static_cast<int32_t>(index % 100U) * q8(40), q8(-1000) + static_cast<int32_t>(index / 100U) * q8(20),
                           q8(12), q8(8)};
