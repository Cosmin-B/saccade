...
constexpr std::array<uint8_t, 5> original_text{static_cast<uint8_t>('h'), static_cast<uint8_t>('e'), static_cast<uint8_t>('l'),
...
SaccadeResult capture_execution(void* context, SaccadeSpanU8 plan, uint32_t available_permissions, uint64_t now_ns) noexcept {
    ...
    if (validated != SACCADE_OK || plan.size > capture->bytes.size())
        return validated;
...
SaccadeSpanU8 make_scene(ScenePacket* packet, uint64_t current_scene_epoch = scene_epoch, uint64_t current_frame_id = frame_id) noexcept {
...
const SaccadeInputCommand* captured_commands(const ExecutionCapture& capture) noexcept {
    return reinterpret_cast<const SaccadeInputCommand*>(capture.bytes.data() + captured_header(capture)->commands_offset);
}
...
int main() {
    ...
    if (scenes.publish_copy(make_scene(&packet)) != SACCADE_OK)
        return result(TestResult::scene_publish_failed);
    ...
    auto single = config(first_plan_id, saccade::interaction::SelectionMode::single, saccade::interaction::ActionKind::click);
    if (engine.begin(single) != SACCADE_OK)
        return result(TestResult::single_begin_failed);
    saccade::application::SessionEvent event{};
    if (engine.set_target_position(9) != SACCADE_ERROR_INVALID_ARGUMENT || engine.set_target_position(0) != SACCADE_OK ||
        engine.enter_symbol(invalid_symbol, action_time_ns, &event) != SACCADE_ERROR_NOT_FOUND || engine.prefix_count() != 0)
        return result(TestResult::invalid_prefix_failed);
    if (enter_label(&engine, 0, &event) != SACCADE_OK || !event.action_executed || engine.active() || capture.calls != 1 ||
        captured_header(capture)->plan_id != first_plan_id || captured_header(capture)->command_count != 1 ||
        captured_commands(capture)[0].kind != SACCADE_INPUT_COMMAND_CLICK || captured_commands(capture)[0].x_q8 != 319 ||
        captured_commands(capture)[0].y_q8 != 319)
        return result(TestResult::single_execution_failed);

    auto dual = config(second_plan_id, saccade::interaction::SelectionMode::dual, saccade::interaction::ActionKind::drag);
    ...
    auto multi = config(third_plan_id, saccade::interaction::SelectionMode::multi, saccade::interaction::ActionKind::click);
    if (engine.begin(multi) != SACCADE_OK || enter_label(&engine, 0, &event) != SACCADE_OK ||
        enter_label(&engine, 1, &event) != SACCADE_OK || engine.confirm(action_time_ns, &event) != SACCADE_OK || !event.action_executed ||
        captured_header(capture)->command_count != 2)
        return result(TestResult::multi_execution_failed);

    std::array<uint8_t, original_text.size()> mutable_text = original_text;
    auto text = config(fourth_plan_id, saccade::interaction::SelectionMode::single, saccade::interaction::ActionKind::text);
    text.request.text = {mutable_text.data(), mutable_text.size()};
    if (engine.begin(text) != SACCADE_OK)
    ...
    if (text_command.kind != SACCADE_INPUT_COMMAND_TEXT || text_command.payload_size != original_text.size() ||
        std::memcmp(capture.bytes.data() + text_command.payload_offset, original_text.data(), original_text.size()) != 0)
        return result(TestResult::text_copy_failed);

    auto stable_refresh = config(sixth_plan_id, saccade::interaction::SelectionMode::dual, saccade::interaction::ActionKind::drag);
    if (engine.begin(stable_refresh) != SACCADE_OK)
    ...
    auto* refreshed_targets = reinterpret_cast<SaccadeTargetRecord*>(packet.bytes.data() + sizeof(SaccadeTargetPacketHeader));
    std::swap(refreshed_targets[0], refreshed_targets[2]);

    int32_t refreshed_drop_x_q8 = 0;
    for (uint32_t index = 0; index < target_count; ++index) {
        if (refreshed_targets[index].target_id != frozen_labels[1].target_id)
            continue;
    ...
    const saccade::application::SessionEpochs refreshed_epochs{refreshed_scene_epoch, transform_epoch, topology_epoch, permission_epoch,
                                                               focus_id};
    ...
    for (uint32_t index = 0; index < target_count; ++index) {
        ...
        if (refreshed_label.target_id != frozen_labels[index].target_id ||
            refreshed_label.symbol_count != frozen_labels[index].symbol_count || refreshed_label.symbols != frozen_labels[index].symbols ||
    ...
    if (enter_label(&engine, 1, &event) != SACCADE_OK || !event.action_executed || engine.active() ||
        captured_header(capture)->plan_id != sixth_plan_id || captured_header(capture)->command_count != expected_drag_commands ||
        captured_commands(capture)[2].x_q8 != refreshed_drop_x_q8) {
        return result(TestResult::stable_refresh_failed);
    }

    auto stale = config(fifth_plan_id, saccade::interaction::SelectionMode::single, saccade::interaction::ActionKind::click);
    if (engine.begin_latest(stale) != SACCADE_OK)
