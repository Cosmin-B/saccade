...
bool valid(const DebuggerLayout& layout, int32_t width, int32_t height) noexcept {
    ...
    for (uint32_t index = 0; index < rects.size(); ++index) {
        if (!inside(rects[index], width, height))
            return false;
        for (uint32_t other = index + 1U; other < rects.size(); ++other) {
            if (overlaps(rects[index], rects[other]))
                return false;
        }
    }
    return true;
...
int main() {
    ...
    if (saccade::application::make_debugger_layout(860, 560, &standard) != SACCADE_OK || standard.compact || !valid(standard, 860, 560) ||
        standard.views.x != 16 || standard.views.y != 16 || standard.content.y != 52 || standard.actions[0].width != 116 ||
        standard.fault.width != 150 || standard.arm_fault.width != 156)
        return 1;

    DebuggerLayout compact{};
    if (saccade::application::make_debugger_layout(640, 480, &compact) != SACCADE_OK || !compact.compact || !valid(compact, 640, 480) ||
        compact.actions[0].y != compact.actions[3].y || compact.fault.y <= compact.actions[0].y || compact.arm_fault.y != compact.fault.y)
        return 2;

    DebuggerLayout minimum{};
    if (saccade::application::make_debugger_layout(saccade::application::debugger_minimum_width,
                                                   saccade::application::debugger_minimum_height, &minimum) != SACCADE_OK ||
        !minimum.compact || !valid(minimum, saccade::application::debugger_minimum_width, saccade::application::debugger_minimum_height))
        return 3;
