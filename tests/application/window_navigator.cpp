...
int main() {
    ...
    constexpr std::array<saccade::application::WindowDirection, 4> directions{saccade::application::WindowDirection::left,
                                                                              saccade::application::WindowDirection::right,
                                                                              saccade::application::WindowDirection::up,
                                                                              saccade::application::WindowDirection::down};
    constexpr std::array<uint64_t, 4> expected{11, 16, 13, 16};
    for (uint32_t index = 0; index < directions.size(); ++index)
        if (navigator.directional(snapshot, 10, directions[index], &selected) != SACCADE_OK || selected != expected[index])
    ...
    tied.count = 3;
    if (navigator.directional(tied, 100, saccade::application::WindowDirection::right, &selected) != SACCADE_OK || selected != 101)
        return static_cast<int>(TestResult::direction_failed);
    ...
    if (saccade::scene::build_window_scene(config, snapshot.windows.data(), snapshot.count, {bytes.data(), bytes.size()}, &byte_size) !=
            SACCADE_OK ||
        saccade::scene::validate_packet({bytes.data(), byte_size}, &scene) != SACCADE_OK || scene.header->target_count != 6 ||
        scene.targets[0].target_id != 10 || scene.targets[0].window_id != 10 || scene.targets[0].role != SACCADE_TARGET_ROLE_WINDOW ||
        scene.targets[0].capability_bits != SACCADE_TARGET_CAPABILITY_WINDOW_ACTIVATE || scene.target_text(0).size != 6 ||
        std::memcmp(scene.target_text(0).data, "Window", 6) != 0)
