...
backend::d3d12::DirectMlExecutionPolicy execution_policy(application::ComputePolicy policy) noexcept {
    if (policy == application::ComputePolicy::cpu_only) return backend::d3d12::DirectMlExecutionPolicy::software_only;
...
uint32_t required_compute_capability(application::ComputePolicy policy) noexcept {
    if (policy == application::ComputePolicy::cpu_only) return SACCADE_PROVIDER_CAPABILITY_CPU;
    ...
    if (policy == application::ComputePolicy::cpu_and_accelerator) return SACCADE_PROVIDER_CAPABILITY_ACCELERATOR;
    return 0;
}
...
bool virtual_desktop(const geometry::DisplaySnapshot& snapshot, VirtualDesktop* output) noexcept {
    if (output == nullptr || snapshot.epoch == 0 || snapshot.count == 0) return false;
    ...
    if (left < INT32_MIN || top < INT32_MIN || right <= left || bottom <= top ||
        static_cast<uint64_t>(right - left) > UINT32_MAX || static_cast<uint64_t>(bottom - top) > UINT32_MAX)
        return false;
...
bool system_dark_theme() noexcept {
    ...
    const LSTATUS status = RegGetValueW(HKEY_CURRENT_USER, personalize_key, L"AppsUseLightTheme", RRF_RT_REG_DWORD,
                                        nullptr, &apps_use_light_theme, &size);
...
SaccadeResult DesktopPipeline::initialize(const DesktopPipelineConfig& config) noexcept {
    ...
    if (config.artifact_path == nullptr || config.shader_directory == nullptr || config.settings == nullptr ||
        config.verifier.verify == nullptr || config.start_time_ns == 0 ||
    ...
    result = input_.initialize(desktop, {this, submit_with_send_input, activate_window, preflight_input},
                               permission_epoch_, pointer.x, pointer.y);
    if (result == SACCADE_OK) input_initialized_ = true;
    if (result != SACCADE_OK) return fail(result, DesktopPipelineStage::input);
    const NeuralBridgeConfig bridge_config{inference_.runtime(), capture_provider_.device(),
                                           capture_provider_.context(), provider_.graphics_device()};
    result = bridge_.initialize(bridge_config);
    if (result == SACCADE_OK) bridge_initialized_ = true;
    ...
    if (graphics == nullptr) return fail(SACCADE_ERROR_STATE, DesktopPipelineStage::provider);
    ...
    result =
        overlays_.initialize(graphics, overlay_queue, config.shader_directory, {this, load_overlay, observe_overlay});
...
SaccadeResult DesktopPipeline::apply_settings(const application::SettingsDocument& settings, uint64_t now_ns) noexcept {
    ...
    if (result != SACCADE_OK) return result;
    VirtualDesktop desktop{};
    if (!virtual_desktop(displays_.snapshot(), &desktop)) return SACCADE_ERROR_STATE;
    const geometry::PointQ8 pointer = pointer_position();
    const geometry::PointQ8 center = desktop_center(desktop);
    settings_ = settings;
    ...
    if (result == SACCADE_OK) {
        if ((previous_source == application::TargetSource::semantic ||
             previous_source == application::TargetSource::fused) &&
            source_ != application::TargetSource::semantic && source_ != application::TargetSource::fused) {
            semantic_window_id_ = 0;
        }
        if (previous_scope != scope_) semantic_window_id_ = 0;
...
SaccadeResult DesktopPipeline::publish_overlays() noexcept {
    ...
    for (uint32_t index = 0; index < snapshot.count; ++index) {
        ...
        result = runtime_.compose_overlay(config, &overlay_workspace_,
                                          {overlay_arena_.data() + offset, overlay_arena_.size() - offset}, &composed);
        if (result != SACCADE_OK) return result;
...
SaccadeResult DesktopPipeline::start_overlay() noexcept {
    if (overlay_dirty_) {
        ...
        if (published != SACCADE_OK) return fail(published, DesktopPipelineStage::overlay);
    }
...
SaccadeResult DesktopPipeline::request(application::Command command, uint64_t now_ns) noexcept {
    ...
    if (topology_pending_) {
        ...
        if (synchronized != SACCADE_OK) return synchronized;
    }
    ...
    if (window_navigation_command(command)) return navigate_window(command, now_ns, false);
...
SaccadeResult DesktopPipeline::navigate_window(application::Command command, uint64_t now_ns,
                                               bool agent_preflight) noexcept {
    ...
    if (agent_preflight) {
        ...
        if (topology_pending_ || displays_.snapshot().epoch != expected_topology_epoch ||
            permission_epoch_ != expected_permission_epoch ||
            reinterpret_cast<uintptr_t>(GetForegroundWindow()) != current) {
            return fail(SACCADE_ERROR_STALE_HANDLE, DesktopPipelineStage::input);
        }
        if (!input_available_ ||
            qualify_surface(action_point_qualifier_).disposition != ActionPointDisposition::qualified) {
            return fail(SACCADE_ERROR_PERMISSION, DesktopPipelineStage::input);
        }
        if (!target_window_available(selected)) return fail(SACCADE_ERROR_NOT_FOUND, DesktopPipelineStage::input);
    }
...
SaccadeResult DesktopPipeline::resolve_scope(geometry::RectQ8* output, uint64_t* display_id) noexcept {
    *display_id = 0;
    ...
    if (scope_ == application::TargetScope::active_window) {
        ...
        RECT bounds{};
        if (window == nullptr || GetWindowRect(window, &bounds) == 0 || bounds.right <= bounds.left ||
            bounds.bottom <= bounds.top || !q8(bounds.left, &output->x) || !q8(bounds.top, &output->y) ||
            !q8(bounds.right - bounds.left, &output->width) || !q8(bounds.bottom - bounds.top, &output->height))
    ...
    VirtualDesktop desktop{};
    if (!virtual_desktop(snapshot, &desktop) || desktop.width > INT32_MAX || desktop.height > INT32_MAX ||
...
SaccadeResult DesktopPipeline::offer_frames(uint64_t now_ns, uint32_t* offered) noexcept {
    *offered = 0;
    ...
    if (source_count == 0) return fail(SACCADE_ERROR_NOT_FOUND, DesktopPipelineStage::capture_set);
    for (uint32_t index = 0; index < snapshot.count; ++index) {
        const geometry::DisplaySurface& display = snapshot.displays[index];
        if (scope_filter_enabled_ && !intersects(display.desktop_bounds, scope_rect_)) continue;
...
SaccadeResult DesktopPipeline::request_semantic(uint64_t frame_id, uint64_t transform_epoch,
    ...
    if (width > INT32_MAX || height > INT32_MAX) return SACCADE_ERROR_CAPACITY;
...
SaccadeResult DesktopPipeline::invalidate_semantic() noexcept {
    ...
    if (result == SACCADE_OK) semantic_window_id_ = 0;
    return result;
}
...
SaccadeResult DesktopPipeline::advance(uint64_t now_ns, DesktopPipelineAdvance* output) noexcept {
    ...
    *output = {};
    ...
    if (!input_available_ || input_.synthetic_input_active()) {
        const bool surface_qualified =
            qualify_surface(action_point_qualifier_).disposition == ActionPointDisposition::qualified;
        if (!input_available_) {
            if (!surface_qualified) return SACCADE_ERROR_PERMISSION;
            const SaccadeResult restored = set_input_available(true, now_ns);
            if (restored != SACCADE_OK) return restored;
    ...
    if (semantic_window_id_ != 0 && semantic_window_id_ != reinterpret_cast<uintptr_t>(GetForegroundWindow())) {
        ...
        if (invalidated != SACCADE_OK) return fail(invalidated, DesktopPipelineStage::accessibility);
    }
    ...
    const bool semantic_source =
        source_ == application::TargetSource::semantic || source_ == application::TargetSource::fused;
    if (topology_ready && capture_running_ && now_ns >= next_capture_ns_ &&
    ...
    if (topology_ready && overlay_running_) {
        ...
        for (uint32_t index = 0; index < snapshot.count; ++index) {
            ...
            if (!slot.present_pending_ && slot.reveal_ticks_ == 0 && !active_animation) continue;
            ...
            if (presented == SACCADE_OK) {
                ...
                if (slot.reveal_ticks_ != 0) --slot.reveal_ticks_;
            }
        }
    }
    if (!runtime_.active()) {
...
SaccadeResult DesktopPipeline::update_input_desktop() noexcept {
    VirtualDesktop desktop{};
    if (!virtual_desktop(displays_.snapshot(), &desktop)) return SACCADE_ERROR_CAPACITY;
...
SaccadeResult DesktopPipeline::synchronize_topology() noexcept {
    ...
    if (result == SACCADE_OK) result = captures_.synchronize(displays_.snapshot());
    if (result == SACCADE_OK && overlays_initialized_) result = overlays_.synchronize(displays_.snapshot());
    if (result == SACCADE_OK) {
        overlay_frames_.fill(OverlayFrameSlot{});
        for (uint32_t index = 0; index < displays_.snapshot().count; ++index)
            overlay_frames_[index].display_id_ = displays_.snapshot().displays[index].display_id;
...
SaccadeResult DesktopPipeline::refresh_topology() noexcept {
    ...
    if (!topology_pending_) return SACCADE_OK;
    result = synchronize_topology();
    if (result == SACCADE_ERROR_BUSY) return SACCADE_OK;
...
SaccadeResult DesktopPipeline::preflight_input(void* context, const input::PlanView& plan, uint32_t command_index,
    ...
    if (active_window != nullptr) (void)GetWindowThreadProcessId(active_window, &active_process);
    ...
    const bool absolute = command_index < plan.header->command_count &&
                          (plan.commands[command_index].flags & SACCADE_INPUT_COMMAND_ABSOLUTE) != 0;
    const bool window_activation = command_index < plan.header->command_count &&
                                   plan.commands[command_index].kind == SACCADE_INPUT_COMMAND_WINDOW_ACTIVATE;
    if (!state.surface_secure && !absolute) {
        const uint64_t qualified_window =
            plan.header->window_id == 0 || window_activation ? state.window_id : plan.header->window_id;
        state.surface_secure =
            pipeline->action_point_qualifier_.qualify_focus(qualified_window) != ActionPointDisposition::qualified;
    }
    if (absolute) {
        state.target_point_secure = pipeline->action_point_qualifier_.qualify(
                                        plan.commands[command_index].x_q8, plan.commands[command_index].y_q8,
...
SaccadeResult DesktopPipeline::cycle_agent_window(void* context, bool backward) noexcept {
    ...
    return pipeline->navigate_window(
        backward ? application::Command::window_cycle_backward : application::Command::window_cycle_forward, 0, true);
}
...
bool DesktopPipeline::semantic_query_current(void* context, const SaccadeAccessibilityQueryDesc& query) noexcept {
    ...
    if (pipeline == nullptr || !pipeline->input_available_ || query.session_epoch != pipeline->config_.start_time_ns ||
        ...
        (pipeline->source_ != application::TargetSource::semantic &&
         pipeline->source_ != application::TargetSource::fused)) {
        return false;
    }

    const HWND window = GetForegroundWindow();
    if (reinterpret_cast<uintptr_t>(window) != query.window_id) return false;
    RECT bounds{};
    return window != nullptr && GetWindowRect(window, &bounds) != 0 && bounds.left == query.scope.x &&
           bounds.top == query.scope.y && bounds.right - bounds.left == query.scope.width &&
           bounds.bottom - bounds.top == query.scope.height;
}
...
SaccadeResult DesktopPipeline::activate_window(void*, uint64_t window_id) noexcept {
    ...
    if (window == nullptr || IsWindow(window) == 0) return SACCADE_ERROR_NOT_FOUND;
    if (IsIconic(window) != 0) (void)ShowWindowAsync(window, SW_RESTORE);
...
SaccadeResult DesktopPipeline::load_overlay(void* context, uint64_t display_id,
    ...
    auto* pipeline = static_cast<DesktopPipeline*>(context);
    if (pipeline == nullptr || output == nullptr) return SACCADE_ERROR_INVALID_ARGUMENT;
    if (!pipeline->runtime_.active()) return SACCADE_ERROR_NOT_FOUND;
    ...
    if (slot == nullptr || slot->byte_size_ == 0) return SACCADE_ERROR_NOT_FOUND;
    *output = {};
...
void DesktopPipeline::observe_overlay(void* context, uint64_t, SaccadeResult result,
                                      const backend::d3d12::OverlaySubmission*) noexcept {
    auto* pipeline = static_cast<DesktopPipeline*>(context);
    if (pipeline != nullptr && result != SACCADE_OK && result != SACCADE_ERROR_BUSY &&
...
SaccadeResult DesktopPipeline::shutdown() noexcept {
    ...
    if (bridge_initialized_) {
        ...
        if (stopped == SACCADE_OK) bridge_initialized_ = false;
        preserve_first_error(stopped, &result);
    }
    ...
    if (overlays_initialized_) {
        ...
        if (stopped == SACCADE_OK) overlays_initialized_ = false;
        preserve_first_error(stopped, &result);
    }
