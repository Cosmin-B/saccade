...
SaccadeResult AgentPipe::initialize(AgentPipeConfig config, AgentPipeStorage* storage) noexcept {
    ...
    DWORD token_size = 0;
    const BOOL token_read = GetTokenInformation(token, TokenUser, token_user_.data(), static_cast<DWORD>(token_user_.size()), &token_size);
    CloseHandle(token);
    if (!token_read)
        return token_size > token_user_.size() ? SACCADE_ERROR_CAPACITY : SACCADE_ERROR_PERMISSION;
    ...
    if (!InitializeAcl(reinterpret_cast<PACL>(acl_.data()), static_cast<DWORD>(acl_.size()), ACL_REVISION) ||
        !AddAccessAllowedAce(reinterpret_cast<PACL>(acl_.data()), ACL_REVISION, GENERIC_READ | GENERIC_WRITE, token_user->User.Sid) ||
    ...
    if (config.endpoint != nullptr) {
        ...
        if (!ProcessIdToSessionId(GetCurrentProcessId(), &session_id))
            return SACCADE_ERROR_BACKEND;
        const int written =
            swprintf_s(endpoint_.data(), endpoint_.size(), L"\\\\.\\pipe\\Saccade.Agent.v1.%lu", static_cast<unsigned long>(session_id));
        if (written <= 0 || static_cast<size_t>(written) >= endpoint_.size())
            return SACCADE_ERROR_CAPACITY;
    }
...
SaccadeResult AgentPipe::create_pipe() noexcept {
    pipe_ = CreateNamedPipeW(endpoint_.data(), PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
                             PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT | PIPE_REJECT_REMOTE_CLIENTS, 1, 64U * 1024U,
                             64U * 1024U, 0, &security_attributes_);
...
SaccadeResult AgentPipe::begin_connect() noexcept {
    ...
    if (ConnectNamedPipe(pipe_, &operation_))
        return SACCADE_OK;
...
SaccadeResult AgentPipe::begin_write(size_t byte_size) noexcept {
    if (byte_size == 0 || byte_size > storage_->response.size())
        return SACCADE_ERROR_CAPACITY;
    operation_ = {};
    operation_.hEvent = event_;
    ResetEvent(event_);
    ...
    if (!valid) {
        ...
        if (client_capability_bits_ == 0) {
            ...
            if (BCryptGenRandom(nullptr, reinterpret_cast<PUCHAR>(completion.service_nonce),
                                static_cast<ULONG>(sizeof(completion.service_nonce)), BCRYPT_USE_SYSTEM_PREFERRED_RNG) != 0)
                return SACCADE_ERROR_BACKEND;
            authenticated_ = true;
        }
    }
...
SaccadeResult AgentPipe::complete_read(uint32_t byte_size, uint64_t now_ns) noexcept {
    ...
    SaccadeResult result = authenticated_ ? config_.request(config_.context, {storage_->request.data(), byte_size}, client_capability_bits_,
