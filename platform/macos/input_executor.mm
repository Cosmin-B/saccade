...
constexpr auto modifier_order = std::to_array<ModifierMapping>({{SACCADE_INPUT_MODIFIER_SHIFT, kVK_Shift, kCGEventFlagMaskShift},
...
constexpr auto button_order = std::to_array<uint32_t>({SACCADE_INPUT_BUTTON_LEFT, SACCADE_INPUT_BUTTON_RIGHT, SACCADE_INPUT_BUTTON_MIDDLE});
...
bool button_mapping(uint32_t button, CGMouseButton* native_button, CGEventType* down, CGEventType* up, CGEventType* dragged) noexcept {
...
bool add_key(std::array<uint32_t, input::maximum_held_keys>* keys, uint32_t* count, uint32_t usage) noexcept {
    if (*count == input::maximum_held_keys || std::find(keys->begin(), keys->begin() + *count, usage) != keys->begin() + *count) {
        return false;
    }
    (*keys)[(*count)++] = usage;
    return true;
...
int64_t point_delta(int32_t q8) noexcept {
    ...
    return value >= 0 ? (value + rounding) >> coordinate_fraction_bits : -((-value + rounding) >> coordinate_fraction_bits);
}

class NativeEmitter final {
  public:
    NativeEmitter(const InputSink& sink, CGEventRef mouse, CGEventRef keyboard, CGEventRef scroll, CGPoint point) noexcept
    ...
    bool button(uint32_t button, bool up, uint32_t click_count) noexcept {
        ...
        CGEventType dragged = kCGEventNull;
        if (!button_mapping(button, &native_button, &down, &up_type, &dragged))
            return false;
        if (!up)
            transient_buttons_ |= button;
        if (!submit_mouse(up ? up_type : down, native_button, click_count))
            return false;
        if (up)
            transient_buttons_ &= ~button;
        return true;
    ...
    bool scroll(int32_t x_q8, int32_t y_q8) noexcept {
        ...
        CGEventSetIntegerValueField(scroll_, kCGScrollWheelEventFixedPtDeltaAxis1, static_cast<int64_t>(y_q8) * scroll_fixed_point_scale);
        CGEventSetIntegerValueField(scroll_, kCGScrollWheelEventFixedPtDeltaAxis2, static_cast<int64_t>(x_q8) * scroll_fixed_point_scale);
    ...
    bool key(uint32_t usage, bool up) noexcept {
        CGKeyCode keycode = 0;
        if (!keycode_from_hid_usage(usage, &keycode))
            return false;
        if (!up && !add_key(&transient_keys_, &transient_key_count_, usage))
            return false;
        if (!submit_keyboard(up ? kCGEventKeyUp : kCGEventKeyDown, keycode, nullptr, 0))
            return false;
        if (up)
            remove_key(&transient_keys_, &transient_key_count_, usage);
        return true;
    }
    ...
    bool modifiers(uint32_t modifiers, bool up) noexcept {
        for (uint32_t offset = 0; offset < modifier_order.size(); ++offset) {
            ...
            if ((modifiers & mapping.bit) == 0)
                continue;
            if (up)
                flags_ &= ~mapping.flag;
            else
                flags_ |= mapping.flag;
            if (!submit_keyboard(kCGEventFlagsChanged, mapping.keycode, nullptr, 0))
                return false;
        }
        return true;
    }
    ...
    void release_transient() noexcept {
        for (uint32_t button_value : button_order) {
            if ((transient_buttons_ & button_value) != 0)
                (void)button(button_value, true, first_click_count);
        }
        ...
        for (const ModifierMapping& mapping : modifier_order) {
            if ((flags_ & mapping.flag) != 0)
                active_modifiers |= mapping.bit;
        }
  ...
  private:
    ...
    bool submit(CGEventRef event) noexcept {
        ...
        ++submit_calls_;
        if (!sink_.submit(sink_.context, event))
            return false;
        ++total_;
        return true;
    }
...
bool emit_command(NativeEmitter* emitter, SaccadeSpanU8 bytes, const SaccadeInputCommand& command, uint32_t held_buttons) noexcept {
    switch (command.kind) {
    ...
    case SACCADE_INPUT_COMMAND_BUTTON_UP:
        if ((command.flags & SACCADE_INPUT_COMMAND_ABSOLUTE) != 0 && !emitter->move(command.x_q8, command.y_q8, held_buttons))
            return false;
    ...
    case SACCADE_INPUT_COMMAND_CLICK:
        ...
        for (uint32_t click = 0; click < command.data1; ++click) {
            ...
            if (!emitter->button(command.data0, false, click_count) || !emitter->button(command.data0, true, click_count))
                return false;
        }
        return emitter->modifiers(command.data2, true);
    case SACCADE_INPUT_COMMAND_SCROLL:
        return emitter->move(command.x_q8, command.y_q8, held_buttons) && emitter->scroll(command.delta_x_q8, command.delta_y_q8);
    case SACCADE_INPUT_COMMAND_KEY_DOWN:
    case SACCADE_INPUT_COMMAND_KEY_UP:
        return emitter->modifiers(command.data1, false) && emitter->key(command.data0, command.kind == SACCADE_INPUT_COMMAND_KEY_UP) &&
...
bool dictionary_number(CFDictionaryRef dictionary, CFStringRef key, int64_t* output) noexcept {
    ...
    return number != nullptr && CFGetTypeID(number) == CFNumberGetTypeID() && CFNumberGetValue(number, kCFNumberSInt64Type, output);
}
...
bool ax_geometry(AXUIElementRef element, CGRect* output) noexcept {
    ...
    CGPoint position{};
    CGSize size{};
    const bool valid = position_result == kAXErrorSuccess && size_result == kAXErrorSuccess && position_value != nullptr &&
                       size_value != nullptr && CFGetTypeID(position_value) == AXValueGetTypeID() &&
                       ...
                       AXValueGetValue(static_cast<AXValueRef>(size_value), kAXValueTypeCGSize, &size) && size.width > 0 && size.height > 0;
...
SaccadeResult InputExecutor::initialize(const Desktop& desktop, const InputSink& sink, uint64_t permission_epoch, int32_t pointer_x_q8,
    ...
    *output = {};
    ...
    auto emit_one = [&](const SaccadeInputCommand& command) noexcept {
        return command.kind == SACCADE_INPUT_COMMAND_WINDOW_ACTIVATE
                   ? sink_.activate_window != nullptr && sink_.activate_window(sink_.context, active_plan_.header->window_id) == SACCADE_OK
                   : emit_command(&emitter, active_bytes_, command, physical_.state().buttons | emitter.transient_buttons());
    };
    for (;;) {
...
SaccadeResult InputExecutor::emit_release(const input::SyntheticRelease& release) noexcept {
    const SaccadePhysicalInputState state = physical_.state();
    NativeEmitter emitter(sink_, mouse_event_, keyboard_event_, scroll_event_, point_from_q8(state.pointer_x_q8, state.pointer_y_q8));
    for (uint32_t button : button_order) {
    ...
    for (uint32_t index = 0; index < release.held_key_count; ++index) {
        if (!emitter.key(release.held_keys[index], true))
            return SACCADE_ERROR_BACKEND;
    }
    if (!emitter.modifiers(release.modifiers, true))
        return SACCADE_ERROR_BACKEND;
...
SaccadeResult activate_window_public(void*, uint64_t window_id) noexcept {
    ...
    @autoreleasepool {
        ...
        if (application_element == nullptr)
            return SACCADE_ERROR_BACKEND;
        ...
        if (copied == kAXErrorSuccess && values != nullptr && CFGetTypeID(values) == CFArrayGetTypeID()) {
            ...
            for (CFIndex index = 0; index < CFArrayGetCount(ax_windows); ++index) {
                const auto candidate = static_cast<AXUIElementRef>(const_cast<void*>(CFArrayGetValueAtIndex(ax_windows, index)));
                CGRect bounds{};
                if (!ax_geometry(candidate, &bounds) || !CGRectEqualToRect(bounds, target_bounds))
                    continue;
                best = candidate;
                ++exact_matches;
            }
        }
        if (best != nullptr && exact_matches == 1)
            CFRetain(best);
        else
            best = nullptr;
        if (values != nullptr)
        ...
        if (AXUIElementIsAttributeSettable(best, kAXMinimizedAttribute, &minimized_settable) == kAXErrorSuccess && minimized_settable) {
