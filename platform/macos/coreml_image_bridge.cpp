...
SaccadeResult CoreMlImageBridge::begin_scope(SceneCaptureSet* captures, const SceneCaptureFrame* frames,
    ...
    if (!initialized_ || frames == nullptr || displays == nullptr || display_count == 0 || display_count > geometry::display_capacity ||
        !geometry::rect_valid(scope) || source_id == 0) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
...
SaccadeResult CoreMlImageBridge::release_captures() noexcept {
    ...
    for (uint32_t index = 0; index < capture_count_; ++index) {
        ...
        if (frame.frame.frame == 0)
            continue;
        const SaccadeResult result = frame.release != nullptr ? frame.release(frame.release_context, frame) : capture_set_->release(frame);
        if (result != SACCADE_OK)
            return result;
        frame = {};
