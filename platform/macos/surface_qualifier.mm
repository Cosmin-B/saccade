...
AccessibilityRole role_for(NSString* role) noexcept {
    if ([role isEqualToString:(NSString*)kAXWindowRole])
        return AccessibilityRole::window;
    if ([role isEqualToString:(NSString*)kAXSheetRole])
        return AccessibilityRole::sheet;
    if ([role isEqualToString:(NSString*)kAXTextFieldRole])
...
AccessibilitySubrole subrole_for(NSString* subrole) noexcept {
    if ([subrole isEqualToString:(NSString*)kAXStandardWindowSubrole])
        return AccessibilitySubrole::standard_window;
    if ([subrole isEqualToString:(NSString*)kAXDialogSubrole])
        return AccessibilitySubrole::dialog;
    if ([subrole isEqualToString:(NSString*)kAXSystemDialogSubrole])
        return AccessibilitySubrole::system_dialog;
    if ([subrole isEqualToString:(NSString*)kAXSystemFloatingWindowSubrole]) {
        return AccessibilitySubrole::system_floating_window;
    }
    if ([subrole isEqualToString:(NSString*)kAXFloatingWindowSubrole])
        return AccessibilitySubrole::floating_window;
    if ([subrole isEqualToString:(NSString*)kAXSecureTextFieldSubrole])
        return AccessibilitySubrole::secure_text_field;
...
bool copy_role(AXUIElementRef element, AccessibilityActionPoint* output) noexcept {
    ...
    value = nullptr;
    const AXError protected_error =
        AXUIElementCopyAttributeValue(element, (__bridge CFStringRef)NSAccessibilityContainsProtectedContentAttribute, &value);
...
bool system_surface(const AccessibilityActionPoint& point) noexcept {
    return point.role == AccessibilityRole::system_dialog || point.subrole == AccessibilitySubrole::system_dialog ||
           point.role == AccessibilityRole::system_floating_window || point.subrole == AccessibilitySubrole::system_floating_window;
}
...
bool native_accessibility(void*, uint64_t frontmost_pid, AccessibilityEvidence* output) noexcept {
    ...
    const AXError focused_error = AXUIElementCopyAttributeValue(application, kAXFocusedUIElementAttribute, &focused_value);
    CFRelease(application);
    if (focused_error != kAXErrorSuccess || focused_value == nullptr || CFGetTypeID(focused_value) != AXUIElementGetTypeID()) {
        if (focused_value != nullptr)
            CFRelease(focused_value);
        output->status = focused_value == nullptr ? AccessibilitySampleStatus::missing : AccessibilitySampleStatus::wrong_type;
        return true;
    }
    ...
    for (;;) {
        ...
        if (parent_error == kAXErrorNoValue || parent_error == kAXErrorAttributeUnsupported)
            break;
        if (parent_error != kAXErrorSuccess || parent_value == nullptr || CFGetTypeID(parent_value) != AXUIElementGetTypeID()) {
            if (parent_value != nullptr)
                CFRelease(parent_value);
            output->status = parent_value == nullptr ? AccessibilitySampleStatus::missing : AccessibilitySampleStatus::wrong_type;
            break;
        }
    ...
    const AXError children_error =
        AXUIElementCopyAttributeValue(static_cast<AXUIElementRef>(focused_value), kAXChildrenAttribute, &children_value);
...
uint32_t classify(const SessionEvidence& session, uint64_t frontmost_pid, const AccessibilityEvidence& accessibility,
    ...
    if (!session.type_valid)
        reasons |= surface_reason_wrong_type;
    if (!session.on_console || !session.login_done)
        reasons |= surface_reason_session_missing;
    if (frontmost_pid == 0)
        reasons |= surface_reason_frontmost_missing;
    ...
    if (accessibility.status == AccessibilitySampleStatus::success) {
        ...
        if (accessibility.focus_pid != frontmost_pid)
            reasons |= surface_reason_pid_mismatch;
        ...
        for (uint32_t i = 0; i < accessibility.ancestor_count; ++i) {
            ...
            if (point.role == AccessibilityRole::system_dialog || point.subrole == AccessibilitySubrole::system_dialog) {
                reasons |= surface_reason_system_dialog;
            }
            if (point.role == AccessibilityRole::system_floating_window || point.subrole == AccessibilitySubrole::system_floating_window) {
        ...
        for (uint32_t i = 0; i < accessibility.action_point_count; ++i) {
            if (secure_point(accessibility.action_points[i]))
                reasons |= surface_reason_secure_input;
        }
        for (uint32_t i = 0; i < accessibility.ancestor_count; ++i) {
            if (secure_point(accessibility.ancestors[i]))
                reasons |= surface_reason_secure_input;
        }
        if (accessibility.secure_text_present)
            reasons |= surface_reason_secure_input;
    }
    if (secure_input)
        reasons |= surface_reason_secure_input;
...
ActionPointDisposition qualify_action_point(int32_t x_q8, int32_t y_q8) noexcept {
    ...
    const AXError hit =
        AXUIElementCopyElementAtPosition(system, static_cast<float>(x_q8) / 256.0F, static_cast<float>(y_q8) / 256.0F, &element);
    CFRelease(system);
...
SurfaceQualifier::SurfaceQualifier(const SurfaceQualifierProbe& probe) noexcept : probe_(probe), owner_thread_(current_thread()) {}
...
bool SurfaceQualifier::refresh() noexcept {
    ...
    const bool sampled =
        (probe_.sample_session == nullptr ? native_session(nullptr, &session) : probe_.sample_session(probe_.context, &session)) &&
        ...
        (probe_.sample_accessibility == nullptr ? native_accessibility(nullptr, frontmost_pid, &accessibility)
    ...
    const bool sampled_clock =
        probe_.sample_time == nullptr ? native_time(nullptr, &sampled_time) : probe_.sample_time(probe_.context, &sampled_time);
    ++snapshot_.epoch;
    ...
    snapshot_.disposition = snapshot_.reason_bits == surface_reason_none ? SurfaceDisposition::qualified : SurfaceDisposition::blocked;
