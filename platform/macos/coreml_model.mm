...
bool compute_policy_valid(CoreMlComputePolicy policy) noexcept {
    return policy == CoreMlComputePolicy::all || policy == CoreMlComputePolicy::cpu_and_gpu || policy == CoreMlComputePolicy::cpu_only ||
           policy == CoreMlComputePolicy::cpu_and_neural_engine;
}
...
bool dimensions_equal(NSArray<NSNumber*>* shape, NSUInteger first, NSUInteger second) noexcept {
    return shape != nil && shape.count == 2 && shape[0].unsignedIntegerValue == first && shape[1].unsignedIntegerValue == second;
}

bool array_is_float32(MLFeatureDescription* description, NSArray<NSNumber*>* expected_shape) noexcept {
    return description != nil && description.type == MLFeatureTypeMultiArray && description.multiArrayConstraint != nil &&
...
bool string_from_span(SaccadeSpanU8 bytes, NSString** output) noexcept {
    ...
    NSString* value = [[NSString alloc] initWithBytes:bytes.data length:static_cast<NSUInteger>(bytes.size) encoding:NSUTF8StringEncoding];
    if (value == nil)
        return false;
    *output = value;
    return true;
...
bool model_url(const char* root_text, SaccadeSpanU8 locator, NSURL** output) noexcept {
    if (root_text == nullptr || root_text[0] == '\0' || output == nullptr)
        return false;
    NSString* locator_text = nil;
    if (!string_from_span(locator, &locator_text))
        return false;
    ...
    if (![candidate_value hasPrefix:prefix])
        return false;
    *output = candidate;
    return true;
}
...
SaccadeResult coreml_bundle_digest(const char* bundle_path, std::array<uint8_t, CC_SHA256_DIGEST_LENGTH>* output) noexcept {
    ...
    try {
        ...
        std::sort(files.begin(), files.end(),
                  [](const BundleFile& left, const BundleFile& right) noexcept { return left.relative_ < right.relative_; });
        ...
        for (const BundleFile& file : files) {
            ...
            if (file.relative_.size() > UINT32_MAX || !digest_u32(&digest, static_cast<uint32_t>(file.relative_.size())) ||
                !digest_update(&digest, file.relative_.data(), file.relative_.size()) || !digest_u64(&digest, file.size_)) {
                return SACCADE_ERROR_BACKEND;
            }
            ...
            while (remaining != 0) {
                ...
                if (stream.gcount() != static_cast<std::streamsize>(requested) || !digest_update(&digest, buffer.data(), requested)) {
                    return SACCADE_ERROR_PERMISSION;
                }
                remaining -= requested;
            }
...
SaccadeResult verify_model_description(MLModel* model, const model::ArtifactView& artifact, const model::coreml::Contract& contract,
                                       NSString* input_name, NSString* target_rows_name, NSString* target_count_name) noexcept {
    ...
    if (input == nil || input.type != MLFeatureTypeImage || input.imageConstraint == nil ||
        input.imageConstraint.pixelFormatType != kCVPixelFormatType_32BGRA || input.imageConstraint.pixelsWide != artifact.input_width ||
...
bool count_from_array(MLMultiArray* array, uint32_t maximum, uint32_t* output) noexcept {
    ...
    [array getBytesWithHandler:^(const void* bytes, NSInteger size) {
      if (bytes == nullptr || size < static_cast<NSInteger>(sizeof(value)))
          return;
...
SaccadeResult CoreMlModel::initialize(const model::ArtifactView& artifact, CoreMlModelConfig config) noexcept {
    Impl& state = impl();
    if (state.initialized_ || config.model_root == nullptr || !zero_reserved(config) || !compute_policy_valid(config.compute_policy)) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    ...
    @autoreleasepool {
        ...
        if (digested != SACCADE_OK)
            return digested;
        if (actual_digest != contract.bundle_sha256)
            return SACCADE_ERROR_PERMISSION;

        NSString* input_name = nil;
        NSString* target_rows_name = nil;
        NSString* target_count_name = nil;
        if (!string_from_span(contract.input_name, &input_name) || !string_from_span(contract.target_rows_name, &target_rows_name) ||
        ...
        if (model == nil || error != nil)
            return SACCADE_ERROR_BACKEND;
        const SaccadeResult valid = verify_model_description(model, artifact, contract, input_name, target_rows_name, target_count_name);
        if (valid != SACCADE_OK)
            return valid;
        state.model_ = model;
...
SaccadeResult CoreMlModel::predict(const CoreMlPrediction& request, SaccadeMutableSpanU8 output, CoreMlPredictionResult* result) noexcept {
    Impl& state = impl();
    if (!state.initialized_ || result == nullptr || request.pixel_buffer == nullptr || request.pixel_format != SACCADE_FORMAT_BGRA8 ||
        request.width != state.artifact_.input_width || request.height != state.artifact_.input_height || request.scope.x != 0 ||
        request.scope.y != 0 || request.scope.width != static_cast<int32_t>(request.width) ||
    ...
    *result = {};
    @autoreleasepool {
        ...
        NSError* error = nil;
        MLDictionaryFeatureProvider* input = [[MLDictionaryFeatureProvider alloc] initWithDictionary:@{state.input_name_ : image}
                                                                                               error:&error];
        ...
        [rows getBytesWithHandler:^(const void* bytes, NSInteger size) {
          const uint64_t required_bytes =
              static_cast<uint64_t>(state.contract_.candidate_capacity) * rows.strides[0].unsignedIntegerValue * sizeof(float);
          ...
          if (bytes == nullptr || size < required)
              return;
          const model::coreml::TargetRows view{bytes, model::coreml::ScalarType::float32, state.contract_.candidate_capacity,
                                               static_cast<uint32_t>(rows.strides[0].unsignedIntegerValue),
                                               model::coreml::target_row_components};
          decoded = model::coreml::decode_target_rows(state.contract_, view, candidate_count, request.scope, state.candidates_.data(),
                                                      state.contract_.candidate_capacity, &decoded_count);
        }];
        ...
        const SaccadeResult published = kernels::targets::postprocess(state.candidates_.data(), decoded_count, config, request.epochs,
