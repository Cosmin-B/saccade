...
bool snapshot_valid(const geometry::DisplaySnapshot& snapshot) noexcept {
    ...
    for (uint32_t index = 0; index < snapshot.count; ++index) {
        if (snapshot.displays[index].display_id == 0) {
            return false;
        }
        for (uint32_t previous = 0; previous < index; ++previous) {
            if (snapshot.displays[previous].display_id == snapshot.displays[index].display_id) {
                return false;
            }
        }
    }
...
bool same_display(const geometry::DisplaySurface& left, const geometry::DisplaySurface& right) noexcept {
...
SaccadeResult SceneCaptureSet::remove(StreamSlot& slot) noexcept {
    ...
    if (slot.running_) {
        ...
        if (stopped != SACCADE_OK && stopped != SACCADE_ERROR_STATE) {
            return stopped;
        }
    }
...
SaccadeResult SceneCaptureSet::synchronize(const geometry::DisplaySnapshot& snapshot) noexcept {
    ...
    if (snapshot.epoch == stats_.topology_epoch) {
        return SACCADE_OK;
    }
    ...
    for (const StreamSlot& slot : streams_) {
        const geometry::DisplaySurface* display = slot.active_ ? snapshot_display(snapshot, slot.display_id_) : nullptr;
        if (slot.active_ && slot.leased_ && (display == nullptr || !same_display(slot.display_, *display))) {
            ++stats_.failures;
            return SACCADE_ERROR_BUSY;
        }
    }
    for (StreamSlot& slot : streams_) {
        const geometry::DisplaySurface* display = slot.active_ ? snapshot_display(snapshot, slot.display_id_) : nullptr;
        if (slot.active_ && (display == nullptr || !same_display(slot.display_, *display))) {
    ...
    for (uint32_t index = 0; index < snapshot.count; ++index) {
        ...
        if (find(display_id) != nullptr) {
            continue;
        }
        StreamSlot* free_slot = nullptr;
        for (StreamSlot& candidate : streams_) {
        ...
        if (result != SACCADE_OK) {
            if (stream != 0) {
                (void)backend_.ops.destroy(backend_.context, stream);
            }
            ++stats_.failures;
            return result;
        }
        free_slot->display_ = snapshot.displays[index];
...
SaccadeResult SceneCaptureSet::acquire(uint64_t display_id, SceneCaptureFrame* output) noexcept {
    ...
    const SaccadeResult imported = provider_->read_native_frame(slot->stream_, frame.frame, &native);
