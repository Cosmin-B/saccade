...
constexpr auto letter_keycodes =
    std::to_array<CGKeyCode>({kVK_ANSI_A, kVK_ANSI_B, kVK_ANSI_C, kVK_ANSI_D, kVK_ANSI_E, kVK_ANSI_F, kVK_ANSI_G, kVK_ANSI_H, kVK_ANSI_I,
...
constexpr auto digit_keycodes = std::to_array<CGKeyCode>(
    {kVK_ANSI_1, kVK_ANSI_2, kVK_ANSI_3, kVK_ANSI_4, kVK_ANSI_5, kVK_ANSI_6, kVK_ANSI_7, kVK_ANSI_8, kVK_ANSI_9, kVK_ANSI_0});

constexpr auto function_keycodes =
    std::to_array<CGKeyCode>({kVK_F1, kVK_F2, kVK_F3, kVK_F4, kVK_F5, kVK_F6, kVK_F7, kVK_F8, kVK_F9, kVK_F10, kVK_F11, kVK_F12});

uint16_t symbol_from_source(TISInputSourceRef source, uint32_t usage) noexcept {
    CGKeyCode keycode = 0;
    if (source == nullptr || !keycode_from_hid_usage(usage, &keycode))
        return 0;
    ...
    const OSStatus translated = layout == nullptr
                                    ? paramErr
    ...
    if (translated != noErr || symbol_count != 1 || (symbols[0] >= 0xd800 && symbols[0] <= 0xdfff))
        return 0;
    return symbols[0];
}
...
bool language_match(const char* requested, CFStringRef candidate, bool exact) noexcept {
    ...
    if (candidate == nullptr || !CFStringGetCString(candidate, candidate_bytes.data(), candidate_bytes.size(), kCFStringEncodingUTF8)) {
        return false;
    }
    ...
    if (!exact) {
        ...
        requested_size = requested_separator == nullptr ? requested_size : static_cast<size_t>(requested_separator - requested);
        candidate_size =
            candidate_separator == nullptr ? candidate_size : static_cast<size_t>(candidate_separator - candidate_bytes.data());
    }
    if (requested_size != candidate_size)
        return false;
    for (size_t index = 0; index < requested_size; ++index) {
        const char left = requested[index] >= 'A' && requested[index] <= 'Z' ? requested[index] + ('a' - 'A') : requested[index];
        const char right =
            candidate_bytes[index] >= 'A' && candidate_bytes[index] <= 'Z' ? candidate_bytes[index] + ('a' - 'A') : candidate_bytes[index];
        if (left != right)
            return false;
    }
    return true;
    ...
    for (uint32_t pass = 0; pass < 2 && selected == nullptr; ++pass) {
        ...
        for (CFIndex source_index = 0; source_index < source_count && selected == nullptr; ++source_index) {
            ...
            for (CFIndex language_index = 0; language_index < language_count; ++language_index) {
                ...
                if (!language_match(language, candidate, exact))
                    continue;
                CFRetain(source);
                selected = source;
                break;
...
uint64_t source_token(TISInputSourceRef source) noexcept {
    const auto identifier =
        source == nullptr ? nullptr : static_cast<CFStringRef>(TISGetInputSourceProperty(source, kTISPropertyInputSourceID));
