...
bool valid_generation(const ExplicitWindowSceneGeneration& generation) noexcept {
    return generation.scene_epoch != 0 && generation.frame_id != 0 && generation.transform_epoch != 0 && generation.topology_epoch != 0 &&
           generation.permission_epoch != 0;
}
...
bool token_matches(const ExplicitWindowActionToken& token, const ExplicitWindowIdentity& identity, uint64_t session_epoch,
    ...
    return token.session_epoch == session_epoch && token.scene_epoch == generation.scene_epoch && token.frame_id == generation.frame_id &&
           token.transform_epoch == generation.transform_epoch && token.topology_epoch == generation.topology_epoch &&
           token.permission_epoch == generation.permission_epoch && token.process_id == identity.process_id &&
           token.window_id == identity.window_id && token.capture_source_id == identity.capture_source_id &&
           same_bounds(token.bounds, identity.bounds);
}
...
SaccadeResult ExplicitWindowSession::publish(const ExplicitWindowIdentity& identity, const ExplicitWindowSceneGeneration& generation,
...
SaccadeResult ExplicitWindowSession::validate(const ExplicitWindowIdentity& identity,
    ...
    if (!active_ || !same_identity(identity_, identity) || !token_matches(token, identity_, session_epoch_, generation_)) {
        return SACCADE_ERROR_STALE_HANDLE;
    }
    return SACCADE_OK;
}
...
ExplicitWindowActivationAdmission ExplicitWindowActivationState::admit(const ExplicitWindowActivationKey& key) noexcept {
    ...
    if (phase_ == Phase::waiting)
        return same_activation_key(key_, key) ? ExplicitWindowActivationAdmission::resume : ExplicitWindowActivationAdmission::busy;
