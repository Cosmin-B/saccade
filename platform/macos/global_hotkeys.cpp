...
uint32_t carbon_modifiers(uint32_t modifiers) noexcept {
    uint32_t result = 0;
    if ((modifiers & SACCADE_INPUT_MODIFIER_SHIFT) != 0)
        result |= shiftKey;
    if ((modifiers & SACCADE_INPUT_MODIFIER_CONTROL) != 0)
        result |= controlKey;
    if ((modifiers & SACCADE_INPUT_MODIFIER_ALT) != 0)
        result |= optionKey;
    if ((modifiers & SACCADE_INPUT_MODIFIER_META) != 0)
        result |= cmdKey;
    return result;
}
...
bool bindings_valid(const application::HotkeyBinding* bindings, uint32_t count) noexcept {
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        CGKeyCode keycode = 0;
        if (!command_valid(binding.command) || binding.physical_key == 0 || !keycode_from_hid_usage(binding.physical_key, &keycode) ||
        ...
        for (uint32_t previous = 0; previous < index; ++previous) {
            if (bindings[previous].physical_key == binding.physical_key && bindings[previous].modifiers == binding.modifiers)
                return false;
        }
    }
    return true;
...
OSStatus hotkey_event(EventHandlerCallRef, EventRef event, void* context) noexcept {
    ...
    const OSStatus read =
        GetEventParameter(event, kEventParamDirectObject, typeEventHotKeyID, nullptr, sizeof(identifier), nullptr, &identifier);
    if (read != noErr || identifier.signature != hotkey_signature)
    ...
    return hotkeys->dispatch_registered_id(identifier.id, timestamp_ns) == SACCADE_OK ? static_cast<OSStatus>(noErr)
...
SaccadeResult GlobalHotkeys::register_binding(uint32_t index) noexcept {
    ...
    if (slot.native_ != nullptr || (slot.binding_.flags & application::hotkey_session_only) != 0)
        return SACCADE_OK;
    ...
    const OSStatus registered =
        RegisterEventHotKey(keycode, carbon_modifiers(slot.binding_.modifiers), identifier, GetApplicationEventTarget(), 0, &native);
...
SaccadeResult GlobalHotkeys::initialize(application::CommandSink sink) noexcept {
    ...
    if (installed != noErr || handler == nullptr)
        return SACCADE_ERROR_BACKEND;
    sink_ = sink;
    handler_ = handler;
    owner_ = pthread_self();
...
SaccadeResult GlobalHotkeys::replace(const application::HotkeyBinding* bindings, uint32_t count) noexcept {
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        if ((bindings[index].flags & application::hotkey_session_only) != 0)
            continue;
        if (suspended_ && (bindings[index].flags & application::hotkey_always_active) == 0)
            continue;
...
SaccadeResult GlobalHotkeys::set_suspended(bool value) noexcept {
    ...
    if (value) {
        ...
        for (; index < binding_count_; ++index) {
            if ((bindings_[index].binding_.flags & (application::hotkey_always_active | application::hotkey_session_only)) != 0)
                continue;
            if (unregister_binding(index) == SACCADE_OK)
                continue;

            for (uint32_t restore = 0; restore < index; ++restore) {
                if ((bindings_[restore].binding_.flags & (application::hotkey_always_active | application::hotkey_session_only)) == 0)
        ...
        for (; index < binding_count_; ++index) {
            ...
            for (uint32_t rollback = 0; rollback < index; ++rollback) {
                if ((bindings_[rollback].binding_.flags & (application::hotkey_always_active | application::hotkey_session_only)) == 0)
...
SaccadeResult GlobalHotkeys::dispatch_registered_id(uint32_t registration_id, uint64_t timestamp_ns) noexcept {
    ...
    if (sink_.command_observed != nullptr)
        sink_.command_observed(sink_.context, timestamp_ns);
    sink_.command(sink_.context, {timestamp_ns, binding.command, binding.physical_key, binding.modifiers, binding.flags});
    ++stats_.dispatched;
    return SACCADE_OK;
}

SaccadeResult GlobalHotkeys::dispatch_physical(uint32_t physical_key, uint32_t modifiers, uint64_t timestamp_ns) noexcept {
    if (!initialized_ || !owns_thread())
        return SACCADE_ERROR_STATE;
    ++stats_.events;
    for (uint32_t index = 0; index < binding_count_; ++index) {
