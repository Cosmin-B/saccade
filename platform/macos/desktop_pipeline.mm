...
SaccadeResult public_window_identity(uint64_t window_id, ExplicitWindowIdentity* output) noexcept {
    ...
    *output = {};
    ...
    CFArrayRef windows = CGWindowListCopyWindowInfo(kCGWindowListOptionIncludingWindow | kCGWindowListExcludeDesktopElements, native_id);
    ...
    CGRect bounds{};
    const bool valid =
        dictionary_i64(description, kCGWindowOwnerPID, &process_id) && process_id > 0 && process_id <= INT32_MAX &&
        dictionary_i64(description, kCGWindowLayer, &layer) && layer == 0 && dictionary_i64(description, kCGWindowSharingState, &sharing) &&
        sharing != kCGWindowSharingNone && onscreen == kCFBooleanTrue && bounds_dictionary != nullptr &&
        CGRectMakeWithDictionaryRepresentation(bounds_dictionary, &bounds) && std::isfinite(bounds.origin.x) &&
        std::isfinite(bounds.origin.y) && std::isfinite(bounds.size.width) && std::isfinite(bounds.size.height) && bounds.size.width > 0 &&
        bounds.size.height > 0 && bounds.origin.x >= INT32_MIN && bounds.origin.x <= INT32_MAX && bounds.origin.y >= INT32_MIN &&
        bounds.origin.y <= INT32_MAX && bounds.size.width <= INT32_MAX && bounds.size.height <= INT32_MAX;
    if (valid) {
...
bool same_explicit_identity(const ExplicitWindowIdentity& left, const ExplicitWindowIdentity& right) noexcept {
    return left.process_id == right.process_id && left.window_id == right.window_id && left.capture_source_id == right.capture_source_id &&
           left.bounds.x == right.bounds.x && left.bounds.y == right.bounds.y && left.bounds.width == right.bounds.width &&
           left.bounds.height == right.bounds.height && left.flags == right.flags && left.reserved == right.reserved;
}
...
SaccadeResult DesktopPipeline::release_explicit_capture_frame(void* context, const SceneCaptureFrame& frame) noexcept {
    return context == nullptr ? SACCADE_ERROR_INVALID_ARGUMENT : static_cast<DesktopPipeline*>(context)->explicit_capture_.release(frame);
}
...
SaccadeResult DesktopPipeline::retire_explicit_agent_scene(ExplicitWindowRetirementReason reason) noexcept {
    if (!explicit_session_.active() && explicit_accessibility_ticket_ == 0 && !explicit_capture_.active() && !explicit_capture_.retiring())
        return SACCADE_OK;
    const SaccadeAccessibilityProviderDesc provider = accessibility_.descriptor();
    ...
    if (explicit_capture_.active() || explicit_capture_.retiring()) {
        const SaccadeResult retired = explicit_capture_.active() ? explicit_capture_.retire(reason) : explicit_capture_.drain_retirement();
        if (retired != SACCADE_OK)
            return retired;
    }
...
SaccadeResult DesktopPipeline::acquire_explicit_agent_scene(const SaccadeAgentScope& scope, const SaccadeAgentFreshness& freshness,
                                                            scene::PacketView* scene, application::InteractionState* output) noexcept {
    if (!capture_permission_available_)
        return SACCADE_ERROR_PERMISSION;
    ...
    if (freshness.policy != SACCADE_AGENT_FRESHNESS_LATEST_VALID && !after_generation)
        return SACCADE_ERROR_INVALID_ARGUMENT;
    if (explicit_session_.active() && (!same_explicit_identity(explicit_identity_, identity) || explicit_source_mode_ != source_mode)) {
    ...
    if (!explicit_session_.active()) {
        ...
        explicit_accessibility_key_ = {session_epoch,       frame_id,          query.transform_epoch, query.topology_epoch,
                                       identity.process_id, identity.window_id};
        explicit_capture_validated_ = false;
        explicit_scene_ready_ = false;
        explicit_scene_size_ = 0;
        explicit_semantic_size_ = 0;
    ...
    if (result != SACCADE_OK) {
        ...
        const SaccadeResult retired =
            retire_explicit_agent_scene(result == SACCADE_ERROR_NOT_FOUND ? ExplicitWindowRetirementReason::disappeared
    ...
    if (!explicit_capture_validated_) {
        ...
        if (result == SACCADE_OK && visual_required) {
            ...
            explicit_runtime_previous_scene_epoch_ = runtime_.acquire_scene(&previous) == SACCADE_OK ? previous.header->scene_epoch : 0;
    ...
    if (!explicit_semantic_ready_) {
        ...
        size_t required = 0;
        result = provider.ops.collect(provider.context, status.snapshot, {explicit_semantic_bytes_.data(), explicit_semantic_bytes_.size()},
                                      &required);
        if (result == SACCADE_OK)
    ...
    if (!explicit_scene_ready_ && explicit_semantic_ready_ && explicit_visual_ready_) {
        ...
        if (source_mode == SACCADE_AGENT_SOURCE_FUSED) {
            ...
            result = scene::fuse(packets.data(), static_cast<uint32_t>(packets.size()),
                                 fusion_config(settings_.detector, artifact_.view().max_targets), epochs, &explicit_fusion_workspace_,
                                 {explicit_scene_bytes_.data(), explicit_scene_bytes_.size()}, &explicit_scene_size_, &fusion_stats);
            if (result != SACCADE_OK)
                return result;
        } else {
...
SaccadeResult DesktopPipeline::acquire_agent_scene(void* context, const SaccadeAgentScope& scope, const SaccadeAgentFreshness& freshness,
                                                   scene::PacketView* scene, application::InteractionState* output) noexcept {
    auto* pipeline = static_cast<DesktopPipeline*>(context);
    ...
    if (result != SACCADE_OK)
        return result;
    if (freshness.policy == SACCADE_AGENT_FRESHNESS_AFTER_GENERATION && scene->header->scene_epoch <= freshness.after_generation)
        return SACCADE_ERROR_BUSY;
...
SaccadeResult DesktopPipeline::execute_agent_background_press(void* context, uint64_t request_id, const SaccadeAgentGeneration& generation,
                                                              uint64_t session_epoch, const SaccadeAgentTarget& target, bool dry_run,
    ...
    *execution = {};
    ...
    const AccessibilityPressRequest requested{key,
                                              target.target_id,
                                              {target.bounds.x_q8, target.bounds.y_q8, target.bounds.width_q8, target.bounds.height_q8}};
    ...
    const auto same_request = [&same_key](const AccessibilityPressRequest& left, const AccessibilityPressRequest& right) noexcept {
        return same_key(left.generation, right.generation) && left.target_id == right.target_id && left.bounds_q8.x == right.bounds_q8.x &&
               left.bounds_q8.y == right.bounds_q8.y && left.bounds_q8.width == right.bounds_q8.width &&
               left.bounds_q8.height == right.bounds_q8.height;
    };
    if (pipeline->agent_press_ticket_ != 0) {
        const bool same_pending =
            pipeline->agent_press_request_id_ == request_id && same_request(pipeline->agent_press_request_, requested);
        AccessibilityPressStatus status{};
...
SaccadeResult DesktopPipeline::prepare_agent_window_activation(void* context, uint64_t request_id, uint64_t process_id, uint64_t window_id,
                                                               uint64_t source_generation, uint64_t deadline_ns, bool dry_run,
                                                               agent::BackgroundActionExecution* execution) noexcept {
    if (context == nullptr || execution == nullptr || request_id == 0 || process_id == 0 || window_id == 0 || source_generation == 0 ||
        deadline_ns == 0 || dry_run) {
        return SACCADE_ERROR_INVALID_ARGUMENT;
    }
    auto* pipeline = static_cast<DesktopPipeline*>(context);
    *execution = {};
    ...
    const uint64_t now_ns = static_cast<uint64_t>(monotonic.tv_sec) * UINT64_C(1'000'000'000) + static_cast<uint64_t>(monotonic.tv_nsec);
    ...
    if (admission == ExplicitWindowActivationAdmission::start) {
        scene::PacketView previous{};
        const uint64_t previous_scene_epoch = pipeline->runtime_.acquire_scene(&previous) == SACCADE_OK ? previous.header->scene_epoch : 0;
