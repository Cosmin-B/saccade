...
| `SaccadeOverlayTarget` | 48 bytes | Stable ID, local target and label positions, confidence, style, and glyphs |
...
publication explicitly, avoiding a misleading key.
...
without waiting. Packet validation occurs once per immutable scene and transform
...
runs once at session activation in the interaction module. None belongs on the 120 Hz
display path.
